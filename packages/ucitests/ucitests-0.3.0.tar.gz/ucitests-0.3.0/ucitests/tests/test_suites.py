...
class TestingSubprocessSuiteWithLoadFile(suites.SubprocessedSuiteWithLoadFile):
    ...
    def setUp(self):
        ...
        fixtures.override_env(self, 'UCISELFTESTS_IN_SUBPROCESS', '1')
...
class TestForSubprocess(unittest.TestCase):
    ...
    def test_fail(self):
        if os.environ.get('UCISELFTESTS_IN_SUBPROCESS', False):
...
class TestRunTestsConcurrent(unittest.TestCase):
    ...
    def run_suite(self, suite):
        ...
        concurrent_suite = testtools.ConcurrentTestSuite(
            suite, runners.split_suite_for(2, self.suite_class))
        res.startTestRun()
...
class TestRunTestsConcurrentPicked(unittest.TestCase):
    ...
    def run_suite(self, suite):
        ...
        # subprocesses.
        concurrent_suite = testtools.ConcurrentTestSuite(
            suite, runners.split_suite_for(3, self.suite_class))
        res.startTestRun()
        concurrent_suite.run(res)
        res.stopTestRun()
        return res
