...
class ForkedSuite(WorkerSuite):
    ...
    def run_parent(self, result):
        ...
        collector = subunit.ByteStreamToStreamResult(self.stream)
    ...
    def run_child(self, result):
        ...
        try:
            result = testtools.ExtendedToStreamDecorator(
                subunit.StreamResultToBytes(self.stream))
    ...
    def run(self, result, debug=False):
        ...
        try:
            if setup_done:
                ...
                # Collect the stream sent by the child
                collector = subunit.ByteStreamToStreamResult(process.stdout)
                collector.run(result)
        finally:
            while self._cleanups:
                cleanup, args, kwargs = self._cleanups.pop()
