...
def main():
    ...
    parser.add_argument('-x','--exclude', help='exclude these stack elements. csv list with choices in [i-elb,e-elb,efs,rds,ecs]',
                                          type=lambda x: are_exclusions_valid(parser, x))
    parser.add_argument('-p','--params',  help='stack parameter overrides (formatted as json string)')
    ...
    stack_params = get_stack_params(action, args.app, args.env, args.vpc, 
    ...
    # **************************** Print Time! ************************************
    user_msg = "%s stack %s with these resources: %s "%(action,stack_name,str(get_inclusions(exclusions)))
    ...
    if args.dryrun:
        ...
        print "cost calculator url: %s"%str(response['Url'])
        print user_msg
