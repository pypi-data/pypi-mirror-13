...
def get_service(app_arg, myservice_arg):
    ...
    if myservice_arg:
        ...
        service_descriptor, service_name, servicefile_path = get_service_from_file(myservice_arg)
...
def get_service_parmeters(app_alias, env_arg, suffix_arg, 
    ...
    # combine service params as well (e.g. so users can add yac-ref references 
    # to custom params in their service definition file)
    service_parmeters.update(service_descriptor["service-params"])

    # create additional params via the service inputs script
    service_input_script = get_variable(service_descriptor, "service-inputs","")
    service_input_params = apply_custom_fxn(service_input_script, service_parmeters)
    ...
    service_parmeters.update(service_input_params)
