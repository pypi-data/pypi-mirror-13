...
def main():
    ...
    parser.add_argument('--validate', 
               help='validate the contents of a service descriptor file (arg is the path to the service descriptor)',
                        type=lambda x: is_valid_file(parser, x))
    parser.add_argument('--register', 
               help='register a service with the yac service registry  (arg is the path to the service descriptor)',
                        type=lambda x: is_valid_file(parser, x))
    ...
    parser.add_argument('--list',  help='list keys of all files in the registry (arg is search string into the registry)') 
    ...
    if args.validate:
        ...
        validation_errors = validate_service(args.validate)

        if validation_errors:
            print ("Your service description failed validation checks. Please fix the following errors. %s"%validation_errors)
        else:
            print "Service is ready to be registered!"

    elif args.register:

        # first validate the service description
        validation_errors = validate_service(args.register)

        if not validation_errors:

            # pull descriptor from file
            service_descriptor, service_name, servicefile_path = get_service_from_file(args.register)
            ...
            try:
                register_service(service_name, args.register, challenge)

                # publish the service 
                publish_service_description(service_name, args.register)
