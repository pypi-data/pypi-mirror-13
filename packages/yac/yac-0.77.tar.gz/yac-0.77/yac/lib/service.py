...
def validate_service(service_descriptor):
    ...
    val_errors = validate_dictionary(vpc_prefs, SUPPORTED_KEYS, REQUIRED_FIELDS)
...
def get_service(service_arg, servicefile_path):
    ...
    if servicefile_path:
        ...
        service_alias = get_alias_from_name(service_name)

    else:

        servicefile_path = ""

        # Treat service_arg as the service name. See if it is complete (i.e.
        # includes a version label)
        if is_service_name_complete(service_arg):
        ...
        # Treat service_arg is a service name that lacks a version
        elif is_service_available_partial_name(service_arg):

            # get complete name
            service_name = get_complete_name(service_arg)
    ...
    return service_descriptor, service_name, service_alias, servicefile_path
...
def get_service_parmeters(service_alias, env_arg, suffix_arg, 
    ...
    set_variable(service_parmeters,"app",service_alias)
    ...
    # add default values for vpc-related params if non were provided
