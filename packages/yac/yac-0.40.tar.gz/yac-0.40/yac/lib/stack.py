...
def get_stack_params(stack_name, action, app_arg, env_arg, vpc_arg, 
                     suffix_arg, noecs_arg, snapshot_arg, 
    ...
    stack_params = [{"ParameterKey": "AppName", "ParameterValue": app_arg.title()},
                    ...
                    {"ParameterKey": "NtpServers", "ParameterValue": naming_std['ntp_servers']['value']},
                    {"ParameterKey": "CostCenter", "ParameterValue": naming_std['cost_center']['value']},
                    {"ParameterKey": "Owner", "ParameterValue": naming_std['owner']['value']},
    ...
    if ('RestoreConfigs' in stack_template['Parameters'] and 'Value' not in stack_template['Parameters']['RestoreConfigs']):
        stack_params.append({"ParameterKey": "RestoreConfigs", "ParameterValue": get_default_container_configs_s3_url(app_arg, env_arg,naming_std['s3_bucket']['value'],'backups')})
    ...
    if (naming_std['outbound-proxy']['name']):
        ...
        stack_params.append({"ParameterKey": "ProxyName", "ParameterValue": naming_std['outbound-proxy']['name'] })
        stack_params.append({"ParameterKey": "ProxyPort", "ParameterValue": naming_std['outbound-proxy']['port'] })
        stack_params.append({"ParameterKey": "HttpsProxy", "ParameterValue": "https://%s:%s"%(naming_std['outbound-proxy']['name'],naming_std['outbound-proxy']['port']) })

    if 'efs' in exclusions:
        # treat AppEFS as a param
        stack_params.append({"ParameterKey": "AppEFS", "ParameterValue": ""})

    for resource in Set(get_stack_elements()) - Set(exclusions):
        ...
        if resource=='rds':
            ...
            if action==BUILDING:
                ...
                if snapshot_arg:
                    stack_params.append({"ParameterKey": "CreateDBFromThisSnapshot", "ParameterValue": snapshot_arg})
