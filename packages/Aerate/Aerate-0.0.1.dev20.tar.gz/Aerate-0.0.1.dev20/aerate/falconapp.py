...
class Aerate(API):
    ...
    def _add_auths_to_route(self, auths=[]):
        self._before.extendleft(Auth._wrap_auths(auths))

    def _remove_auths_from_global(self, auths=[]):
        [self._before.popleft() for x in range(len(Auth._wrap_auths(auths)))]
    ...
    def _add_route_with_auth(self, op_id, res):
        ...
        http_method = self.op_dict[op_id].http_method
        # add authorize method
        if res.get_authorize_method(http_method):
            self._before.append(res.get_authorize_method(http_method))

        # add before hooks
        if res.get_before_method(http_method):
            self._before.append(res.get_before_method(http_method))

        self.add_route(self._get_path_name_by_id(op_id), res)
        # add after hooks
        if res.get_after_method(http_method):
            self._after.append(res.get_after_method(http_method))
