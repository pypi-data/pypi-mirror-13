...
class WebDriverFinalizer:
    ...
    def finalise_saucelabs_driver(driver, test_failed, test_context):
        ...
        try:
            ...
            test_context.logger.info("SauceLabs report: <a href=\"{}\" target=\"_blank\">Saucelabs Report</a>".format(report_url))
        ...
        if driver is not None:
            if test_failed:
                ...
                try:
                    ...
                    logger.info(
                        "Screenshot can be found here: <a href=\"{}\" target=\"_blank\">Screenshot File</a>".format(screenshot_dir))
                except Exception as e:
                    logger.error('Test Finalizer: Exception thrown when attempting screenshot: ' + filename + e.message)
                ...
                try:
                    ...
                    logger.error(
                        'Test Finalizer: Exception thrown when attempting to get the drivers final url'+e.message)
    ...
    def finalise_skygrid_driver_failure(driver, test_context):
        ...
        try:
            ...
            test_context.logger.error(e.message)
        ...
        test_context.logger.info("This test report: <a href=\"{}\" target=\"_blank\">Test Report</a>".format(test_report))
        test_context.logger.info(
            "This test session: <a href=\"{}\" target=\"_blank\">Session Report</a>".format(session_report))
