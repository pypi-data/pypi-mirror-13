...
def watcher_add(path, read_all=False, filter=None):
    ...
    # file
        ...
        if os.path.isfile(path):
            ...
            watch = list(__monitor.add_watch(path, mask).values())[0]
...
def watcher_del(path):
    ...
    if __watcher[path]['watch']:
        result &= list(__monitor.rm_watch(__watcher[path]['watch']).values())[0]
...
def _read_new_line(path):
    ...
    with open(path, 'r') as f:
        ...
        for line in f.readlines():
            ...
            if filter and filter.check(message=line):
