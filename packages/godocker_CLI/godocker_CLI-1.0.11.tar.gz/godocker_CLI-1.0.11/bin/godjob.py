...
def killall(login):
    ...
    HttpUtils.http_delete_request("/api/1.0/user/"+login+"/task", Auth.server, {'Authorization':'Bearer '+Auth.token}, Auth.noCert)
...
def create(name, description, tags, project, cpu, ram, image, external_image, script, interactive, root, volume, array, login, label, parent, tmp_storage, env, cmd):
    """ create a new job """
