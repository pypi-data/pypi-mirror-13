...
def get_flv(video_id, cookies):
    flv_url = get_flv_url(video_id, cookies)
    response = requests.get(flv_url, cookies=cookies)
    content = response.content
    return content

def get_mylist_info(mylist_id):
...
def get_flv_url(video_id, cookies):
    ...
    response = requests.get(getflv_url, cookies=cookies)
    response_body = urllib.parse.unquote(response.text)
    flv_url = re.search(r'url=([^&]+)', response_body).group(1)
    return flv_url

def get_nicohistory_cookie(video_id, cookies):
    response = requests.get('http://www.nicovideo.jp/watch/' + video_id, cookies=cookies)
    return response.cookies
