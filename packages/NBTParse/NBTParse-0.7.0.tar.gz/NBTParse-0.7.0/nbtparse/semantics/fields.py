...
class SingleField(AbstractField):
    ...
    Concrete implementations must override either :meth:`to_python` or
    :meth:`to_python_ex`, and similarly either :meth:`from_python` or
    :meth:`from_python_ex`.
    ...
    def to_python_ex(self, tag: tags.TagMixin, obj: NBT_OBJECT) -> object:
        """Extended form of to_python().

        Takes precedence if both are defined.

        """
        return self.to_python(tag)
    ...
    def from_python_ex(self, value: object, obj: NBT_OBJECT) -> tags.TagMixin:
...
NEW_UUID = _utils.Sentinel(__name__, 'NEW_UUID',"""

Sentinel value for :class:`UUIDField`.

When used as the default value, indicates that the field should create a new
type 4 UUID for each instantiation.
...
class UUIDField(MultiField):
    ...
    A :obj:`default` of :obj:`NEW_UUID` will generate a new UUID every time.
...
NOW = _utils.Sentinel(__name__, 'NOW', """

Sentinel value for :class:`UTCField`.
...
class UTCField(SingleField):
    ...
    A default of :obj:`NOW` indicates the field should default to the current
    time.
    ...
    .. note::
        ...
        The :class:`~datetime.datetime` objects produced by this field are
        always *aware*, with a tzinfo value of :obj:`~datetime.timezone.utc`.
        The field does accept naive objects, but it assumes they represent
        local time, which may not be correct for your application.  In
        particular, the convention of using naive objects to represent UTC is
        not supported and will produce incorrect results unless local time is
        UTC.

    """
    def __init__(self, nbt_name: str, *, default: dt.datetime=EPOCH):
    ...
    @staticmethod
    def to_python(nbt: tags.LongTag) -> dt.datetime:
        utc = dt.timezone.utc
        try:
            return dt.datetime.fromtimestamp(nbt, utc)
        except ValueError:
            return None

    @staticmethod
    def from_python(dt: dt.datetime) -> tags.LongTag:
        posix_time = dt.timestamp()
        return tags.LongTag(posix_time)

    def set_default(self, obj: NBT_OBJECT):
        if self.default is NOW:
            self.__set__(obj, dt.datetime.now(dt.timezone.utc))
        else:
            super().set_default(obj)


SELF_REFERENCE = _utils.Sentinel(__name__, 'SELF_REFERENCE', """
