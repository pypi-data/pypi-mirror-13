...
from hurm.db.entities import Location
...
def duties_table(translate, document, location):
    ...
    allduties = [duty for task in location.tasks for duty in task.duties]
    ...
    for date, duties in groupby(sorted(allduties, key=task_date), task_date):
...
class LocationDuties(BasicEditionPrintout):
    ...
    def getSubTitle(self):
        count = sum(len(task.duties) for task in self.location.tasks)
    ...
    def getElements(self):
        ...
        yield duties_table(self.translate, self.doc, self.location)
