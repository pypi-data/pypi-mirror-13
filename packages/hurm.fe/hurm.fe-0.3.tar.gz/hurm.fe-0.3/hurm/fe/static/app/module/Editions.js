...
Ext.define('HuRM.module.Editions.Actions', {
        ...
        me.availsAction = me.addAction(new Ext.Action({
            ...
            handler: function() {
                var record = me.component.getSelectionModel().getSelection()[0],
                    ...
                    edition = record.get('description'),
                    module = me.module.app.getModule('availabilities-win');
                module.createOrShowWindow(idedition, edition);
            }
        }));
    ...
    showEditWindow: function(record) {
        ...
        var size = desktop.getReasonableWindowSize(550, 440);
    ...
    statics: {
        selectEdition: function(app, callback) {
            ...
            store = Ext.create('Ext.data.Store', {
                ...
                proxy: {
                    type: 'ajax',
                    url: '/data/editions?only_cols=idedition,description&sort=startdate&dir=DESC',
                    reader: {
                        type: 'json',
                        root: 'root',
                        idProperty: 'idedition',
                    ...
                    handler: function() {
                        ...
                        if(frm.isValid()) {
                            var field = frm.getFields().items[0],
                                idedition = field.getValue(),
                                edition = field.getRawValue();
                            win.destroy();
                            callback(idedition, edition);
                        }
                    }
                }]
            });
