        ...
        me.dutiesAction = me.addAction(new Ext.Action({
            ...
            handler: function() {
                var record = me.component.getSelectionModel().getSelection()[0],
                    ...
                    npersons = record.get('npersons'),
                    module = me.module.app.getModule('duties-win');
                module.createOrShowWindow(idtask, location, activity, date,
                                          starttime, endtime, npersons);
            }
        }));
    },
...
Ext.define('HuRM.module.Tasks', {
    ...
    createOrShowWindow: function(idedition, edition, idlocation, location) {
        ...
        if((me.single_location = Ext.isDefined(idlocation))) {
            width = 650;
            height = 350;
        }
        ...
        me.configure(
            ...
            function(done) {
                ...
                if(me.single_location) {
                    ...
                    config.newRecordData.idlocation = idlocation;
                    config.newRecordData.Location = location;
                    config.stickyFilters.push({
                        id: 'location',
                        property: 'idlocation',
                        value: idlocation
                    });
                    title = Ext.String.format(_("Tasks at {0} for edition “{1}”"),
                                              location, edition);
                } else {
                    title = Ext.String.format(_("Tasks related to edition “{0}”"),
                                              edition);
                }
