...
from i3bfutils.tool import (io, cache)
import datetime
import re
import difflib
import operator
...
class AlarmIndicator():
    ...
    @cache.timeout(each='day')
    def indicator(self):
        """String that indicates days until next alarm"""
...
def str2date(datestr, year=datetime.date.today().year):
    ...
    The format of `datestr` is [YEAR-]MONTH-DAY.  If YEAR is not specified in
    `datestr`, the `year` argument is used, which defaults to the current
    year.
    ...
    The special string "EASTER" is replace with the Easter date of `year`.
    """
    ...
    if match is None:
        return str2date(_guess_datename(datestr))
    else:
