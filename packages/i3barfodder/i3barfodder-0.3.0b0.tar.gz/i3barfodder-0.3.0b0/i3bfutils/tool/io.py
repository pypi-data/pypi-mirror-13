...
def push(output=None, **kwargs):
    ...
    If `output` is dict-like or list-like it is converted to JSON first.
    """
...
def read_file(path):
    """Return contents from file `path` (without trailing newline)"""
    try:
...
def handle_click(click, **callbacks):
    ...
    `click` is passed on to callback.
    """
    if click is None:
        return
    else:
