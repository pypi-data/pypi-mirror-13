...
class LyMaker(object):
    ...
    def _addIntro(self):
        ...
        if self.denominator == 4:
            text += 'global = { \\time %d/4 }\nKey = { \\key %s }\n\n' % (self.quarters,key)
            text += 'generalpause = { \\time 1/4 r4 |  \\time %d/4  }\n\n' % (self.quarters)
        else:    
            text += 'global = { \\time %d/8 }\nKey = { \\key %s }\n\n' % (self.eighths,key)
            text += 'generalpause = { \\time 1/4 r4 |  \\time %d/8  }\n\n' % (self.eighths)
        return text
    ...
    def _makeSong(self,chords):
        ...
        dm = DrumMaker(self.eighths,self.downbeats,self.onbeat,5,self.denominator,99,0,0,self.drumsAsInstr)
    ...
    def _makePart(self,name,id,chords,instance,bassmode,drummode,tempo,harmonymode,melodymode,percussion,feel,percbeat,count):
        result = ""    
    ...
    def process(self,r):
        ...
        for s in structure:
            ...
            tempo = part.tempo
            addPause = False
            if i > 0 and part.tempo > 0 and part.tempo != lasttempo:
                addPause = True
                lasttempo = part.tempo    
            elif part.tempo == 0:
                lasttempo = self.tempo

            instance = instances[i]
            i += 1
            text += self._makePart(part.name,s,chords,instance,bassmode,drummode,tempo,harmonymode,melodymode,percussion,feel,percbeat,i)
            if addPause:
                callstring += "\\generalpause\n"
