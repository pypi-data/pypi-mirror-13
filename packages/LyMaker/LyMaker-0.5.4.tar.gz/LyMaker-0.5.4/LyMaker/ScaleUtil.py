...
class Scale(object):
    ...
    def get(self,index):
        if index > len(self.notes)-1:
            print "get: wrong note offset %d" % (index)
            return -1
        else:
    ...
    def getNameFromNote(self,note):
        ...
        if note >= 0 and note < 12:
            ...
            print "Look up of note %d, adjusted to %d, failed, scale: %s" % (save,note,notes)
...
class ScaleUtil(object):
    ...
    def setKey(self,key):
        ...
        #print "scale: %s %s flat %d" % (note,stype,flat)
        self.chords = []
        self.chordNames = []
	...
	if len(self.chords) == 0:
        ...
        for n in self.chordNames:
            ...
            for note in ch:
                if note < 0:
                    print "wrong note offset %d" % (note)
    ...
    def getNoteFromName(self,key):
        ...
        try:
            ...
            try:
                ...
                index = -1
        if index < 0:
            print "getNoteFromName: wrong note offset key %s, offset %d" % (key,index)
...
class ChordProgressions(object):
            ...
            if i > 0:
                ...
                if step > 0:
                    if step == 3 or step == 4:
                        ...
                        offset -= 1
            if (note+offset < 0) or (note+offset > len(self.scale.notes)):
                print "getLilypondChord: wrong note offset root %d, offset %d" % (note,offset)
    ...
    def generate(self):
        ...
        twelvetone = False
        if self.scale.scale_type == Scale.twelvetone:
            twelvetone = True
            print "Generate progressions for twelve-tone"
        bar = 0
        while bar < self.bars:
            beats = self.beats
            out = ""
            if twelvetone:
                ...
                while x < 4:
                    ...
                    if len(out):
                        out += ","
                    out += self.scale.getNameFromNote(self.scale.get(i))
                    x += 1    
                chords += out
                chords += "|"
                bar += 1
                continue
            #else:
                #print "Generate progressions for scale %s" % progressions
            ...
            while beats > 0:
                ...
                while note < self.notes:
                    if len(out) > 0 and not out.endswith(";"):
                        ...
                        #print "chord %d, offset %d step %d" % (chord,offset,step)  
                    if (chord+offset < 0) or (chord+offset > 11):
                        print "generate: wrong note offset root %d, offset %d" % (chord,offset)
