...
class FieldListFile(object):
    ...
    def save(self, filename=None, file_object=None):
        ...
        with info:
            for name in sorted(vars(type(self))):
                ...
                if isinstance(field, Field):
                    if isinstance(field.to_str, collections.Callable):
    ...
    def load(self, filename=None, file_object=None):
        ...
        with info:
            for line in info:
                ...
                if value == 'None':
                    value = None
                elif isinstance(field, Field) and isinstance(field.from_str, collections.Callable):
    ...
    def __repr__(self):
        return "%s(%s)" % (
            ...
            ', '.join(['%s=%r' % x for x in list(self.items())]))
