...
def link_same_files(root_dir, name_prefix=None, always_symlink=False,
    ...
    hash_file_map = {}
    for filepaths in (v for v in size_file_map.values() if len(v) > 1):
        for fpath in filepaths:
            hash_file_map.setdefault(hash_file(fpath), []).append(fpath)

    # Prune unique files
    hash_file_map = {k: v for k, v in hash_file_map.items() if len(v) > 1}
    ...
    for k, files in hash_file_map.items():
