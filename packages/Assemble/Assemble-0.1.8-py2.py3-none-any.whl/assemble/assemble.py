...
class Package(object):
    ...
    def patch_version(self, version, major, minor, patch):
        ...
        if major or minor or patch:
            ...
            major_v, minor_v, patch_v, extra = match.groups()

            if major:
                major_v = int(major_v) + 1
                minor_v = 0
                patch_v = 0
            elif minor:
                ...
                patch_v = int(patch_v) + 1

            version = '.'.join(map(str, [major_v, minor_v, patch_v]))
            if extra:
                version += extra
        ...
        assert version not in run('git', 'tag', capture=True, silent=True)
