...
def patch_version(version, major, minor, patch):
    ...
    """
    package.patch_version(version, major, minor, patch)
...
def requirements_scan(equal, named):
    ...
    not_found = list()
    for module_name, distribution in scan_directory(package.package_path):
        if not distribution:
            not_found.append(module_name)
        else:
            if named:
                click.echo('# - {}'.format(module_name))

            if equal:
                click.echo(distribution)
            else:
                click.echo(distribution.replace('==', '>='))
