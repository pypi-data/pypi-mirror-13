...
class Function:
    ...
    def __mul__(self, f):
        #return Function(compose(self.__f, f))
        #return Function(compose(self, f))
        return compose(self, f)

    def __rmul__(self, f):
        #return Function(compose(f, self.__f))
        #return Function(compose(f, self))
    ...
    def __str__(self):
        return "("+self.__name__+" :: "+createSig(self.__f)+")"
...
class ComposedFunction:
    ...
    def __mul__(self, f):
        return ComposedFunction(*(list(self.__funcs)+[f]))

    def __rmul__(self, f):
        return ComposedFunction(*([f]+list(self.__funcs)))
...
"""
def tupleToSig(t):
    #if all_(lambda x: type(x)==type,t): return " -> ".join(map(type2str,t))
    #return " -> ".join(map(type2str,t))

    if type(t)==type: return type2str(t)
"""

def createSig(f):
    #if isinstance(f,Function):
    #    return createSig(f._f)
