...
class Key(object):
    ...
    def encrypt(self, data, flasg=b''):
        ...
        if not isinstance(data, MPI):
            raise ValueError("data should be a MPI, got {} instead".format(type(data)))

        # We must start by building a S-Expression with flags and data
        #data_sexp = SExpression(u"(data (flags {})(value #{}#))".format(padding.decode(), data.value).encode())
        print(data)
        if flasg == b'':
            data_sexp = SExpression(b"(data (flags)(value %m))", data)
        else:
            data_sexp = SExpression(b"(data (flags %s)(value %m))", flags, data)
    ...
    def decrypt(self, encrypted, flags=b''):
        ...
        if flags == b'':
            sexp_encr = SExpression(b'(enc-val (flags)(%s %S))', self.algo, sexp_algo)
        else:
            sexp_encr = SExpression(u'(enc-val (flags %s)(%s %S))', flags, self.algo, sexp_algo)
