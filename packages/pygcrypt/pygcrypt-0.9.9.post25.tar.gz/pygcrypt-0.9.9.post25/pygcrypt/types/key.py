...
class Key(object):
    ...
    def encrypt(self, data, flasg=b''):
        ...
        if isinstance(data, MPI):
            data_sexp = SExpression(b'(data (flags raw)(value %M))', data)

        # We must start by building a S-Expression with flags and data
        if flasg == b'':
            data_sexp = SExpression(b"(data (flags)(value %b))", len(data), data)
        else:
            data_sexp = SExpression(b"(data (flags %s)(value %b))", flags, len(data), data)
    ...
    def decrypt(self, encrypted, flags=b''):
        ...
        try:
            value = sexp_decrypted.getmpi(1)
            value.format = 'USG'
            return value.to_bytes()
        except:
            value = sexp_decrypted.getdata(1)
            return value
