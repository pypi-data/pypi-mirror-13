...
class CloudHubApplicationTracker(NamedApplicationApiBase):
    ...
    def transactions(self, search_criteria, start_date, end_date, count, offset):
        query = DotDict()
        if search_criteria:
            query.searchParameters = [x.to_dict() for x in search_criteria]
        query.startDate = '2016-01-13T00:00:00.000Z' #start_date.strftime('%Y-%m-%dT%H:%M:%S.%f')
        end_date = '2016-01-14T13:46:54.399Z' #end_date if end_date else datetime.datetime.now()
        query.endDate = end_date #.strftime('%Y-%m-%dT%H:%M:%S.%f')
