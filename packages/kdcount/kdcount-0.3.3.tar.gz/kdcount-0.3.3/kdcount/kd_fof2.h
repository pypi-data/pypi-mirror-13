static int kd_fof2_check(KDNode * node[2], double rmax2,
        kd_fof2_callback callback, void * data);
 ...
 * */
static int kd_fof2_leaf_node(KDNode * node[2], double maxr, struct kdfof2_data * fof);

static int kd_fof2_node_node(KDNode * node[2], double maxr, struct kdfof2_data * fof) {
    ...
    if (distmax >= rmax2) {
        ...
        if(node[open]->dim >= 0) {
            ...
            int rt;
            rt = kd_fof2(node, maxr, callback, data);
            if(rt != 0) {
                node[open] = save;
                return rt;
            }
    ...
    return kd_fof2_check(node, rmax2, callback, data);
}
