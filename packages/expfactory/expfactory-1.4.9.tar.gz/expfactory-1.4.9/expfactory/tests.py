...
def circle_ci_test(experiment_tags,web_folder,delete=True):
    ...
    if "CIRCLE_BRANCH" in os.environ.keys():
        ...
        # Compare commits
        current_commits = []
        for commit in current_build["all_commit_details"]:
            current_commits.append(commit["commit"])
        last_commits = []
        for commit in last_build["all_commit_details"]:
            last_commits.append(commit["commit"])
        files_changed = []
        print current_commits
        print last_commits
        for current_commit in current_commits:
            for last_commit in last_commits:
                files_changed = files_changed + os.popen("git diff %s %s --name-only" %(current_commit,last_commit)).readlines()
