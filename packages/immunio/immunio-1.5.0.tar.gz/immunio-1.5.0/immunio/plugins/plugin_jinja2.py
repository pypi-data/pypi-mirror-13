...
def add_compiler_hooks(timer):
    ...
    def _subparse(orig, parser_self, *args, **kwargs):
        ...
        if not hasattr(parser_self.stream, "_immunio_variables"):
            ...
            # Also initialize the flag for handling `trans` blocks
            parser_self.stream._immunio_inside_trans_block = False
    ...
    def _visit_macro(orig, self, node, frame):
        ...
        self.writeline("l_%s.immunio_name = \"%%s:MACRO:%s\""
                       "%%(_immunio_template_name)" %
                       (node.name, node.name))
        self.writeline("l_%s.immunio_sha = \"%s\"" %
                       (node.name, macro_sum))
        self.writeline("l_%s.immunio_filename = \"%s\"" %
...
def add_template_render_hook(run_hook, timer):
    ...
    def _macro_call(orig, self, *args, **kwargs):
        """
        ...
        name = getattr(self, "immunio_name", self.name)
        sha = getattr(self, "immunio_sha")
        filename = getattr(self, "immunio_filename")
    ...
    @monkeypatch("jinja2.environment.Template.render", timer=timer,
                 report_name="plugin.jinja2.render.template_render")
    def _render(orig, template_self, *args, **kwargs):
        ...
        result = post_render_result(result,
                             template_self._immunio_template_sha,
                             template_self._immunio_template_name,
                             template_self.filename)
        return result
