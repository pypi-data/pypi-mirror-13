...
(function () {
    ...
    widget.$inject = ["$mdTheming", "flexTheme"];
    angular
        .module('flexget.components')
...
$templateCache.put("components/authentication/login.tmpl.html","<div class=\"login\" layout=\"column\" layout-padding=\"\"><div class=\"header\"></div><div><form><md-content layout=\"column\" flex-gt-md=\"35\" flex-offset-gt-md=\"33\"><p style=\"color: orange\" class=\"text-success text-center\" ng-if=\"vm.timeout == \'true\'\">Your session timed out</p><p style=\"color: red\">{{ vm.error }}</p><md-input-container flex=\"\"><label>Username</label> <input id=\"username\" ng-model=\"vm.credentials.username\"></md-input-container><md-input-container><label>Password</label> <input ng-model=\"vm.credentials.password\" type=\"password\"></md-input-container><div layout=\"column\" layout-align=\"center center\"><md-checkbox md-no-ink=\"\" aria-label=\"Remember Me\" ng-model=\"vm.remember\" class=\"md-primary\">Remember Me</md-checkbox></div><md-button type=\"submit\" class=\"md-raised md-primary\" data-ng-click=\"vm.login()\">Login</md-button></md-content></form></div></div>");
$templateCache.put("components/home/home.tmpl.html","<div layout=\"row\" layout-align=\"center start\" flex=\"\"><flex-widget title=\"Flexget WebUI\" content-padding=\"\" title-background=\"orange:600\"><div><p>The interface is not yet ready for end users. Consider this preview only state.</p><p>If you still use it anyways, please do report back to us how well it works, does it leak memory etc.</p><p>We also do really require more man-power to develop webui further. Please join the effort!</p><p><a href=\"http://flexget.com/wiki/Web-UI\">http://flexget.com/wiki/Web-UI</a></p></div></flex-widget></div>");
...
$templateCache.put("directives/widget/widget.tmpl.html","<div class=\"widget md-whiteframe-z2\" ng-class=\"::{\'widget-overlay-title\': vm.overlayTitle}\" flex=\"\" layout=\"{{ vm.widgetLayout }}\"><div class=\"widget-title\" ng-if=\"::(vm.title || vm.subtitle)\" layout=\"row\" layout-padding=\"\" layout-align=\"space-between center\" flex-order=\"{{ vm.titleOrder }}\" ng-style=\"::vm.titleStyle\"><div layout=\"column\" ng-if=\"::vm.avatar\"><img ng-src=\"{{::vm.avatar}}\" class=\"widget-avatar\"></div><div flex=\"\" layout=\"column\"><h3 class=\"md-subhead\" ng-if=\"::vm.title\">{{::vm.title}}</h3><p class=\"md-body-1\" ng-if=\"::vm.subtitle\">{{::vm.subtitle}}</p></div><md-menu ng-if=\"::vm.menu\"><md-button class=\"widget-button md-icon-button\" ng-click=\"$mdOpenMenu()\" aria-label=\"open menu\"><md-icon md-font-icon=\"{{::vm.menu.icon}}\"></md-icon></md-button><md-menu-content><md-menu-item ng-repeat=\"item in ::vm.menu.items\"><md-button ng-click=\"item.click($event)\"><md-icon ng-if=\"::item.icon\" md-font-icon=\"{{::item.icon}}\"></md-icon><span>{{::item.title}}</span></md-button></md-menu-item></md-menu-content></md-menu></div><div class=\"widget-content\" layout=\"{{ vm.contentLayout }}\" layout-align=\"{{ vm.contentLayoutAlign }}\" ng-class=\"{\'layout-padding\': vm.contentPadding}\" ng-transclude=\"\" flex-order=\"{{ vm.contentOrder }}\"></div><div class=\"widget-loading ng-hide\" ng-show=\"vm.loading\" layout=\"\" layout-fill=\"\" layout-align=\"center center\"><div class=\"widget-loading-inner\" ng-show=\"vm.loading\"><md-progress-circular md-mode=\"indeterminate\"></md-progress-circular></div></div></div>");
$templateCache.put("plugins/execute/execute.tmpl.html","<div ng-hide=\"stream\" layout=\"column\" layout-align=\"start center\" flex=\"\"><div><h3 class=\"text-center\">Execute Tasks</h3></div><div class=\"text-center\"><md-chips ng-model=\"vm.executeTasks\" md-autocomplete-snap=\"\" md-require-match=\"false\" secondary-placeholder=\"Enter task\"><md-autocomplete md-items=\"task in vm.queryTasks(searchText)\" md-item-text=\"task\" placeholder=\"Enter task\" md-selected-item=\"selectedItem\" md-search-text=\"vm.searchText\"><span md-highlight-text=\"vm.searchText\">{{task}}</span></md-autocomplete></md-chips><md-button class=\"md-raised md-primary\" ng-click=\"run()\">Execute</md-button></div></div><div ng-show=\"stream\" class=\"execute\" layout=\"column\" flex=\"\"><div><h3 class=\"text-center\">Executing</h3><md-progress-linear md-mode=\"determinate\" value=\"{{ stream.percent }}\"></md-progress-linear></div><div ng-show=\"stream\" layout=\"column\" flex=\"\"><md-tabs md-selected=\"selectedIndex\" md-center-tabs=\"\" flex=\"\"><md-tab ng-repeat=\"task in stream.tasks\"><md-tab-label><div layout=\"column\" layout-fill=\"\" layout-padding=\"\" layout-align=\"center\"><div>{{ task.name }}</div><div><wave-spinner ng-show=\"task.status == \'running\'\" class=\"text-primary\"></wave-spinner></div></div></md-tab-label><md-tab-body><div layout=\"row\" layout-align=\"space-around center\"><div ng-hide=\"task.status == \'complete\'\" class=\"text-center\"><div ng-if=\"task.status == \'pending\'\" class=\"md-display-2\">Pending</div><div ng-if=\"task.status == \'running\'\"><div class=\"md-display-2\">{{ task.phase | executePhaseFilter }}</div><div><small>({{ task.plugin }})</small></div></div></div><div ng-if=\"task.status == \'complete\'\"><md-list><md-subheader class=\"md-no-sticky text-center\"><span>Accepted {{ task.accepted }}</span> <span>Rejected {{ task.rejected }}</span> <span>Accepted {{ task.failed }}</span> <span>Undecided {{ task.undecided }}</span></md-subheader><md-list-item class=\"md-2-line\" ng-repeat=\"entry in task.entries\"><md-icon class=\"fa fa-check-circle\"></md-icon><h4>{{ entry.title }}</h4><p><small>{{ entry.accepted_by }}{{ entry.rejected_by }}{{ entry.failed_by }}</small></p><md-icon class=\"md-secondary\" ng-click=\"doSecondaryAction($event)\" aria-label=\"Chat\" md-svg-icon=\"communication:message\"></md-icon></md-list-item></md-list><div ng-repeat=\"\" flex=\"\">{{ entry.title }}</div></div></div><div layout=\"row\" layout-align=\"space-around center\"><div></div><md-button class=\"md-raised md-primary\" ng-click=\"clear()\">Clear</md-button><div></div></div></md-tab-body></md-tab></md-tabs></div></div>");
$templateCache.put("plugins/history/history.tmpl.html","<md-content flex=\"\"><flex-widget><section ng-repeat=\"(key, value) in vm.entries | groupBy: \'time | limitTo : 10\'\"><md-subheader class=\"md-primary\">{{ key }}</md-subheader><md-list layout-padding=\"\"><md-list-item class=\"md-2-line\" ng-repeat=\"entry in value\"><div class=\"md-list-item-text\"><h3>{{ entry.title }}</h3><p>{{ entry.task }}</p></div></md-list-item></md-list></section></flex-widget></md-content>");
$templateCache.put("plugins/log/log.tmpl.html","<flex-widget layout=\"column\" class=\"log\" content-padding=\"\"><div class=\"widget-title\" layout=\"row\" layout-padding=\"\" layout-align=\"space-between center\"><div flex=\"\" layout=\"row\"><h3 class=\"md-subhead\">Server Log<p class=\"md-body-1\" ng-class=\"{\'text-success\': vm.status == \'Streaming\', \'text-primary\': vm.status == \'Connecting\', \'text-danger\': vm.status == \'Disconnected\'}\">{{ vm.status }}</p></h3><div flex=\"\"></div><md-input-container md-no-float=\"\" class=\"md-block\" style=\"margin: 0px\" flex=\"60\" flex-gt-md=\"70\"><md-icon class=\"fa fa-filter\"></md-icon><label>Filter</label><md-tooltip md-direction=\"left\">Supports and, or, (), and \"\"</md-tooltip><input type=\"text\" aria-label=\"message\" ng-model=\"vm.filter.search\" ng-change=\"vm.refresh()\" ng-model-options=\"vm.refreshOpts\"></md-input-container></div><md-menu><md-button class=\"widget-button md-icon-button\" ng-click=\"$mdOpenMenu()\" aria-label=\"open menu\"><md-icon md-font-icon=\"fa fa-ellipsis-v\"></md-icon></md-button><md-menu-content><md-menu-item layout-margin=\"\"><md-input-container><label>Max Lines</label> <input type=\"number\" aria-label=\"lines\" ng-model=\"vm.filter.lines\" ng-change=\"vm.refresh()\" ng-model-options=\"vm.refreshOpts\"></md-input-container></md-menu-item><md-menu-item><md-button ng-click=\"vm.refresh()\"><md-icon md-font-icon=\"fa fa-refresh\"></md-icon><span>Refresh</span></md-button></md-menu-item><md-menu-item><md-button ng-click=\"vm.stop()\"><md-icon md-font-icon=\"fa fa-stop\"></md-icon><span>Stop</span></md-button></md-menu-item></md-menu-content></md-menu></div><div id=\"log-grid\" ui-grid=\"vm.gridOptions\" ui-grid-auto-resize=\"\" flex=\"\" ui-grid-auto-scroll=\"\"></div></flex-widget>");
