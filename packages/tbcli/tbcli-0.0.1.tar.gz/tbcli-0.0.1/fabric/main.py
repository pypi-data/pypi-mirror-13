...
def main(fabfile_locations=None):
    ...
    try:
        ...
        # Hanle profile
        if 'profile' in state.env:
            from ansible.inventory import Inventory
            # i_profile, i_proj, i_stage = state.env.profile.split(':')
            i_file, i_group = state.env.profile.split(':', 1)
            i_file_path = 'profiles/%s/configs/hosts' % i_file
            state.env.inventory = Inventory(i_file_path)
            state.env.stage = i_group.split(':')[-1]
            i_group = state.env.inventory.get_group(i_group)
            if i_group:
