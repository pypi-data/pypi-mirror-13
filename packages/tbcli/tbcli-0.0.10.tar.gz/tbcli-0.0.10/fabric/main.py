...
def main(fabfile_locations=None):
    ...
    try:
        ...
        # Hanle profile
        if state.env.profile:
            from ansible.inventory import Inventory
            # i_profile, i_proj, i_stage = state.env.profile.split(':')
            # rio:stf:test ---> profiles/rio/stf/hosts
            # rio -> profile, stf -> project, test -> group
            i_file, i_proj, i_stage = state.env.profile.split(':')
            i_file_path = 'profiles/%s/%s/hosts' % (i_file, i_proj)
            state.env.inventory = Inventory(i_file_path)
            state.env.stage = i_stage
            state.env.project = i_proj
            i_group = state.env.inventory.get_group('%s:%s' % (i_proj, i_stage))
            if i_group:
                state.env['hosts'] = [h.get_variables()['inventory_hostname'] for h in i_group.get_hosts()]
                print '--->', state.env.hosts
