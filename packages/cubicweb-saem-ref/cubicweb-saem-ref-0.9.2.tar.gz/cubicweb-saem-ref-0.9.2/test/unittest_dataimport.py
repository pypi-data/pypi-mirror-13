        ...
        expected = [
            ...
            ('AgentPlace',
             ...
             {'name': set([u'Bordeaux (Gironde, France)']),
              ...
              'place_address': set(['5']),
              'vocabulary_source': set([u'http://catalogue.bnf.fr/ark:/12148/cb152418385']),
             },
            ),
            ('AgentPlace',
             '7',
            ...
            blablabla.</p>''']),
              'description_format': set([u'text/html']),
              'vocabulary_source': set([u'http://data.culture.fr/thesaurus/page/ark:/67717/T1-200']),
             },
            ),
            ('AgentFunction',
             '15',
             {'name': set([u'environnement']),
              'function_agent': set(['FRAD033_EAC_00001']),
              'vocabulary_source': set([u'http://data.culture.fr/thesaurus/page/ark:/67717/T1-1074']),
             },
            ),
            ('Agent',
             'CG33-DIRADSJ',
...
class EACDataImportTC(CubicWebTC):
    ...
    def test_FRAD033_EAC_00001(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            self.assertEqual(len(created), 27)
            ...
            yield self._check_structure, cnx, agent
            self.set_description('check History')
            yield self._check_history, cnx, agent
            self.set_description('check Mandate')
            yield self._check_mandate, cnx, agent
            self.set_description('check LegalStatus')
            yield self._check_legal_status, cnx, agent
            self.set_description('check relations')
            yield self._check_eac_relations, cnx, agent
            self.set_description('check vocabulary source')
            yield self._check_vocabulary_source, cnx, agent
            self.set_description('check control')
            yield self._check_control, cnx, agent
    ...
    def _check_vocabulary_source(self, cnx, agent):
        functions = dict((f.name, f) for f in agent.reverse_function_agent)
        self.assertEqual(functions['action sociale'].vocabulary_source[0].cwuri,
                         'http://data.culture.fr/thesaurus/page/ark:/67717/T1-200')
        self.assertEqual(functions['action sociale'].vocabulary_source[0].cw_etype,
                         'ExternalUri')
        self.assertEqual(functions['environnement'].vocabulary_source[0].cwuri,
                         'http://data.culture.fr/thesaurus/page/ark:/67717/T1-1074')
        self.assertEqual(functions['environnement'].vocabulary_source[0].cw_etype,
                         'Concept')
        place = cnx.find('AgentPlace', role=u'siege').one()
        self.assertEqual(place.vocabulary_source[0].cwuri,
                         'http://catalogue.bnf.fr/ark:/12148/cb152418385')
