    ...
    def test_externaluri_to_concept(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            exturi = cnx.create_entity('ExternalUri', cwuri=u'http://someuri/someobject',
                                       ...
                                       reverse_vocabulary_source=[place, function, info])
            cnx.commit()
            ...
            self.assertEqual(place.vocabulary_source[0].eid, concept.eid)
            function.cw_clear_all_caches()
            self.assertEqual(function.vocabulary_source[0].eid, concept.eid)
            info.cw_clear_all_caches()
            self.assertEqual(info.vocabulary_source[0].eid, concept.eid)
...
class EntityLifeCycleTC(CubicWebTC):
    ...
    def test_agent_create_update(self):
        with self.new_access(u'bob').repo_cnx() as cnx:
            ...
            yield self._check_create, cnx, agent.eid, 'created agent'
            agent.cw_set(isni=u'123', name=u'Adam')
            cnx.commit()
            self.set_description('agent update')
            yield self._check_modification, cnx, agent.eid, 'modified isni, name'
    ...
    def test_concept(self):
        with self.new_access(u'bob').repo_cnx() as cnx:
            ...
            yield self._check_create, cnx, scheme.eid, 'created conceptscheme'
            concept = scheme.add_concept(u'hello')
            cnx.commit()
            self.set_description('concept scheme add concept')
            yield self._check_modification, cnx, scheme.eid, 'added in_scheme_object'
            self.set_description('concept creation')
            yield self._check_create, cnx, concept.eid, 'created concept'
            subconcept = concept.add_concept(u'goodbye')
            cnx.commit()
            self.set_description('concept add concept')
            yield self._check_modification, cnx, concept.eid, 'added broader_concept_object'
            self.set_description('subconcept creation')
            yield self._check_create, cnx, subconcept.eid, 'created concept'
            self.set_description('subconcept creation scheme')
            rset = cnx.execute('Activity X ORDERBY X DESC WHERE X type "modify", X used %(x)s',
                               {'x': scheme.eid})
            yield self.assertEqual, len(rset), 2
            yield self.assertEqual, rset.get_entity(0, 0).description, 'added in_scheme_object'
