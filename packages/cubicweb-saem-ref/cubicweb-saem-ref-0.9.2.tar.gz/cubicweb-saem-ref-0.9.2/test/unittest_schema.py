    ...
    def test_on_update_set_end_date_before_start_date(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            with self.assertRaises(ValidationError) as cm:
                ...
                cnx.commit()
            self.assertIn("must be greater than", str(cm.exception))
...
class AgentTC(CubicWebTC):
    ...
    def test_fti(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            place = cnx.create_entity('AgentPlace', name=u'place',
                                      place_address=address, place_agent=agent)
            function = cnx.create_entity('AgentFunction', name=u'function',
                                         function_agent=agent)
            info = cnx.create_entity('LegalStatus', term=u'legal status', legal_status_agent=agent)
            cnx.commit()
            for search in (u'guenievre', u'europe', u'place', u'function', u'legal status'):
                self.set_description(search)
                yield self.assertEqual, cnx.execute('Agent X WHERE X has_text %(search)s',
                                                    {'search': search}).one().eid, agent.eid

    def test_graph_structure(self):
        graph = PERMISSIONS_GRAPHS['Agent'](self.schema)
        expected = {
            'AgentFunction': {('function_agent', 'subject'): ['Agent']},
            'AgentPlace': {('place_agent', 'subject'): ['Agent']},
            'EACResourceRelation': {('resource_relation_agent', 'subject'): ['Agent']},
            'EACSource': {('source_agent', 'subject'): ['Agent']},
            'History': {('history_agent', 'subject'): ['Agent']},
            'LegalStatus': {('legal_status_agent', 'subject'): ['Agent']},
            'Mandate': {('mandate_agent', 'subject'): ['Agent']},
            'PhoneNumber': {('phone_number', 'object'): ['Agent']},
            'PostalAddress': {('place_address', 'object'): ['AgentPlace']},
            'Structure': {('structure_agent', 'subject'): ['Agent']},
        }
        self.assertEqual(graph.parent_structure('Agent'),
                         expected)
    ...
    def test_security_base(self):
        ...
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.commit()
        with self.new_access('toto').client_cnx() as cnx:
            self.assertUnauthorized(cnx, 'SET X name "bobby" WHERE X is Agent, X name "bob"')
            self.assertUnauthorized(cnx, 'SET X agent_kind K WHERE X name "bob", K name "authority"')
            self.assertUnauthorized(cnx, 'SET F name "director" WHERE X name "bob", F function_agent X')
...
class SEDAProfileTC(CubicWebTC):
        ...
        with self.admin_access.client_cnx() as cnx:
            ...
            for search in ('name', 'annotation', 'description'):
                ...
                rset = cnx.execute('Any X WHERE X has_text %(search)s',
                                   {'search': search[::-1]})
                yield self.assertEqual, [r for r, in rset.rows], [profile.eid]
    ...
    def _test_relative_permissions_through(self, permission):
        ...
        expected = [
            'NOT EXISTS(X seda_parent A), U in_group G, G name IN ("managers", "users")',
            'X seda_parent A, U has_update_permission A',
        ]
    ...
    def test_dates_constraints(self):
        with self.admin_access.client_cnx() as cnx:
            ...
            with self.assertRaises(ValidationError) as cm:
                ...
                cnx.commit()
            cnx.rollback()
            yield assert_error, cm, datetype
    ...
    def test_scheme_value_constraint(self):
        with self.admin_access.client_cnx() as cnx:
            ...
            with self.assertRaises(ValidationError) as cm:
                ...
                cnx.commit()
            self.assertEqual(cm.exception.errors,
                             {'seda_keyword_value-subject': u"concept doesn't belong to the scheme associated to the keyword"})
            cnx.rollback()
            kw = cnx.create_entity('SEDAKeyword', seda_keyword_of=cd)
            kw.cw_set(seda_keyword_value=concept,
...
class WorkflowPermissionsTC(CubicWebTC):
    ...
    def test_conceptscheme_permissions(self):
        with self.admin_access.repo_cnx() as cnx:
            ...
            cnx.commit()
            # published, no modification allowed.
            scheme.cw_adapt_to('IWorkflowable').fire_transition('publish')
            cnx.commit()
            with self.assertRaises(Unauthorized):
                scheme.add_concept(u'plop')
                cnx.commit()
            cnx.rollback()
            with self.assertRaises(Unauthorized):
                cnx.create_entity('Label', label=u'arhg', label_of=concept)
                cnx.commit()
            cnx.rollback()
