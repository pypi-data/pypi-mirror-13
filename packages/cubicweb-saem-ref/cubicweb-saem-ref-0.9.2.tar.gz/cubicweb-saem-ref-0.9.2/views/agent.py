...
from cubicweb.web.views import calendar, tabs, uicfg, ajaxcontroller

from cubes.relationwidget import views as rwdg

from cubes.saem_ref.views import (JQueryIncompleteDatePicker, EditableRsetTableColRenderer,
                                  ImportEntityComponent,
                                  add_etype_link, dropdown_button, editlinks, external_link,
...
class TextInputCheckSimilar(fw.TextInput):
    ...
    def _render(self, form, field, render):
        ...
        req.add_css('cubes.saem_ref.css')
...
class EACImportComponent(ImportEntityComponent):
    ...
    __select__ = ImportEntityComponent.__select__ & is_instance('Agent')
...
afs.tag_subject_of(('*', 'vocabulary_source', '*'),
                   formtype=('main', 'muledit'),
                   section='attributes')
affk.tag_subject_of(('*', 'vocabulary_source', '*'),
                    {'widget': rwdg.RelationFacetWidget(dialog_options={'width': 800})})

# autocomplete form for agent name
affk.set_field_kwargs('Agent', 'name', widget=TextInputCheckSimilar({'size': 80},
                                                                    data_initfunc=get_agents_names))
...
def add_relations_button(req, entity, msg, *rtype_roles, **extraurlparams):
    ...
    if links:
        ...
        return dropdown_button(req, req._(msg), *links)
...
class AgentQualifiedRelationComponent(component.EntityCtxComponent):
    """Abstract component for a qualified relation, to be displayed in the context of an Agent"""
    __select__ = EntityView.__select__ & is_instance('Agent') & partial_has_related_entities()
    context = 'navcontentbottom'
    # The relation type the Agent is object of and the qualified relation subject of.
    rtype = None
    role = 'object'
    # RQL to build table rset, assuming last columns if the qualified relation entity.
    rql = None

    @property
    def title(self):
        return self._cw.__(self.rtype + '_object')

    def render_body(self, w):
        rset = self._cw.execute(self.rql + ', X eid %(x)s', {'x': self.entity.eid})
        if rset:
            editurlparams = {'__redirectpath': self.entity.rest_path()}
            colrenderers = {len(rset[0]) - 1: EditableRsetTableColRenderer(editurlparams)}
            self._cw.view('table', rset, w=w, column_renderers=colrenderers)
...
class AsConceptEntityView(EntityView):
    ...
    def entity_call(self, entity, concept_rtype, text_attr, details_attr=None,
        ...
        concepts = entity.related(concept_rtype, entities=True)
        concept = concepts[0] if concepts else None
        scheme_title, scheme_uri = None, None
...
class AgentPlaceAsConceptView(EntityView):
    ...
    def entity_call(self, entity, **kwargs):
        ...
        entity.view('saem.entity_as_concept', w=self.w,
                    concept_rtype='vocabulary_source', text_attr='name',
                    **kwargs)
...
class AgentFunctionView(AgentRelatedEntitiesListView):
    ...
    subvid_kwargs = {'concept_rtype': 'vocabulary_source', 'text_attr': 'name',
...
class AgentMandateView(AgentRelatedEntitiesListView):
    ...
    rtype = 'mandate_agent'
    subvid = 'saem.entity_as_concept'
    subvid_kwargs = {'concept_rtype': 'vocabulary_source', 'text_attr': 'term',
                     'details_attr': 'description'}
...
class DepositAgentConceptsProfilesTab(AgentTabView):
    ...
    def generate_add_button(self, entity):
        ...
        if links:
            ...
            self.w(dropdown_button(self._cw, self._cw._('add'), *links))
            self.w(tags.div(klass='clearfix'))
...
pvds.tag_subject_of(('EACResourceRelation', 'resource_relation_resource',
                    'ExternalURI'), {'vid': 'saem.external_link'})
