...
from cubicweb.web.views import (idownloadable, uicfg, baseviews, tableview, primary,
                                ajaxcontroller, formrenderers, actions, editforms)

from cubes.relationwidget import views as rwdg

from cubes.compound.views import CloneAction
from cubes.saem_ref.views import (add_etype_link, configure_relation_widget,
                                  jqtree)
from cubes.saem_ref.views.startup import index_dropdown_button
...
class SEDAMetaFieldMixIn(object):
    ...
    def cardinality_field(self, form):
    ...
    def annotation_field(self, form):
    ...
    def render(self, form, renderer):
        ...
        form._cw.add_css('cubes.saem_ref.css')
    ...
    def _render_subfield(self, form, field, renderer):
...
# related Agent
pvs.tag_subject_of(('*', 'seda_transferring_agent', 'Agent'), 'hidden')
...
class AppraisalRuleValueView(EntityView):
    ...
    def entity_call(self, entity):
        _ = self._cw._
        if entity.seda_appraisal_rule_duration:
            duration = entity.seda_appraisal_rule_duration[0].value or _('<no value>')
        else:
            duration = _('<no value>')
        self.w('<div><span>%s</span> %s</div>' % (_('seda_appraisal_rule_duration'),
                                                  xml_escape(duration)))
        if (entity.seda_appraisal_rule_code and
                entity.seda_appraisal_rule_code[0].seda_appraisal_rule_code_value):
            rule = entity.seda_appraisal_rule_code[0]
...
afs.tag_subject_of(('SEDAAppraisalRuleCode', 'seda_appraisal_rule_code_value', '*'),
                   'main', 'attributes')
affk.set_field_kwargs('SEDAAppraisalRuleDuration', 'value', widget=fw.TextInput({'size': 80}))
...
class SEDACompoundEntityInlinedFormRenderer(formrenderers.EntityInlinedFormRenderer):
    ...
    def open_form(self, w, form, values):
        """Overriden from :class:`formrenderers.EntityInlinedFormRenderer` to add the 'formGroup' class to
        then enclosing div. Beside this, HTML structure is left unchanged and so opened div are
        closed in :meth:`formrenderers.EntityInlinedFormRenderer.close_form`
        """
        form._cw.add_css('cubes.saem_ref.css')
        try:
...
# custom widget to search concept related to dynamic scheme (for keywords) #########################

class ConceptAutoCompleteWidget(fw.TextInput):
    """Derive from simple text input to create an autocompletion widget"""
    def _render(self, form, field, render):
        slave_id = field.dom_id(form, self.suffix)
        master_id = slave_id.replace('keyword_value', 'keyword_scheme')
        if form.edited_entity.has_eid():
            scheme = form.edited_entity.scheme.eid
            concept = form.edited_entity.concept
        else:
            scheme = None
            concept = None
        req = form._cw
        req.add_js(('cubicweb.js', 'cubicweb.ajax.js', 'cubes.saem_ref.js'))
        req.add_onload(uilib.js.saem.initSchemeConceptFormField(master_id, slave_id, scheme))
        if concept is None:
            value = ''
            eid = fw.INTERNAL_FIELD_VALUE
        else:
            value = concept.label()
            eid = concept.eid
        ...
        inputs = [
            tags.input(name=field.input_name(form, 'Label'), id=slave_id + 'Label',
                       ...
                       klass='form-control', type='text', value=value),
            tags.input(name=field.input_name(form), id=slave_id,
                       type='hidden', value=unicode(eid))
        ]
        return u'\n'.join(inputs)

affk.set_field_kwargs('SEDAKeyword', 'seda_keyword_value', widget=ConceptAutoCompleteWidget())
...
class SEDAPrimaryView(primary.PrimaryView):
    ...
    def entity_call(self, entity, **kwargs):
        ...
        self.w(tags.div(id=_import_div_id(entity), style='display: none'))
...
class SEDAImportAction(action.Action):
    ...
    etype = title = None  # to be specified in concrete classes
...
class SEDAImportProfileArchiveObjectAction(SEDAImportAction):
    ...
    etype = title = 'ProfileArchiveObject'
...
class SEDADoImportView(controller.Controller):
    ...
    def publish(self, rset=None):
        ...
        cloned = self._cw.entity_from_eid(int(self._cw.form['cloned']))
        self._cw.create_entity(cloned.cw_etype, seda_parent=entity, seda_clone_of=cloned)
        msg = self._cw._('%(etype)s #%(eid)s has been imported') % {
            'etype': self._cw._(cloned.cw_etype),
            'eid': cloned.eid}
...
class SelectProfileArchiveObjectEntitiesTableView(rwdg.SelectEntitiesTableView):
    ...
    def archive_object_description_level(archiveobject):
        desc = archiveobject.content_description
        if desc:
            return desc.seda_description_level[0]
        return None
...
class AddEntityComponent(component.CtxComponent):
    ...
    def render_body(self, w):
        ...
        w(index_dropdown_button(req, req._('Add a new component'), links))
...
class SEDALibView(baseviews.SameETypeListView):
    ...
    @property
    def title(self):
        return self._cw._('SEDA components library')
