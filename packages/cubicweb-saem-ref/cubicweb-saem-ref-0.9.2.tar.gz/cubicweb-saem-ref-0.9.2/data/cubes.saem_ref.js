...
saem = {
    ...
    // vocabulary
    initSchemeConceptFormField: function(masterSelectId, dependentSelectId, schemeEid) {
        ...
        masterSelect.change(function() {
            saem.updateSchemeConceptFormField(this, dependentSelectId);
        });
            ...
            .autocomplete(
                {source: function(request, response) {
                    var form = ajaxFuncArgs('keyword_concepts',
                                            {'q': request.term, 'scheme': saem.currentSchemeEid});
        ...
        // add key press handler so that value which isn't matching a vocabulary value will be erased
        cw.jqNode(dependentSelectId+'Label').keypress(function(evt) {
            if (evt.keyCode == $.ui.keyCode.ENTER || evt.keyCode == $.ui.keyCode.TAB) {
                ...
                $labelInput = cw.jqNode(dependentSelectId+'Label');
                if ($.inArray($labelInput.val(), validChoices) == -1) {
                    $labelInput.val('');
        ...
        if (schemeEid == null) {
          schemeEid = masterSelect.val();
        }
        saem.currentSchemeEid = schemeEid;
    },

    updateSchemeConceptFormField: function(masterSelect, dependentSelectId) {
        // save current scheme eid and reset concept select
