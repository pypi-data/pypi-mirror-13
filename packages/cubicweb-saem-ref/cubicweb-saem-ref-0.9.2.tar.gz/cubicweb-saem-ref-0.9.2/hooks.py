...
from cubicweb.hooks.metadata import InitMetaAttrsHook
...
class UpdateModificationDateOp(hook.DataOperationMixIn, hook.Operation):
    ...
    def add_entity(self, entity):
        """Add entity, its parent entities (up to the container root) for update of their modification date
        at commit time.
        """
...
class AssignARKMixIn(object):
    """mix-in class for ARK assignment hooks"""
    __regid__ = 'saem_ref.ark.assign'
    events = ('before_add_entity',)
    order = InitMetaAttrsHook.order - 1


class AssignARKHook(AssignARKMixIn, hook.Hook):
    """When an agent or a SEDA profile is created, assign a local ark identifier to it."""
    __select__ = hook.Hook.__select__ & is_instance('Agent', 'SEDAProfile')

    def __call__(self):
        entity = self.entity
        ark = entity.cw_edited.get('ark')
        if not ark:
            ark = entity.cw_edited['ark'] = entity.cw_adapt_to('IARKGenerator').generate_ark()
        if 'cwuri' not in entity.cw_edited:
            entity.cw_edited['cwuri'] = u'ark:/' + ark


class AssignARKToSKOSHook(AssignARKMixIn, hook.Hook):
    """when some scheme or concept is created, assign a local ark to it"""
    __select__ = hook.Hook.__select__ & is_instance('ConceptScheme', 'Concept')

    def __call__(self):
        set_skos_ark_and_cwuri(self.entity)


def set_skos_ark_and_cwuri(entity):
    ark = entity.cw_edited.get('ark')
    if not ark:
        cwuri = entity.cw_edited.get('cwuri')
        ark = None if cwuri is None else extract_ark(cwuri)
        if ark is None:
            ark = entity.cw_adapt_to('IARKGenerator').generate_ark()
        entity.cw_edited['ark'] = ark
    if 'cwuri' not in entity.cw_edited:
        entity.cw_edited['cwuri'] = u'ark:/' + ark
...
# other parts in schema and hooks

from cubicweb.hooks import metadata, workflow  # noqa


class InitMetaAttrsHook(metadata.MetaDataHook):
...
class UpdateMetaAttrsHook(metadata.MetaDataHook):
...
class SetModificationDateOnStateChange(workflow.WorkflowHook):
    """update entity's modification date after changing its state"""
    __regid__ = 'wfsyncmdate'
    __select__ = workflow.WorkflowHook.__select__ & hook.match_rtype('in_state')
    events = ('after_add_relation',)
...
def registration_callback(vreg):
    vreg.register_all(globals().values(), __name__, (InitMetaAttrsHook, UpdateMetaAttrsHook,
                                                     SetModificationDateOnStateChange))
    vreg.register_and_replace(InitMetaAttrsHook,
                              metadata.InitMetaAttrsHook)
    vreg.register_and_replace(UpdateMetaAttrsHook,
                              metadata.UpdateMetaAttrsHook)
    vreg.register_and_replace(SetModificationDateOnStateChange,
                              workflow.SetModificationDateOnStateChange)
