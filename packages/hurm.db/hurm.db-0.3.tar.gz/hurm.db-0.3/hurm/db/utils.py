...
from . import translatable_string as _
...
def save_changes(sasess, request, modified, deleted, clogger=None):
    ...
    from .tables import persons, password_manager
    ...
    try:
        for key, mdata in modified:
            ...
            table = table_from_primary_key(key)
            is_persons_table = table is persons

            fvalues = {}
            for f, v in mdata.items():
                if f in table.c and f != key:
                    if v != '':
                        if is_persons_table and f == 'password':
                            v = password_manager.encode(v)
                        fvalues[f] = v
                    else:
                        fvalues[f] = None
        ...
        if clogger is not None:
            clogger.warning('Changes rolled back due to an exception')
        logger.error('Error while saving changes: %s', e)
        raise
    except:
        if clogger is not None:
            clogger.warning('Changes rolled back due to an exception')
        logger.exception('Unrecognized error while saving changes')
        raise
