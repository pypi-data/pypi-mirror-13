...
def test_save_changes(session, edition_test):
    try:
        ...
        foobar = session.query(entities.Person).get(i[0]['idperson'])
        assert foobar.firstname == 'Foo'
        assert foobar.lastname == 'Bar'
        assert foobar.password and foobar.password != 'simple'
...
def test_dumpy(edition_test):
    dump = utils.dump(edition_test, [{
        ...
        'other': 'birthdate,mobile,phone,email,role,availabilities'.split(',')
    }, {
