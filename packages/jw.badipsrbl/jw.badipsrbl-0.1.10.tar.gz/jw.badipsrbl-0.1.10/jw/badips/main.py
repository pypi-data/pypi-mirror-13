...
from _socket import SOCK_DGRAM, gethostbyaddr
        ...
        def run(self):
            ...
            while True:
                ...
                for _ in ['once']:  # Pseudo block
                    # Only bad guys request invalid IPs. Report them.
                    if len(sname) != 4:
                        Logger.warning('-> Invalid IP (not 4 elements): "%s". Reporting spammer %s', sname, address[0])
                        try:
                            Logger.info(
                                "Reported %s as 'spam'. Result: %s",
                                address[0],
                                TextIOWrapper(urlopen('https://www.badips.com/add/spam/%s' % address[0])).read()
                            )
                        except:
                    ...
                    try:
                        ...
                        Logger.warning('Invalid IP (non-integers): "%s". Reporting spammer %s', sname, address[0])
                        try:
                            Logger.info(
                                "Reported %s as 'spam'. Result: %s",
                                address[0],
                    ...
                    if all(c in self.mailCategories for c in data['Categories']):
                        try:
                            host = gethostbyaddr(qname)[0]
                        except Exception as e:
                            host = '%s (%s)' % (qname, e)
                        Logger.info('-> Only reported in mail categories: %s', host)
                        break
                    ...
                    try:
                        Logger.info(
                            "Reported %s as 'badbot'. Result: %s",
                            address[0],
                            TextIOWrapper(urlopen('https://www.badips.com/add/spam/%s' % qname)).read()
                        )
                    except:
