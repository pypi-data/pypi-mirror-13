...
class EngineManager(object):
    ...
    config_rules = (
        ...
        Rule("connect_url", required=True, type=str, regex=r"^\w+://"),
        # 编码
        Rule("encoding", required=False, type=str, default="utf-8"),
        # 连接参数，采用json的形式:{'ssl': {'cert': xxx, 'key': xxx, 'ca': xxx}}
        Rule("connect_args", required=False,
             type=str, default="{}", logic=be_json("connect_args")),
        # 该数据源使用的连接池策略
        Rule("pool_policy", required=False, type=str, default=None)
    )

    # 连接池的配置项验证规则，section名称皆以dbpool_开头
    pool_config_rules = (
        # 连接池类型，比如sqlalchemy.pool.QueuePool，可以是字符串，也可以是具体的类对象
        Rule("poolclass", required=True, type=(Pool, str)),
        # 连接池大小
        Rule("pool_size", required=False, type=(int, str),
...
class Query(object):
    ...
    def __call__(self, context):
        ...
        # 处理查询项
        query_items = []
        if isinstance(self._query_items, str):
...
class SQL(object):
    ...
    def __call__(self, context):
        ...
        if isinstance(self._sql, str) and \
                _SELECT_STATEMENT_REGEX.search(self._sql):
            self._execute_select_statement(session, context)
        else:
            # 非查询语句
            try:
