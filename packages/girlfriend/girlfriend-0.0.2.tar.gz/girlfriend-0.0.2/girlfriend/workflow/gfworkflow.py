...
class Context(AbstractContext):
    ...
    def __init__(self, config, args, plugin_mgr, parent=None):
        super(Context, self).__init__()
        self.delegate = {}
        if parent:
            self.delegate.update(parent.delegate)
        self._config = config
...
class Job(AbstractJob):
    ...
    def execute(self, context):
        ...
        # 获取运行时参数
        args = self._get_runtime_args(context)
    ...
    def _get_runtime_args(self, context):
        """获取运行时参数
        ...
        if not context_args:
        ...
        elif isinstance(self._args, SequenceCollectionType):
            ...
            if context_args:
                args_schema = context_args
        elif isinstance(self._args, types.DictType):
...
class WorkflowUnitExistedException(GirlFriendBizException):
    ...
    def __init__(self, unit_name):
        return u"工作单元 '{}' 已经存在".format(unit_name)
