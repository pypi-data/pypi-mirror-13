...
class Scheduler(object):
    ...
    def claim_calls(self):
        ...
        logger.debug("Looking for claimable calls between %s-%s", start_block, end_block)
        ...
        for call_address in upcoming_calls:
            if call_address in self.active_claims:
                logger.debug("Call %s already claimed", call_address)
                continue
            ...
            if not scheduled_call.is_claimable:
                logger.debug("Call %s not claimable", call_address)
                continue
    ...
    def schedule_calls(self):
        ...
        logger.debug("Looking for calls between %s-%s", start_block, end_block)
        upcoming_calls = self.enumerate_calls(start_block, end_block)

        for call_address in upcoming_calls:
            logger.debug("Evaluating %s for scheduling", call_address)
            if call_address in self.active_calls:
                logger.debug("Call %s already scheduled", call_address)
                continue
            ...
            if not scheduled_call.is_callable:
                logger.debug("Call %s not callable", call_address)
                continue
