...
func = netprop.func
def procfunc(x, net, inp, trg, s, e):
    """
    Per process function netprop.func function
    """
    return func(x, net.conec, net.bconecno, net.units, net.inno, net.outno, inp[s:e], trg[s:e])
def mpfunc(x, pool, net, inp, trg, splitters):
    ...
    for splitter in splitters:
        s, e = splitter
        res += [pool.apply_async(procfunc, (x, net, inp, trg, s, e))]
    return sum([r.get() for r in res])

grad = netprop.grad
def procgrad(x, net, inp, trg, s, e):
    """
    Per process function netprop.grad function
    """
    return grad(x, net.conec, net.bconecno, net.units, net.inno, net.outno, inp[s:e], trg[s:e])
def mpgrad(x, pool, net, inp, trg, splitters):
    ...
    for splitter in splitters:
        s, e = splitter
        res += [pool.apply_async(procgrad, (x, net, inp, trg, s, e))]
    return sum([r.get() for r in res])
