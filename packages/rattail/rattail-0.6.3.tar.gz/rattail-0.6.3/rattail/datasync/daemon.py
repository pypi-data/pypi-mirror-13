...
def watch_for_changes(config, watcher):
    ...
    while not error:
        ...
        try:
            ...
            log.exception("error calling watcher.get_changes(), this watcher will now *terminate*")
            error = True
        else:
...
def consume_changes(profile, consumers):
    ...
    def process(session, consumer, changes):
        try:
            ...
            log.exception("error calling consumer.process_changes(), this consumer will now *terminate*")
            return False
