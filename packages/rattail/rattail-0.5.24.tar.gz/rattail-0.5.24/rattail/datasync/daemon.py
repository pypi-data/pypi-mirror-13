...
def watch_for_changes(config, watcher):
    ...
    while True:
        ...
        try:
            ...
            log.exception("error calling watcher.get_changes()")
        else:
...
def consume_changes(profile, consumers):
    ...
    def process(session, consumer, changes):
        try:
            ...
            log.exception("error calling consumer.process_changes()")
            return False
            ...
            if changes:
                ...
                for change in changes:
                    if timestamp and change.obtained != timestamp:
                        if not process(session, consumer, batch):
                            error = True
                            break
                        batch = []
                        batchcount += 1
                ...
                if not error:
                    error = not process(session, consumer, batch)

                log.debug("processed changes in {0} batches{1}".format(
                    batchcount, " (with errors)" if error else ""))
