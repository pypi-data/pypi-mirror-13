...
from sign import get_ripple_from_secret, root_key_from_seed, parse_seed, \
    ecdsa_sign, ecc_point_to_bytes_compressed, get_ripple_from_pubkey
...
class RestClient:
  ...
  def get_trustlines(self, address, currency=None, counterparty=None, limit=None, parameters=None, **kwargs):
    ...
    response = self._request(url, d_parameters, kwargs, method="GET")
    print "rrrrrr",response, d_parameters
    del d_parameters
  ...
  def order_transaction_history(self, address, parameters=None):
  ...
  def add_relations(self, address, secret, relations_type, counterparty, 
    limit_currency=None, limit_issuer=None, limit_value=None, is_sync=False):
    if is_sync:
