...
class RedisMixin(AptMixin):
    ...
    redis_tcp_keepalive_pattern = '#?\s*tcp-keepalive \w+'
    ...
    @uses_fabric
    def redis_configure(self, bind=None, loglevel=None, tcp_keepalive=None):
        if bind is None:
            bind = self.redis_bind
        if loglevel is None:
            loglevel = self.redis_loglevel
        if tcp_keepalive is None:
            tcp_keepalive = self.redis_tcp_keepalive
        ...
        files.sed(self.redis_conf, self.redis_loglevel_pattern, loglevel,
                  use_sudo=True)
        files.sed(self.redis_conf, self.redis_tcp_keepalive_pattern, tcp_keepalive,
                  use_sudo=True)
