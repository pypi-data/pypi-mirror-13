...
class ConanMultiPackager(object):
    ...
    def add_common_builds(self, package_name=None, shared_option_name=None, visual_versions=None):
        ...
        if platform.system() == "Windows":
            for visual_version in visual_versions:
                for arch in ["x86", "x86_64"]:
                    self.add_visual_builds(visual_version, arch, package_name, shared_option_name)
        else:
            self.add_other_builds(package_name, shared_option_name)

    def add_visual_builds(self, visual_version, arch, package_name, shared_option_name):
        ...
        shared_name = "%s:%s" % (package_name, shared_option_name)
        if package_name and shared_option_name:
            sets.append([{"build_type": "Release", "compiler.runtime": "MT"}, {shared_name: False}])
            sets.append([{"build_type": "Debug", "compiler.runtime": "MTd"}, {shared_name: False}])
            sets.append([{"build_type": "Debug", "compiler.runtime": "MDd"}, {shared_name: False}])
            sets.append([{"build_type": "Release", "compiler.runtime": "MD"}, {shared_name: False}])
            sets.append([{"build_type": "Debug", "compiler.runtime": "MDd"}, {shared_name: True}])
            sets.append([{"build_type": "Release", "compiler.runtime": "MD"}, {shared_name: True}])
        else:
    ...
    def add_other_builds(self):
        # Not specified compiler or compiler version, will use the auto detected
        for arch in ["x86", "x86_64"]:
            for shared in [True, False]:
                for build_type in ["Debug", "Release"]:
                    self.add({"arch":arch, "build_type": build_type}, {"zlib:shared": shared})
