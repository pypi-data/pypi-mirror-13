...
class ConanMultiPackager(object):
    ...
    def add_commonbuilds(self, package_name=None, shared_option_name=None, visual_versions=None):

        visual_versions = visual_versions or [10, 12, 14]
        if shared_option_name and package_name:
            shared_option_name = "%s:%s" % (package_name, shared_option_name)
        else:
        ...
        if platform.system() == "Windows":
            for visual_version in visual_versions:
                for arch in ["x86", "x86_64"]:
                    self.add_visualbuilds(visual_version, arch, shared_option_name)
        else:
            self.add_otherbuilds(shared_option_name)

    def add_visualbuilds(self, visual_version, arch, shared_option_name):
    ...
    def add_otherbuilds(self, shared_option_name):
