...
class I2CDevice(object):
    ...
    def i2c_read(self, length):
        ...
        logging.info('{} -> PC: {}'.format(self.address, repr(response)))
        return response

    def i2c_write(self, bytes):
        logging.info('PC -> {}: {}'.format(self.address, repr(bytes)))
        return self.i2c_bus.i2c_write(self.address, bytes)

    def i2c_read_register(self, register, length):
        logging.info('0x{:02X} -> PC: reg 0x{:02X}:'.format(self.address, register))
    ...
    def i2c_write_register(self, register, bytes):
        logging.info('PC -> 0x{:02X} reg 0x{:02X}: {}'.format(self.address, register, repr(bytes)))
