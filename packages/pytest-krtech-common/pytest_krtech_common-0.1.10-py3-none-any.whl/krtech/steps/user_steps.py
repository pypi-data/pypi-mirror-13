...
from hamcrest import assert_that, equal_to, is_, not_none, none, contains_string, equal_to_ignoring_case, has_item, not_
...
class UserSteps(object):
    ...
    def should_see_text_in_list(self, list_, text):
        l = list(map(lambda x: x.text, list_.elements))
        assert_that(l, has_item(text), u'Текст отсутствует в списке')

    @allure.step("'{1}' не должен содержать текст '{2}'")
    def should_not_see_text_in_list(self, list_, text):
        l = list(map(lambda x: x.text, list_.elements))
        assert_that(l, not (has_item(text)), u'Текст присутствует в списке')
    ...
    def waits_for_element_enabled(self, element):
        try:
            WebDriverWait(self.driver, self.element_wait).until(
    ...
    def set_calendar_time(self, calendar, time):
        ...
        js_time = js_time_top \
                  + "$('.xdsoft_time_box.xdsoft_scroller_box').scrollTop($('[data-hour=%s]').position().top);"
        ...
        return len(calendar.time_list) - 1
