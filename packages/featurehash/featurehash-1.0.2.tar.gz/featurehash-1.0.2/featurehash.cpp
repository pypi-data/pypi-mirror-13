...
//-----------------------------------------------------------------------------
//
// @adopted by : Hua Jiang <jianghuachinacom@163.com>,Bingqing Qu <qubingqing@emar.com>
//
// @Version : 0.0.1 (alpha)
 ...
 */
uint32_t feature_hashing(const char* c_feature,unsigned int len,double& sign,uint32_t upper_bound_=8388608,uint32_t min=1,uint32_t seed_=0)
{
	...
	if (*p) {
		...
		return ABS(hashed_value) % upper_bound_+min;
	}
	else {
		sign = converted <0 ? -1.0 : 1.0;
		return ABS(converted) % upper_bound_ + min;
	}
}
...
static PyObject * transform_features(PyObject *self,PyObject *args) {
	...
	uint32_t seed=0;
	uint32_t min=1;
	if(!PyArg_ParseTuple(args,"O|iii",&arg,&upper_bound,&min,&seed)) {
		return NULL;
	}
	if(PySequence_Check(arg)){
		uint32_t index = 0;
		...
		for(int i=0;i<size;i++){
			...
			index = feature_hashing(feature.c_str(),feature.length(),sign,upper_bound,min,seed);
			values[index] += sign;
		}
		std::multimap<uint32_t,double>::iterator iter = values.begin();
		pList= PyList_New(0);
...
static PyObject * transform_feature(PyObject *self,PyObject *args) {
	...
	uint32_t min=1;
	unsigned int len;
	if(!PyArg_ParseTuple(args,"s#|iii",&c_feature,&len,&upper_bound,&min,&seed)) {
		return NULL;
	}
	uint32_t h =  feature_hashing(c_feature,len,sign,upper_bound,min,seed);
