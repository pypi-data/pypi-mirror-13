# -*- coding: utf-8 -*-
import collections
from .packets import match_packet
...
class TCPConnection(object):
    ...
    def get_connection(address):
        ...
        try:
            return TCPConnection.__connections[address]
        except:
            return None
    ...
    def remove_connection(address):
        ...
        try:
            return TCPConnection.__connections.pop(address)
        except:
            return None
    ...
    def send_next(self):
        ...
        try:
            res = [self._socket.send(data) for data in
                   [self._queue.popleft() for count in xrange(0, 1)]]
            if 0 in res:
    ...
    def read_next(self):
        ...
        data.pop(len(data) - 1)  # remove data left over from splitting __eof
        for raw in data:
            packet = match_packet(raw)
            if packet:
                self.connection_recv_packet(packet)
            else:
                self.connection_recv_data(raw)
