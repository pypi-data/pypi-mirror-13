...
class TranslationHelper:
    ...
    def __init__(self, config):
        ...
        print("XXX Locale debugging XXX")
        from pprint import pformat
        currentLocale, encoding = locale.getdefaultlocale()
        print("Current locale and encoding:", currentLocale, encoding)
        print("MESSAGES and LOCALE_DIR:", repr(MESSAGES), repr(LOCALE_DIR))
        print("Contents of LOCALE_DIR:\n{}\n".format(
            pformat(os.listdir(LOCALE_DIR))))
        # print("Contents of LOCALE_DIR/<locale>/LC_MESSAGES:",
        #       os.listdir(os.path.join(LOCALE_DIR, currentLocale,
        #                               "LC_MESSAGES")))
        print("gettext.find(MESSAGES, LOCALE_DIR):",
              repr(gettext.find(MESSAGES, LOCALE_DIR)))
        try:
            langInfo = pformat(gettext.translation(MESSAGES, LOCALE_DIR).info())
        except OSError:
            langInfo = "not found"
        print("gettext.translation(MESSAGES, LOCALE_DIR).info():\n{}\n".format(
                langInfo))

        print("XXX End of locale debugging XXX")
