...
class Version(object):
    ...
    def __init__(self, ver):
        ...
        if isinstance(ver, list):
            ...
            match = self._matchRe.match(text_type(ver))
            if match:
                vtuple = self._parseRe.finditer(match.group(0))
                vtuple = tuple(v.group(0) for v in vtuple)
            else:
                raise VersionError('Invalid version: "%s"' % text_type(ver))
    ...
    def __str__(self):
        ...
        """
        return ''.join(
            self._levelValues[l] if l < 0 else '.%d' % l if p >= 0 else text_type(l)
        ...
        if not args.list:
            ...
            if args.synchronize:
                ...
                if args.dry_run or args.verbose:
                    print('%s: %s --> %s' % (filename, content[m.start():m.end()], text_type(args.synchronizedVersion)))
                if not args.dry_run:
                    newContent[m.start():m.end()] = binary_type(args.synchronizedVersion)
            else:
                if args.dry_run or args.verbose:
                    print('%s: %s --> %s' % (filename, content[m.start():m.end()], text_type(v)))
                if not args.dry_run:
                    newContent[m.start():m.end()] = str(v).encode('utf-8')
        if args.list or args.hg_tag is not False:
            vlist.append(v)
    # Write back changes
    outFilename = _TEMP_FILE_TEMPLATE % filename if args.all_or_nothing else filename
    open(outFilename, 'w').write(newContent.decode('utf-8'))
    result = 1
