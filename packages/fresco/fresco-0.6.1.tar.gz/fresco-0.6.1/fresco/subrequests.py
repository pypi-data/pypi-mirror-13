...
def subrequest(view, *args, **kwargs):
    ...
    This has slightly different behaviour depending on the value of ``view``:

    - **If view is a string and contains a / symbol**,
      it is assumed to be a path, and a full request context is
      constructed to emulate a request to the given path.
      This will cause all middleware layers and application hooks
      to be invoked for the subrequest.

    - **If view is a callable**, it will be called directly with
      ``*args, **kwargs``.
      Middleware and hooks will not be called, and any
      :class:`~fresco.routeargs.RouteArg`\s  defined will not be resolved.

    - **Otherwise** the view function is looked up using the same
