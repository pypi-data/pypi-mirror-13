...
OUTPUT: A log file called Iesha.log, unless the user specifies another name during
        the initialization. Then, from the Iesha.log file the system automatically generates
...
class pyClientChatbot():
    ...
    def __init__(self, args, config_file=None):
        ...
        self.client_info = {
            "id": 'Y1MtvRG6izW9qpWK',  # Identifier to access the server
            "max_connections": 1,  # Integer. Indicates how many simultaneous users can handle the chatbot
                             # (WARNING: No implemented yet)
            "image": 'Iesha.png',  # Image to show to the users
            "name": 'Iesha',  # Official name of the chatbot. Must be different
            ...
            "description": 'A teenager girl who loves anime',  # A short description of the chatbot
            "log_file": 'Iesha.log',
            "human": False,
        }
    ...
    def init_logger(self, log_file='Iesha.log',
...
def main(args):
    ...
    parser.add_argument('-n', '--name', help='chatbot name', default='Iesha')
    parser.add_argument('-l', '--log', help='log filename', default='Iesha.log')
    parser.add_argument('-i', '--id', help='chatbot id', default='Y1MtvRG6izW9qpWK')
    parser.add_argument('-c', '--config', help='Configuration file', default='iesha.cfg')
