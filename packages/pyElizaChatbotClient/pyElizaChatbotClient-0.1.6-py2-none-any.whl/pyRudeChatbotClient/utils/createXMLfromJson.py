...
"""
DESCRIPTION: This module is part of the pyChatbotClient package released in the
...
USAGE: This program can be run from the command line by using:
             python createXMLFromJson.py -log Rude.log -user_name John -chatbot_name RudeMan
...
def fntCreateXMLFromLogs(chatbot_name='RudeMan', user_name='John', logfile_name='Rude.log'):

    if logfile_name is None: logfile_name = 'Rude.log'
    if chatbot_name is None: chatbot_name = 'RudeMan'
    ...
    try:
        ...
        for line_log, turn in enumerate(selected_info_logs[1:]):  # Skip reading the header
            ...
            if 'INIT_CHAT' in turn_dict['type_msg']:
                ...
                if len(buffer_turns) > 0 and dialog_xml is not None:
                    for turn_xml in buffer_turns:
                        dialog_xml.append(turn_xml)
                    buffer_turns = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('-log', '--log', default='Rude.log', help='Specify the log '
                                                                     'file to convert. Default=Rude.log')
    parser.add_argument('-chatbot', '--chatbot', default='RudeMan', help='Specify the name of the chatbot. Default=RudeMan')
