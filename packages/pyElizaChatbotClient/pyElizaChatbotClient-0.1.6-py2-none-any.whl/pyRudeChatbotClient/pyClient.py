...
OUTPUT: A log file called Rude.log, unless the user specifies another name during
        the initialization. Then, from the Rude.log file the system automatically generates
...
class pyClientChatbot():
    ...
    def __init__(self, args, config_file=None):
        ...
        self.client_info = {
            "id": '6xtq0enM0Xt5TbJu',  # Identifier to access the server
            "max_connections": 1,  # Integer. Indicates how many simultaneous users can handle the chatbot
                             # (WARNING: No implemented yet)
            "image": 'Rude.gif',  # Image to show to the users
            "name": 'RudeMan',  # Official name of the chatbot. Must be different
            ...
            "description": 'A rude and angry man',  # A short description of the chatbot
            "log_file": 'Rude.log',
            "human": False,
        }
    ...
    def init_logger(self, log_file='Rude.log',
...
def main(args):
    ...
    parser.add_argument('-n', '--name', help='chatbot name', default='RudeMan')
    parser.add_argument('-l', '--log', help='log filename', default='Rude.log')
    parser.add_argument('-i', '--id', help='chatbot id', default='6xtq0enM0Xt5TbJu')
    parser.add_argument('-c', '--config', help='Configuration file', default='rude.cfg')
