...
if __name__ == "__main__":
    x = [0, 0, 0]
    assert gauss_jordan([[3, 2, -1], [2, -2, 4], [-1, .5, -1]],
                        x, [1, -2, 0]) == GJ_UNE_SOLUTION
    x = [0, 0]
    assert gauss_jordan([[3, 2], [6, 4]],
                        x, [6, 12]) == GJ_PLUSIEURS_SOLUTIONS
    assert gauss_jordan([[1, -2], [3, 5], [4, 3]],
                        x, [-1, 8, 7]) == GJ_UNE_SOLUTION
    assert gauss_jordan([[3, 2], [3, 2]], x, [6, 12]) == GJ_ZERO_SOLUTIONS
    assert gauss_jordan([[1, 1], [2, 1], [3, 2]],
