...
if __name__ == "__main__":
    # rabin_karp_matching is tested in knuth_morris_pratt.py
    assert rabin_karp_factor("h", "h", 1) == (0, 0)
    assert rabin_karp_factor("h", "h", 2) == None
    assert rabin_karp_factor("ab", "ab", 2) == (0, 0)
    assert rabin_karp_factor("ab", "ba", 2) == None
    assert rabin_karp_factor("ab", "ba", 1) == (0, 1)
    M = 300000
    s = "a" * M + "b" * M + "a" * M
    t = "c" * M + "b" * M + "c" * M
    assert (rabin_karp_factor(s, t, M)) == (M, M)
