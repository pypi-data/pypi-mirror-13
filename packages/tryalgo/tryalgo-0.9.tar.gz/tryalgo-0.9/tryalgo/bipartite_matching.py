...
if __name__ == "__main__":
    # works also with a dictionary if keys are of the form range(n)
    assert [None, None] == max_bipartite_matching({0: [], 1: []})
    assert [0, None] == max_bipartite_matching({0: [0], 1: [0]})
    assert [0, 1] == max_bipartite_matching({0: [0], 1: [0, 1]})
    assert [0, 1, 2, 4, None] == max_bipartite_matching(
               {0: [0], 1: [0, 1], 2: [2, 3], 3: [1], 4: [0, 3]})
