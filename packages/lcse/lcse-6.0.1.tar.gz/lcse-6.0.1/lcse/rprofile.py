...
#      self.header_attrs['has_normals'] = header[7]
  ...
  def _load(self, f):
    ...
    if self.header_attrs.get('has_centers'):
      self._centers = np.fromfile(f, dtype=np.float64, count=5 * nbuckets).reshape((5, -1), order='F')
  ...
  def get_centers(self):
    """ Get centers of the buckets as an array of phi, theta, x, y, z """

    if self._centers is None and self.version >= 11:
      self._centers = self._get_array('centers')
      centers = self._centers[2:]
      self._centers[2:] = -centers / np.sqrt((centers * centers).sum(0))
  ...
  def get_corners(self):
    ...
    if self._corners is None and self.version >= 11:
      self._corners = self._get_array('corners')
