...
def table(dates, amounts, spacing=-16):
    ...
    >>> table(['2015-12-12', '2015-12-13'], [20, 40])
    ------------------
    2015-12-12      20
    2015-12-13      40
    ------------------
    Total: 60 over 2 days
    ...
    if len(amounts) > 1:
        print "Total: {0} over {1} days".format(sum(amounts), len(amounts))
...
def graph_action(args):
    ...
    if results is None:
        ...
        if len(results) > 1:
            total = (int(sum(results)) if sum(results) % 1 == 0 else sum(results))
            print "Total: {0} over {1} days".format(total, len(results))
