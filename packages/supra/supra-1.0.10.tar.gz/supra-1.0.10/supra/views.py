...
class SupraFormView(FormView):
	...
	def get_form_kwargs(self):
		...
		if self.request.method in ('GET',):
			...
			#end if
		elif (self.body or SupraConf.body) and self.request.method in ('POST', 'PUT'):
			if 'pk' in kwargs['data']:
				self.initial_pk = self.http_kwargs['pk']
			#end if
		...
		#end def
		if self.initial_pk:
			kwargs['instance'] = self.model.objects.filter(pk=self.initial_pk).first()
