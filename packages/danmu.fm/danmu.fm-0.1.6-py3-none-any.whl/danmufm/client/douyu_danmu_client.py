...
from ..misc.exception import TooManyNoMessageError
...
class DouyuDanmuClient(object):
    ...
    def do_login(self):
        ...
        logger.info("第1个消息发送开始")
        recv_msg = self.auth_recv()
        logger.info(str(recv_msg))
        # print(recv_msg)
        logger.info("第1个消息发送结束")
        if "live_stat@=0" in recv_msg:
            ...
            logger.info("第2个消息发送开始")
            self.send_qrl_msg()
            recv_msg = self.auth_recv()
            # print(recv_msg)
            logger.info("第2个消息发送结束")
            recv_msg = self.auth_recv()
            # print(recv_msg)
            logger.info("第3个消息发送开始")
            self.send_auth_keeplive_msg()
            recv_msg = self.auth_recv()
            logger.info("第3个消息发送结束")
            # print(recv_msg)
            logger.info("第4个消息发送结束")
            self.send_danmu_loginreq_msg()
            recv_msg = self.danmu_recv()
            logger.info("第4个消息发送结束")
            # print(recv_msg)
            logger.info("第5个消息发送结束")
    ...
    def get_danmu(self):
        ...
        if "type@=" not in recv_msg:
            self.no_message_count = self.no_message_count + 1
            if self.no_message_count > 10:
                raise TooManyNoMessageError()
            print(recv_msg)
            print("无效消息")
