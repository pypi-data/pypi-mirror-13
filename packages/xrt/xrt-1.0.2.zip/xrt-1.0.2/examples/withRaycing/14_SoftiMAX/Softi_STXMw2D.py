...
sys.path.append(r"/mnt/W7/Ray-tracing")
import numpy as np
import pickle
import time
...
#            nonZ = (np.abs(pE).sum(axis=0) > 0).sum()
#            print('{0} non zero images, {1} expected'.format(nonZ, repeats))
            cEr = pE[:, :repeats]
...
#            mPCA = np.zeros((repeats, repeats), dtype=np.complex128)
            for i in range(4):
