...
class Undulator(object):
    def __init__(self, bl=None, name='und', center=(0, 0, 0),
        ...
        *filamentBeam*: if True the source generates coherent monochromatic
            wavefronts. Required as True for the wave propagation calculations
            in partially coherent regime.
...
#            ii += self.gIntervals
    ...
    def tuning_curves(self, energy, theta, psi, harmonics, Ks):
        ...
        for iK, K in enumerate(Ks):
            ...
            self.Ky = K
            self.reset()
            I0, l1, l2, l3 = self.intensities_on_mesh(
                energy, theta, psi, harmonics)
...
#        np.seterr(divide='ignore')
        ...
        for ictx, ctx in enumerate(self.cl_ctx):
            ...
            if ictx < nctx - 1:
                ndsize.extend([np.floor(NRAYS*nCU[ictx]/totalCUs)])
            else:
                ndsize.extend([NRAYS-ndstart[ictx]])
            ndslice.extend([slice(ndstart[ictx], ndstart[ictx]+ndsize[ictx])])
            gamma_buf.extend([cl.Buffer(self.cl_ctx[ictx],
                                        self.cl_mf.READ_ONLY |
                                        self.cl_mf.COPY_HOST_PTR,
                                        hostbuf=gamma[ndslice[ictx]])])
            wu_buf.extend([cl.Buffer(self.cl_ctx[ictx],
                                     self.cl_mf.READ_ONLY |
                                     self.cl_mf.COPY_HOST_PTR,
                                     hostbuf=wu[ndslice[ictx]])])
            w_buf.extend([cl.Buffer(self.cl_ctx[ictx],
                                    self.cl_mf.READ_ONLY |
                                    self.cl_mf.COPY_HOST_PTR,
                                    hostbuf=rE[ndslice[ictx]])])
            ww1_buf.extend([cl.Buffer(self.cl_ctx[ictx],
                                      self.cl_mf.READ_ONLY |
                                      self.cl_mf.COPY_HOST_PTR,
                                      hostbuf=ww1[ndslice[ictx]])])
            ddtheta_buf.extend([cl.Buffer(self.cl_ctx[ictx],
                                          self.cl_mf.READ_ONLY |
                                          self.cl_mf.COPY_HOST_PTR,
                                          hostbuf=rTheta[ndslice[ictx]])])
            ddpsi_buf.extend([cl.Buffer(self.cl_ctx[ictx],
                                        self.cl_mf.READ_ONLY |
                                        self.cl_mf.COPY_HOST_PTR,
                                        hostbuf=rPsi[ndslice[ictx]])])
            tg_buf.extend([cl.Buffer(self.cl_ctx[ictx],
                                     self.cl_mf.READ_ONLY |
                                     self.cl_mf.COPY_HOST_PTR,
                                     hostbuf=tg)])
            ag_buf.extend([cl.Buffer(self.cl_ctx[ictx],
                                     self.cl_mf.READ_ONLY |
                                     self.cl_mf.COPY_HOST_PTR,
                                     hostbuf=ag)])
            Is_buf.extend([cl.Buffer(self.cl_ctx[ictx],
                                     self.cl_mf.READ_WRITE |
                                     self.cl_mf.COPY_HOST_PTR,
                                     hostbuf=Is_local[ndslice[ictx]])])
            Ip_buf.extend([cl.Buffer(self.cl_ctx[ictx],
                                     self.cl_mf.READ_WRITE |
                                     self.cl_mf.COPY_HOST_PTR,
                                     hostbuf=Ip_local[ndslice[ictx]])])
            ev_h2d.extend([[]])

            ev_h2d[ictx].extend([cl.enqueue_copy(self.cl_queue[ictx],
                                gamma[ndslice[ictx]], gamma_buf[ictx],
                                is_blocking=self.cl_is_blocking)])
            ev_h2d[ictx].extend([cl.enqueue_copy(self.cl_queue[ictx],
                                wu[ndslice[ictx]], wu_buf[ictx],
                                is_blocking=self.cl_is_blocking)])
            ev_h2d[ictx].extend([cl.enqueue_copy(self.cl_queue[ictx],
                                rE[ndslice[ictx]], w_buf[ictx],
                                is_blocking=self.cl_is_blocking)])
            ev_h2d[ictx].extend([cl.enqueue_copy(self.cl_queue[ictx],
                                ww1[ndslice[ictx]], ww1_buf[ictx],
                                is_blocking=self.cl_is_blocking)])
            ev_h2d[ictx].extend([cl.enqueue_copy(self.cl_queue[ictx],
                                rTheta[ndslice[ictx]], ddtheta_buf[ictx],
                                is_blocking=self.cl_is_blocking)])
            ev_h2d[ictx].extend([cl.enqueue_copy(self.cl_queue[ictx],
                                rPsi[ndslice[ictx]], ddpsi_buf[ictx],
                                is_blocking=self.cl_is_blocking)])
            ev_h2d[ictx].extend([cl.enqueue_copy(self.cl_queue[ictx],
                                tg, tg_buf[ictx],
                                is_blocking=self.cl_is_blocking)])
            ev_h2d[ictx].extend([cl.enqueue_copy(self.cl_queue[ictx],
                                ...
                                is_blocking=self.cl_is_blocking)])
            ev_h2d[ictx].extend([cl.enqueue_copy(self.cl_queue[ictx],
                                Is_local[ndslice[ictx]], Is_buf[ictx],
                                is_blocking=self.cl_is_blocking)])
            ev_h2d[ictx].extend([cl.enqueue_copy(self.cl_queue[ictx],
                                Ip_local[ndslice[ictx]], Ip_buf[ictx],
                                is_blocking=self.cl_is_blocking)])

            global_size.extend([Is_local[ndslice[ictx]].shape])

        local_size = None
#        for ictx, ctx in enumerate(self.cl_ctx):
#            print("h2d events status, context", ictx)
#            for iev, ev in enumerate(ev_h2d[ictx]):
#                status = cl.command_execution_status.to_string(
#                 ev.command_execution_status)
#                print("event", iev,  cl.command_execution_status.to_string(
#                      ev.command_execution_status))
        for ictx, ctx in enumerate(self.cl_ctx):
            if self.taper is not None:
                ev_run.extend([self.cl_program[ictx].undulator_taper(
                                    self.cl_queue[ictx],
                                    global_size[ictx],
                                    local_size,
                                    taper, Kx, Ky, phase,
                                    jend,
                                    gamma_buf[ictx], wu_buf[ictx],
                                    w_buf[ictx], ww1_buf[ictx],
                                    ddtheta_buf[ictx],
                                    ddpsi_buf[ictx],
                                    tg_buf[ictx], ag_buf[ictx],
                                    Is_buf[ictx], Ip_buf[ictx],
                                    wait_for=ev_h2d[ictx])])
            elif self.R0 is not None:
                ev_run.extend([self.cl_program[ictx].undulator_nf(
                                     self.cl_queue[ictx],
                                     global_size[ictx],
                                     local_size,
                                     R0, Kx, Ky, phase,
                                     ...
                                     gamma_buf[ictx], wu_buf[ictx],
                                     w_buf[ictx],
                                     ddtheta_buf[ictx], ddpsi_buf[ictx],
                                     tg_buf[ictx], ag_buf[ictx],
                                     Is_buf[ictx], Ip_buf[ictx],
                                     wait_for=ev_h2d[ictx])])
            else:
                ev_run.extend([self.cl_program[ictx].undulator(
                                      self.cl_queue[ictx],
                                      global_size[ictx],
                                      local_size,
                                      taper, Kx, Ky, phase,
        ...
        for ictx, ctx in enumerate(self.cl_ctx):
            cl.enqueue_copy(self.cl_queue[ictx],
                            Is_local[ndslice[ictx]],
                            Is_buf[ictx],
                            is_blocking=self.cl_is_blocking,
                            wait_for=[ev_run[ictx]])
            cl.enqueue_copy(self.cl_queue[ictx],
                            Ip_local[ndslice[ictx]],
                            Ip_buf[ictx],
                            is_blocking=self.cl_is_blocking,
                            wait_for=[ev_run[ictx]])
        ...
        bwFact = 0.001 if self.distE == 'BW' else 1./w
        if _DEBUG > 10:
            print("build_I_map_CL with {0} device{1} ({2} CUs) has taken"
