...
class Multilayer(object):
    ...
    def __init__(self, tLayer=None, tThickness=0., bLayer=None, bThickness=0.,
        ...
        :class:`Material`. The thicknesses of the layers are given in Å. If the
        multilayer is depth graded, *tThickness* and *bThickness* are on the
        surface and *tThicknessLow* and *bThicknessLow* are at the substrate.
        *power* defines the exponent of the layer thickness power law:

        .. math::
            d_n = A / (B + n)^{power}

        Without depth grading, *tThicknessLow* and *bThicknessLow* are ignored
        (left as zeros). *nPairs* is the number of layer pairs.
        """
        self.tLayer = tLayer
...
#            t2 = time.time()
            ...
            dti = np.float64(self.dti)
            dbi = np.float64(self.dbi)
            ...
            qt_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_ONLY |
                               cl.mem_flags.COPY_HOST_PTR, hostbuf=Qt)
            qb_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_ONLY |
                               cl.mem_flags.COPY_HOST_PTR, hostbuf=Qb)
            ...
            dti_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_WRITE |
                                cl.mem_flags.COPY_HOST_PTR, hostbuf=dti)
            dbi_buf = cl.Buffer(cl_ctx, cl.mem_flags.READ_WRITE |
                                cl.mem_flags.COPY_HOST_PTR, hostbuf=dbi)
            global_size = ri_p.shape
            local_size = None
            cl_program.get_amplitude_graded_multilayer(
                cl_queue,
                global_size,
                local_size,
