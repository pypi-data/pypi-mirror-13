...
#    beamLine.fsm3Z = (edges[:-1] + edges[1:]) * 0.5
    ...
    def afterScript():
        ...
        dump = []
        print("building mutual intencity matrices...")
...
##===#'random':================================================================
#            rank = 256
#            Exy = np.zeros_like(complexPlotEs.total2D)
#            Ixy = np.zeros_like(complexPlotIs.total2D)
#            Exy4D = np.zeros_like(complexPlotEs.total4D)
#            for r in range(rank):
#                randr = np.random.uniform(size=Exy.shape) - 0.5
#                randi = np.random.uniform(size=Exy.shape) - 0.5
##                randImage = np.cos(2*np.pi*randr) + 1j*np.sin(2*np.pi*randi)
#                randImage = randr + 1j*randi
#                Exy += randImage
#                Ixy += (randImage * randImage.conjugate()).real
#                Exy4D += np.outer(randImage, randImage.conjugate())
##=============================================================================
            print("solving eigenvalue problem, {0} of {1}".format(
                  ic, len(complexPlotsEs)))
            start1 = time.time()
...
#    import matplotlib as mpl
    ...
    def plot_modes(name, w, v):
            ...
            plt.text(0, imageExtent[-1]-2,
                     '0th (coherent) mode: w={0:.3f}'.format(w[-1]),
            ...
            plt.text(0, imageExtent[-1]-2,
                     '1st residual mode: w={0:.3f}'.format(w[-2]),
            ...
            plt.text(0, imageExtent[-1]-2,
                     '2nd residual mode: w={0:.3f}'.format(w[-3]),
            ...
            plt.text(0, imageExtent[-1]-2,
                     '3rd residual mode: w={0:.3f}'.format(w[-4]),
