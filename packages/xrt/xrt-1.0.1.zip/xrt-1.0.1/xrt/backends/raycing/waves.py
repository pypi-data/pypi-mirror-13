...
The accumulation of :math:`J_{12}` is done inside a plot object, please see its
options for selecting one of the three analysis ways and consider the example
:ref:`SoftiMAX`.
...
def _diffraction_integral_CL(oeLocal, n, nl, wave, good):
    ...
    x_mesh = myfloat(wave.xDiffr)
    y_mesh = myfloat(wave.yDiffr)
    z_mesh = myfloat(wave.zDiffr)

    nl_loc = myfloat(nl[good])
    bOElo_coord = np.array(
        [oeLocal.x[good], oeLocal.y[good], oeLocal.z[good], 0*oeLocal.z[good]],
        order='F', dtype=myfloat)
    n = np.array(n)
    if len(n.shape) < 2:
        n = n[:, None] * np.ones(oeLocal.x.shape)

    surface_normal = np.array(
        [n[0][good], n[1][good], n[2][good], 0*n[2][good]],
        order='F', dtype=myfloat)
    E_loc = myfloat(oeLocal.E[good])
#    bOEpath = myfloat(oeLocal.path[good])
    Es_loc = mycomplex(oeLocal.Es[good])
    Ep_loc = mycomplex(oeLocal.Ep[good])
    Es_res = np.zeros(imageRays, dtype=mycomplex)
    Ep_res = np.zeros(imageRays, dtype=mycomplex)
    aE_res = np.zeros(imageRays, dtype=mycomplex)
    bE_res = np.zeros(imageRays, dtype=mycomplex)
    cE_res = np.zeros(imageRays, dtype=mycomplex)
...
#    beam_OE_loc_path_buf = []
    ...
    nctx = len(waveCL.cl_ctx)
    time1 = time.time()
#    for ictx, ctx in enumerate(waveCL.cl_ctx):
    for ictx in range(nctx):
        nCU.extend([waveCL.cl_ctx[ictx].devices[0].max_compute_units])
    totalCUs = sum(nCU)

#    for ictx, ctx in enumerate(waveCL.cl_ctx):
    for ictx in range(nctx):
        ndstart.extend([sum(ndsize)])
        #print("ctx", ictx, "ndstart", ndstart[ictx])
        if ictx < nctx - 1:
            ndsize.extend([np.floor(imageRays*nCU[ictx]/totalCUs)])
        else:
            ndsize.extend([imageRays-ndstart[ictx]])
        #print("ctx", ictx, "ndsize", ndsize[ictx])
        ndslice.extend([slice(ndstart[ictx], ndstart[ictx]+ndsize[ictx])])
        #print("ctx", ictx, "slice start", ndstart[ictx])
        #print("ctx", ictx, "slice end", ndstart[ictx]+ndsize[ictx])

        nl_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                 waveCL.cl_mf.READ_ONLY |
                                 waveCL.cl_mf.COPY_HOST_PTR,
                                 hostbuf=nl_loc)])
        xlo_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                  waveCL.cl_mf.READ_ONLY |
                                  waveCL.cl_mf.COPY_HOST_PTR,
                                  hostbuf=x_mesh[ndslice[ictx]])])
        ylo_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                  waveCL.cl_mf.READ_ONLY |
                                  waveCL.cl_mf.COPY_HOST_PTR,
                                  hostbuf=y_mesh[ndslice[ictx]])])
        zlo_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                  waveCL.cl_mf.READ_ONLY |
                                  waveCL.cl_mf.COPY_HOST_PTR,
                                  hostbuf=z_mesh[ndslice[ictx]])])
        Es_loc_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                     waveCL.cl_mf.READ_ONLY |
                                     waveCL.cl_mf.COPY_HOST_PTR,
                                     hostbuf=Es_loc)])
        Ep_loc_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                     waveCL.cl_mf.READ_ONLY |
                                     waveCL.cl_mf.COPY_HOST_PTR,
                                     hostbuf=Ep_loc)])
        E_loc_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                    waveCL.cl_mf.READ_ONLY |
                                    waveCL.cl_mf.COPY_HOST_PTR,
                                    hostbuf=E_loc)])
        beamOElo_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                       waveCL.cl_mf.READ_ONLY |
                                       waveCL.cl_mf.COPY_HOST_PTR,
                                       hostbuf=bOElo_coord)])
        surface_normal_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                             waveCL.cl_mf.READ_ONLY |
                                             waveCL.cl_mf.COPY_HOST_PTR,
                                             hostbuf=surface_normal)])
#        beam_OE_loc_path_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
#                                               waveCL.cl_mf.READ_ONLY |
#                                               waveCL.cl_mf.COPY_HOST_PTR,
#                                               hostbuf=bOEpath)])
        KirchS_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                     waveCL.cl_mf.READ_WRITE |
                                     waveCL.cl_mf.COPY_HOST_PTR,
                                     hostbuf=Es_res[ndslice[ictx]])])
        KirchP_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                     waveCL.cl_mf.READ_WRITE |
                                     waveCL.cl_mf.COPY_HOST_PTR,
                                     hostbuf=Ep_res[ndslice[ictx]])])
        KirchA_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                     waveCL.cl_mf.READ_WRITE |
                                     waveCL.cl_mf.COPY_HOST_PTR,
                                     hostbuf=aE_res[ndslice[ictx]])])
        KirchB_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                     waveCL.cl_mf.READ_WRITE |
                                     waveCL.cl_mf.COPY_HOST_PTR,
                                     hostbuf=bE_res[ndslice[ictx]])])
        KirchC_buf.extend([cl.Buffer(waveCL.cl_ctx[ictx],
                                     waveCL.cl_mf.READ_WRITE |
                                     waveCL.cl_mf.COPY_HOST_PTR,
                                     hostbuf=cE_res[ndslice[ictx]])])

        ev_h2d.extend([[]])

        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            nl_loc, nl_buf[ictx],
                            is_blocking=False)])
        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            x_mesh[ndslice[ictx]], xlo_buf[ictx],
                            is_blocking=False)])
        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            y_mesh[ndslice[ictx]], ylo_buf[ictx],
                            is_blocking=False)])
        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            z_mesh[ndslice[ictx]], zlo_buf[ictx],
                            is_blocking=False)])
        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            Es_loc, Es_loc_buf[ictx],
                            is_blocking=False)])
        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            Ep_loc, Ep_loc_buf[ictx],
                            is_blocking=False)])
        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            E_loc, E_loc_buf[ictx],
                            is_blocking=False)])
        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            bOElo_coord, beamOElo_buf[ictx],
                            is_blocking=False)])
        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            surface_normal, surface_normal_buf[ictx],
                            is_blocking=False)])
#        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
#                            bOEpath, beam_OE_loc_path_buf[ictx],
#                            is_blocking=False)])

        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            Es_res[ndslice[ictx]], KirchS_buf[ictx],
                            is_blocking=False)])
        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            Ep_res[ndslice[ictx]], KirchP_buf[ictx],
                            is_blocking=False)])
        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            aE_res[ndslice[ictx]], KirchA_buf[ictx],
                            is_blocking=False)])
        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            bE_res[ndslice[ictx]], KirchB_buf[ictx],
                            is_blocking=False)])
        ev_h2d[ictx].extend([cl.enqueue_copy(waveCL.cl_queue[ictx],
                            cE_res[ndslice[ictx]], KirchC_buf[ictx],
                            is_blocking=False)])

        global_size.extend([Es_res[ndslice[ictx]].shape])

    local_size = None

    #for ictx, ctx in enumerate(waveCL.cl_ctx):
    for ictx in range(nctx):
        code = waveCL.cl_program[ictx].integrate_kirchhoff
        ev_run.extend(
            [code(waveCL.cl_queue[ictx],
             global_size[ictx],
             local_size,
             np.int32(ndsize[ictx]),
             frontRays,
             myfloat(CHBAR),
             nl_buf[ictx],
             xlo_buf[ictx], ylo_buf[ictx], zlo_buf[ictx],
             Es_loc_buf[ictx], Ep_loc_buf[ictx],
             E_loc_buf[ictx], beamOElo_buf[ictx],
             surface_normal_buf[ictx],  # beam_OE_loc_path_buf[ictx],
             KirchS_buf[ictx], KirchP_buf[ictx],
             KirchA_buf[ictx], KirchB_buf[ictx], KirchC_buf[ictx],
             wait_for=ev_h2d[ictx])])
    ...
    for iev, ev in enumerate(ev_run):
        ...
        if _DEBUG > 20:
            print("ctx status", iev, status)

    for ictx in range(nctx):
        cl.enqueue_copy(waveCL.cl_queue[ictx],
                        Es_res[ndslice[ictx]],
                        KirchS_buf[ictx],
                        is_blocking=False,
                        wait_for=[ev_run[ictx]])
        cl.enqueue_copy(waveCL.cl_queue[ictx],
                        Ep_res[ndslice[ictx]],
                        KirchP_buf[ictx],
                        is_blocking=False,
        ...
        cl.enqueue_copy(waveCL.cl_queue[ictx],
                        ...
                        wait_for=[ev_run[ictx]])
    if _DEBUG > 10:
        print("diffract_CL with {0} device{1} ({2} CUs) has taken"
              " {3} s".format(nctx, 's' if nctx > 1 else '', int(totalCUs),
                              time.time() - time1))
