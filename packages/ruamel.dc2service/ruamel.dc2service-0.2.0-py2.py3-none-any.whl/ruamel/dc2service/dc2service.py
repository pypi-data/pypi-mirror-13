...
class DC2Service(object):
    ...
    def gen_upstart(self, data):
        service_file = Path('/etc/init') / ('-'.join(data['name']) +
                                            '-docker.conf')
        print(service_file)
        template = (data['template'] / 'upstart.template').read_bytes()
        content = template.format(**data)
        # Path('/var/tmp/xxy').write_bytes(content)
        print(content)
    ...
    def gen_systemd(self, data):
        service_file = Path('/etc/systemd/system') / ('-'.join(data['name']) +
                                                      '-docker.service')
        print(service_file)
        template = (data['template'] / 'systemd.template').read_bytes()
        content = template.format(**data)
        print(content)
        # Path('/var/tmp/xxy').write_bytes(content)
        td = Path(tempfile.mkdtemp(prefix='tmp_dc2service'))
    ...
    def _get_data(self, dc_file):
        ...
        ret_val = {}
        error = False
        for pre_comment in _data.ca.comment[1]:
    ...
    def determine_init_service(self):
        ...
        if 'systemd' in Path('/sbin/init').resolve().name:
            return 'systemd'
        try:
            res = subprocess.check_output(['/sbin/init', '--version'])
            if 'upstart' in res:
                return 'upstart'
