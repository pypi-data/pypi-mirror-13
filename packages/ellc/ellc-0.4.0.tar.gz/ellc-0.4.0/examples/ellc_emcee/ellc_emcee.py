...
def _lnlike(varpar_v, lcdata, varpar_l, fixpar_d, priors, ldy_, 
  ...
  try:
    p = varpar_v[(([i for i,s in enumerate(varpar_l) if s=='P'])[0])]
  except:
    p = fixpar_d['P']
  ...
  if return_fit:
    ...
    f = lc(t, radius_1=r_1, radius_2=r_2, sbratio=sb2, incl=incl, 
        light_3=l_3, t_zero=t0, period=p, q=q, f_c=f_c, f_s=f_s,
        rotfac_1=frot1, rotfac_2=frot2,n_int=n_int,
        ld_1='claret', ldc_1=ldc_1, ld_2='claret', ldc_2=ldc_2,
    ...
    wt = 1/e_fit**2
    zp = np.average( (m_mod-m_fit),weights=wt)
    m = m - zp
    return m 
  ...
  e_fit = (lcdata[lcdata['flag'] >= 0])['e_mag']
  wt = 1/e_fit**2
  zp = np.average( (m-m_fit),weights=wt)
  m_fit = m_fit - zp
  chisq = np.sum( wt*(m-m_fit)**2 )
  ...
  for p in priors:
    ...
    if p['p_type'] == 'U':
      ...
      raise Exception('Unknown prior type',self.p_type)
...
def main():
  ...
  parser.add_argument("--print-frequency", "-p", 
    default=100,type=int,metavar='N',
    help='Print summary of progress every N steps'
  )
 ...
 # Could probably tidy this up by iterating over list of varaibles - oh well..
  ...
  teff1,e_teff1 = _get_param('t1 = T_eff,1',None,3500,50000)
  if e_teff1 is None:
  ...
  teff2,e_teff2 = _get_param('t2 = T_eff,1',None,3500,50000)
  if e_teff2 is None:
  ...
  allowed_prior_vars = ['e','om','r_1','r_2','rsum','k','sb2','incl','l_3',
      'T_0','P', 'f_c', 'f_s', 'F_1', 'F_2', 'Teff1', 'logg1', 'Teff2', 
      'logg2', '[M/H]'] 
...
# Limb- and gravity- darkening instance
  ...
  t.add_column(Column(-0.5*sampler.flatlnprobability,name='chisq'))
  ...
  best_index = np.unravel_index(np.argmax(sampler.lnprobability),
      (nwalkers, nsteps))
  best_chisq = -0.5*np.max(sampler.lnprobability)
  print('\n Best chi-squared = ',best_chisq,' in walker ',best_index[0],
