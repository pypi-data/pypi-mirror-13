...
class MovingAverageCrossStrategy(Strategy):
    ...
    def handle_data(self):
        ...
        ihc, ifc, icc = 'ih%02d%02d' % (contract_year, contract_month), \
                        'if%02d%02d' % (contract_year, contract_month), \
                        'ic%02d%02d' % (contract_year, contract_month)

        ihcOld, ifcOld, iccOld = 'ih%02d%02d' % (year, month), \
                                 'if%02d%02d' % (year, month), \
                                 'ic%02d%02d' % (year, month)
...
def run_example():
    ...
    # futures to trade
    ihs = ['ih15%02d' % i for i in range(5, 13)] + ['ih16%02d' % i for i in range(1, 2)]
    ifs = ['if15%02d' % i for i in range(5, 13)] + ['if16%02d' % i for i in range(1, 2)]
    ics = ['ic15%02d' % i for i in range(5, 13)] + ['ic16%02d' % i for i in range(1, 2)]
