...
class MovingAverageCrossStrategy(Strategy):
    ...
    def handle_data(self):
        ...
        if self.current_datetime.hour == 15 and self.current_datetime.minute > 0:
            return
        ...
        ihc, ifc, icc = 'ih15%02d.cffex' % contract_month, 'if15%02d.cffex' % contract_month, 'ic15%02d.cffex' % contract_month
        ihcOld, ifcOld, iccOld = 'ih15%02d.cffex' % month, 'if15%02d.cffex' % month, 'ic15%02d.cffex' % month
...
def run_example():
    ...
    # futures to trade
    ihs = ['ih15%02d.cffex' % i for i in range(3, 13)]
    ifs = ['if15%02d.cffex' % i for i in range(3, 13)]
    ics = ['ic15%02d.cffex' % i for i in range(3, 13)]

    futures = ihs + ifs + ics

    universe = indexes + futures
    startDate = dt.datetime(2015, 5, 5)
    endDate = dt.datetime(2015, 12, 14)
