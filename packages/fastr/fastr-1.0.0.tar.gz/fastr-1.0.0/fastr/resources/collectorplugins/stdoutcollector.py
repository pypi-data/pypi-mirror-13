...
class StdoutCollector(PLUGIN_TYPE):
    """
    ...
    * ``special`` which has two subfields:
    ...
    def _collect_results(self, interface, output, result):
        ...
        loc = location.format(input=inputs, output=outputs, special=specials)
        for line in stdout.splitlines():
            match = re.search(loc, line)
            if match is not None:
                value = '{regexp[0]}'.format(input=inputs, output=outputs, special=specials, regexp=match.groups())
