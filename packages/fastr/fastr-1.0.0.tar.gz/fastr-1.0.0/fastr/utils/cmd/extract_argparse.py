...
def main(args=None, unknown_args=None):
    ...
    tool.hash = hash(os.path.abspath(os.path.normpath(tool.filename + filepath)))
    ...
    tool.inputs = OrderedDict()
    tool.outputs = OrderedDict()
    ...
    for option in parser._optionals._group_actions:
        ...
        tool.inputs[param['id']] = InputParameterDescription(tool, param)
