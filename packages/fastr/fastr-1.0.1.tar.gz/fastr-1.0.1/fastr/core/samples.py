...
class SampleItemBase(tuple):
    ...
    def __new__(cls, index, id_, data, jobs=None):
        ...
        if not isinstance(jobs, set):
            raise exceptions.FastrTypeError("Jobs should be a set of jobs, found {}".format(type(jobs).__name__))
...
class SampleIndex(SampleBaseId):
    ...
    def expand(self, size):
        ...
        # We use the star-args here because it not doing so would make life painful
...
class SampleValue(MutableMapping):
    ...
    def __init__(self, *args, **kwargs):
        ...
        #if not all(isinstance(x, fastr.datatypes.DataType) for x in self.iterelements()):
        #    raise exceptions.FastrTypeError('All elements of a SampleValue should be an instance of fastr.datatypes.DataType')
...
class SampleCollection(MutableMapping):
    ...
    def __init__(self, dimnames, datatype, parent):
        """
    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(value, SampleItem):
            ...
            value = SampleItem(value.index, value.id, value.data.cast(self.datatype), value.jobs)
            ...
            if len(value) == 2 and isinstance(value[0], SampleValue) and isinstance(value[1], set):
                ...
                value = SampleItem(sample_index, sample_id, value[0].cast(self.datatype), value[1])
            else:
            ...
            value = SampleItem(sample_index, sample_id, value.cast(self.datatype), set())
    ...
    def datatype(self):
        """
        The datatype of the item in the SampleCollection

        :rtype: fastr.datatypes.DataType
        """
