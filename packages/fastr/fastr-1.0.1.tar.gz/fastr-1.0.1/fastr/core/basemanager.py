...
class BaseManager(collections.MutableMapping):
    ...
    def __repr__(self):
        ...
        keylist = [self._print_key(x) for x in self.data.keys()]
        if isinstance(keylist[0], tuple):
            # It is just useful here :-)
            # pylint: disable=star-args
            width = zip(*keylist)
            width = [max([len(x) for x in y]) for y in width]
            format_base_key = '  '.join(['{{key[{n}]:<{width}}}'.format(n=n, width=w) for n, w in enumerate(width)])
        else:
            width = max([len(x) for x in keylist if x is not None])
            format_base_key = '{{key:<{width}}}'.format(width=width)

        valuelist = [self._print_value(x) for x in self.data.values()]
        if isinstance(valuelist[0], tuple):
            ...
            width = max([len(x) for x in valuelist])
    ...
    def _scan_directory(self, path, recursive=False):
        """
        ...
        if not isinstance(path, (str, unicode)) and isinstance(path, collections.Iterable):
            ...
            for filename in os.listdir(path):
                ...
                if os.path.isdir(filepath):
                    if recursive:
                        self._scan_directory(filepath, recursive)
                elif os.path.isfile(filepath) and ext == self._item_extension:
                    # Since we cannot know what Plugins might throw, catch all
                    # pylint: disable=bare-except
                    try:
                        self._load_item(filepath)
                    except:
                        fastr.log.warning('Could not load file {}'.format(filepath))
                        exc_type, _, _ = sys.exc_info()
                        exc_info = traceback.format_exc()
                        fastr.log.error('Encountered exception ({}) during loading:\n{}'.format(exc_type.__name__, exc_info))
