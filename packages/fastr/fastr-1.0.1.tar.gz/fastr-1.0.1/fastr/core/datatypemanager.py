...
from fastr.datatypes import BaseDataType, DataType, TypeGroup, EnumType
import fastr.exceptions as exceptions
from fastr.utils.checksum import hashsum


class DataTypeManager(PluginManager):
    """
    ...
    def _populate(self):
        ...
        """
        super(DataTypeManager, self)._populate()
    ...
    def guess_type(self, value, exists=True, options=None):
        """
        ...
        if len(candidates) == 0:
            fastr.log.warning('No valid combinations of options and candidates!')
            return None
        ...
        fastr.log.info('Final candidates: {}'.format(final_candidates))
    ...
    def match_types(self, *args, **kwargs):
        ...
        if len(options) == 0:
            ...
            fastr.log.warning("No preferred DataType matches, (options {}, preferred {})".format(options, preferred))
            return None
