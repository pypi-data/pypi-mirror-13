...
class VirtualFileSystem(IOPlugin):
    ...
    def url_to_path(self, url):
        ...
        .. code-block:: python
          ...
          >>> fastr.vfs_to_path('vfs://tmp/file.ext')
          '/tmp/file.ext'
        ...
        if parsed_url.scheme == self.scheme:
            try:
                return os.path.join(fastr.config.mounts[parsed_url.netloc.lower()],
                                    self._correct_separators(parsed_url.path))
            except KeyError:
    ...
    def path_to_url(self, path, mountpoint=None):
        ...
        if mountpoint is not None:
            return "{scheme}://{mnt}/{path}".format(scheme=self.scheme, mnt=mountpoint, path=path.strip(os.path.sep))
        else:
            ...
            return "{scheme}://{mnt}/{path}".format(scheme=self.scheme, mnt=mnt, path=path.strip(os.path.sep))
