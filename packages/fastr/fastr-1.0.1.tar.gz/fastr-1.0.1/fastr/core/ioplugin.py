...
class IOPluginManager(PluginManager):
    ...
    def __init__(self, path=None, recursive=None):
        ...
        :param path: path to scan, or None to take the path from the config
        :type path: str or None
        :param bool recursive: indicate whether or not to scan the directory recursively
        :return: newly created IOPluginManager
        """
        if path is None:
            path = fastr.config.io_plugins_path

        super(IOPluginManager, self).__init__(path, recursive)
    ...
    def plugin_class(self):
        """
        The PluginClass of the items of the PluginManager
        """
        return IOPlugin

    def _store_item(self, name, value):
        """
        Store an IOPlugin in the IOPluginManager and register the url scheme

        :param name: name of the IOPlugin
        :param value: the IOPlugin to store
        """
        scheme = value.scheme

        if scheme in self:
            fastr.log.debug('Skipping {} from {} (the plugin is already in the {})'.format(scheme, value.filename, type(self).__name__))
        else:
            fastr.log.debug('Loaded IOPlugin {} from {}'.format(scheme, value.filename))
            self.register_url_scheme(scheme)
            self[scheme] = value
    ...
    def _populate(self):
        ...
        """
        super(IOPluginManager, self)._populate()
        if 'vfs' not in self:
            self['vfs'] = fastr.vfs
    ...
    def create_ioplugin_tool():
        ...
        # FIXME: Interfaces won't accept no inputs or no outputs
        # TODO: Add Tool class that hides the corrects inputs, but add them in the interface
        if ('Source', '1.0') not in fastr.toollist:
            source_tool = Tool()
            source_tool.id = 'Source'
            source_tool.name = source_tool.id
            source_tool.version = Version('1.0')
            source_tool.filename = __file__
            source_tool.hash = hash(os.path.abspath(os.path.normpath(source_tool.filename)))
            ...
            fastr.toollist[source_tool.id, source_tool.version] = source_tool

        if ('Sink', '1.0') not in fastr.toollist:
            sink_tool = Tool()
            sink_tool.id = 'Sink'
            sink_tool.name = sink_tool.id
            sink_tool.version = Version('1.0')
            sink_tool.filename = __file__
            sink_tool.hash = hash(os.path.abspath(os.path.normpath(sink_tool.filename)))
            ...
            fastr.toollist[sink_tool.id, sink_tool.version] = sink_tool


class IOPlugin(BasePlugin):
    """
    Base class for all IOPlugins. An IOPlugin manages the Source input and Sink
    output for for a specific url scheme.
    """
