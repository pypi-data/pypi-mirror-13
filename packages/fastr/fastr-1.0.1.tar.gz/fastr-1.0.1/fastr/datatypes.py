...
class BaseDataType(BasePlugin):
    ...
    def test(cls):
        """
        Define the test for the PluginManager. Make sure we are not one of the
        base classes
        """
    ...
    def valid(self):
        ...
        """
        if isinstance(self.value, str) and self.value.startswith('val://'):
            fastr.log.debug('Value URL can be valid, this can only be known upon evaluation')
            return True  # Until we know better, this should be considered valid
