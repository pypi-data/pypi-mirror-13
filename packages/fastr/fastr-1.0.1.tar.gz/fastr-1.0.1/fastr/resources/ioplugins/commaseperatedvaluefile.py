...
class CommaSeperatedValueFile(IOPlugin):
    ...
    variable  usage
    ========= =============================================================================================
    value     indicates the column containing the value of interest, can be int for index or string for key
    id        indicates the column containing the sample id (optional)
    header    indicates if the first row is considered the header, can be ``true`` or ``false`` (optional)
    delimiter indicates the delimiter used in the csv file (optional)
    quote     indicates the quote character used in the csv file (optional)
    ========= =============================================================================================
    ...
    def expand_url(self, url):
        ...
        relpath = {'0': False, '1': True, 'false': False, 'true': True, 'none': False}[query.get('relpath', ('none',))[0]]
        if baseurl[-1] != '/':
            baseurl = baseurl + '/'
        ...
        with open(csvpath, 'rb') as csvfile:
            ...
            for row in reader:
                ...
                if not relpath:
                    ...
                    values.append(baseurl + row[data_column])
