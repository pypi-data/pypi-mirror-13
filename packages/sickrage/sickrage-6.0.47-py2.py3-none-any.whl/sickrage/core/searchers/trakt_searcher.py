...
def setEpisodeToWanted(show, s, e):
    ...
    if epObj:
        ...
        with epObj.lock:
            ...
            sickrage.srCore.LOGGER.info("Setting episode %s S%02dE%02d to wanted" % (show.name, s, e))
...
class srTraktSearcher(object):
    def __init__(self, *args, **kwargs):
        ...
        self.trakt_api = TraktAPI(sickrage.srCore.CONFIG.SSL_VERIFY, sickrage.srCore.CONFIG.TRAKT_TIMEOUT)
        self.todoBacklog = []
        self.todoWanted = []
        self.ShowWatchlist = {}
    ...
    def run(self, force=False):
        ...
        if sickrage.srCore.CONFIG.TRAKT_SYNC_WATCHLIST:
            ...
            if len(sickrage.srCore.CONFIG.ROOT_DIRS.split('|')) < 2:
                sickrage.srCore.LOGGER.warning("No default root directory")
                return
    ...
    def findShow(self, indexer, indexerid):
        ...
        try:
            ...
            if not library:
                sickrage.srCore.LOGGER.warning("Could not connect to trakt service, aborting library check")
                return

            if not len(library):
                sickrage.srCore.LOGGER.debug("No shows found in your library, aborting library update")
                return
            ...
            sickrage.srCore.LOGGER.warning("Could not connect to Trakt service. Aborting library check. Error: %s" % repr(e))
    ...
    def removeShowFromTraktLibrary(self, show_obj):
        if self.findShow(show_obj.indexer, show_obj.indexerid):
            ...
            sickrage.srCore.LOGGER.debug("Removing %s from tv library" % show_obj.name)
    ...
    def addShowToTraktLibrary(self, show_obj):
        ...
        if len(data):
            sickrage.srCore.LOGGER.debug("Adding %s to tv library" % show_obj.name)
    ...
    def syncLibrary(self):
        if sickrage.srCore.CONFIG.TRAKT_SYNC and sickrage.srCore.CONFIG.USE_TRAKT:
            sickrage.srCore.LOGGER.debug("Sync SiCKRAGE with Trakt Collection")

            if self._getShowCollection():
                self.addEpisodeToTraktCollection()
                if sickrage.srCore.CONFIG.TRAKT_SYNC_REMOVE:
                    self.removeEpisodeFromTraktCollection()

    def removeEpisodeFromTraktCollection(self):
        if sickrage.srCore.CONFIG.TRAKT_SYNC_REMOVE and sickrage.srCore.CONFIG.TRAKT_SYNC and sickrage.srCore.CONFIG.USE_TRAKT:
            sickrage.srCore.LOGGER.debug("COLLECTION::REMOVE::START - Look for Episodes to Remove From Trakt Collection")
            ...
            if episodes is not None:
                ...
                for cur_episode in episodes:
                    ...
                    if self._checkInList(trakt_id, str(cur_episode[b"showid"]), str(cur_episode[b"season"]),
                        ...
                        if cur_episode[b"location"] == '':
                            sickrage.srCore.LOGGER.debug("Removing Episode %s S%02dE%02d from collection" %
                ...
                if len(trakt_data):
                    try:
                        ...
                        sickrage.srCore.LOGGER.warning("Could not connect to Trakt service. Error: %s" % e)

            sickrage.srCore.LOGGER.debug("COLLECTION::REMOVE::FINISH - Look for Episodes to Remove From Trakt Collection")

    def addEpisodeToTraktCollection(self):
        if sickrage.srCore.CONFIG.TRAKT_SYNC and sickrage.srCore.CONFIG.USE_TRAKT:
            sickrage.srCore.LOGGER.debug("COLLECTION::ADD::START - Look for Episodes to Add to Trakt Collection")
            ...
            if episodes is not None:
                ...
                for cur_episode in episodes:
                    ...
                    if not self._checkInList(trakt_id, str(cur_episode[b"showid"]), str(cur_episode[b"season"]),
                                             str(cur_episode[b"episode"]), List='Collection'):
                        sickrage.srCore.LOGGER.debug("Adding Episode %s S%02dE%02d to collection" %
                                                 (cur_episode[b"show_name"], cur_episode[b"season"], cur_episode[b"episode"]))
            ...
            sickrage.srCore.LOGGER.debug("COLLECTION::ADD::FINISH - Look for Episodes to Add to Trakt Collection")

    def syncWatchlist(self):
        if sickrage.srCore.CONFIG.TRAKT_SYNC_WATCHLIST and sickrage.srCore.CONFIG.USE_TRAKT:
            sickrage.srCore.LOGGER.debug("Sync SiCKRAGE with Trakt Watchlist")
    ...
    def removeEpisodeFromTraktWatchList(self):
        if sickrage.srCore.CONFIG.TRAKT_SYNC_WATCHLIST and sickrage.srCore.CONFIG.USE_TRAKT:
            sickrage.srCore.LOGGER.debug("WATCHLIST::REMOVE::START - Look for Episodes to Remove from Trakt Watchlist")
            ...
            if episodes is not None:
                ...
                for cur_episode in episodes:
                    ...
                    if self._checkInList(trakt_id, str(cur_episode[b"showid"]), str(cur_episode[b"season"]),
                        ...
                        sickrage.srCore.LOGGER.debug("Removing Episode %s S%02dE%02d from watchlist" %
                                                 (cur_episode[b"show_name"], cur_episode[b"season"], cur_episode[b"episode"]))
                        trakt_data.append((cur_episode[b"showid"], cur_episode[b"indexer"], cur_episode[b"show_name"],
                                           cur_episode[b"startyear"], cur_episode[b"season"], cur_episode[b"episode"]))
                ...
                sickrage.srCore.LOGGER.debug("WATCHLIST::REMOVE::FINISH - Look for Episodes to Remove from Trakt Watchlist")

    def addEpisodeToTraktWatchList(self):
        if sickrage.srCore.CONFIG.TRAKT_SYNC_WATCHLIST and sickrage.srCore.CONFIG.USE_TRAKT:
            sickrage.srCore.LOGGER.debug("WATCHLIST::ADD::START - Look for Episodes to Add to Trakt Watchlist")
            ...
            if episodes is not None:
                ...
                for cur_episode in episodes:
                    ...
                    if not self._checkInList(trakt_id, str(cur_episode[b"showid"]), str(cur_episode[b"season"]),
                        ...
                        sickrage.srCore.LOGGER.debug("Adding Episode %s S%02dE%02d to watchlist" %
                                                 (cur_episode[b"show_name"], cur_episode[b"season"], cur_episode[b"episode"]))
                        trakt_data.append((cur_episode[b"showid"], cur_episode[b"indexer"], cur_episode[b"show_name"],
            ...
            sickrage.srCore.LOGGER.debug("WATCHLIST::ADD::FINISH - Look for Episodes to Add to Trakt Watchlist")

    def addShowToTraktWatchList(self):
        if sickrage.srCore.CONFIG.TRAKT_SYNC_WATCHLIST and sickrage.srCore.CONFIG.USE_TRAKT:
            sickrage.srCore.LOGGER.debug("SHOW_WATCHLIST::ADD::START - Look for Shows to Add to Trakt Watchlist")
            ...
            sickrage.srCore.LOGGER.debug("SHOW_WATCHLIST::ADD::FINISH - Look for Shows to Add to Trakt Watchlist")

    def removeShowFromSickRage(self):
        if sickrage.srCore.CONFIG.TRAKT_SYNC_WATCHLIST and sickrage.srCore.CONFIG.USE_TRAKT and sickrage.srCore.CONFIG.TRAKT_REMOVE_SHOW_FROM_SICKRAGE:
            sickrage.srCore.LOGGER.debug("SHOW_SICKRAGE::REMOVE::START - Look for Shows to remove from SiCKRAGE")
                ...
                for show in sickrage.srCore.SHOWLIST:
                    if show.status == "Ended":
                        ...
                        if 'aired' in progress and 'completed' in progress and progress[b'aired'] == progress[
                            ...
                            sickrage.srCore.LOGGER.debug("Show: %s has been removed from SiCKRAGE" % show.name)

            sickrage.srCore.LOGGER.debug("SHOW_SICKRAGE::REMOVE::FINISH - Trakt Show Watchlist")

    def updateShows(self):
        sickrage.srCore.LOGGER.debug("SHOW_WATCHLIST::CHECK::START - Trakt Show Watchlist")

        if not len(self.ShowWatchlist):
            sickrage.srCore.LOGGER.debug("No shows found in your watchlist, aborting watchlist update")
            return

        indexer = int(sickrage.srCore.CONFIG.TRAKT_DEFAULT_INDEXER)
        ...
        sickrage.srCore.LOGGER.debug("SHOW_WATCHLIST::CHECK::FINISH - Trakt Show Watchlist")

    def updateEpisodes(self):
        """
        Sets episodes to wanted that are in trakt watchlist
        """
        sickrage.srCore.LOGGER.debug("SHOW_WATCHLIST::CHECK::START - Trakt Episode Watchlist")

        if not len(self.EpisodeWatchlist):
            sickrage.srCore.LOGGER.debug("No episode found in your watchlist, aborting episode update")
            return
        ...
        for show_el in self.EpisodeWatchlist[trakt_id]:
            ...
            try:
                ...
                sickrage.srCore.LOGGER.debug("Could not parse the output from trakt for %s " % show[b"title"])
        sickrage.srCore.LOGGER.debug("SHOW_WATCHLIST::CHECK::FINISH - Trakt Episode Watchlist")
    ...
    def addDefaultShow(indexer, indexer_id, name, status):
        ...
        if not findCertainShow(sickrage.srCore.SHOWLIST, int(indexer_id)):
            sickrage.srCore.LOGGER.info("Adding show " + str(indexer_id))
            ...
            if location:
                ...
                if not dir_exists:
                    sickrage.srCore.LOGGER.warning("Unable to create the folder %s , can't add the show" % showPath)
                    return
                else:
                ...
                sickrage.srCore.SHOWQUEUE.addShow(int(indexer), int(indexer_id), showPath,
                                              ...
                                              quality=int(sickrage.srCore.CONFIG.QUALITY_DEFAULT),
                                              flatten_folders=int(sickrage.srCore.CONFIG.FLATTEN_FOLDERS_DEFAULT),
                                              paused=sickrage.srCore.CONFIG.TRAKT_START_PAUSED,
                                              default_status_after=status,
                                              archive=sickrage.srCore.CONFIG.ARCHIVE_DEFAULT)
            else:
                sickrage.srCore.LOGGER.warning("There was an error creating the show, no root directory setting found")
                return

    def manageNewShow(self, show):
        sickrage.srCore.LOGGER.debug("Checking if trakt watch list wants to search for episodes from new show " + show.name)
    ...
    def _getShowWatchlist(self):
        ...
        try:
            ...
            sickrage.srCore.LOGGER.warning("Could not connect to trakt service, cannot download Show Watchlist: %s" % repr(e))
            return False
        return True
    ...
    def _getEpisodeWatchlist(self):
        ...
        try:
            ...
            sickrage.srCore.LOGGER.warning("Could not connect to trakt service, cannot download Episode Watchlist: %s" % repr(e))
            return False
        return True
    ...
    def _getShowCollection(self):
        ...
        try:
            ...
            sickrage.srCore.LOGGER.debug("Getting Show Collection")
            ...
            sickrage.srCore.LOGGER.warning("Could not connect to trakt service, cannot download Show Collection: %s" % repr(e))
            return False
        return True
