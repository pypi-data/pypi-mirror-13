...
def upgrade_packages(user=False):
    ...
    pip_install_cmd = InstallCommand()
    pip_uninstall_cmd = UninstallCommand()
    ...
    while True:
        ...
        install_options = pip_install_cmd.parse_args(['-q', '--upgrade', '--no-cache-dir'])[0]
        install_options.use_user_site = user

        uninstall_options = pip_uninstall_cmd.parse_args(['-q'])[0]
        uninstall_options.use_user_site = user
        ...
        for i, pkg_name in enumerate(packages, start=1):
            try:
                ...
                pip_uninstall_cmd.run(uninstall_options, [pkg_name])
                pip_install_cmd.run(install_options, [pkg_name])
            except KeyboardInterrupt:
                raise
        else:
