...
r['model1'], 'random', 'model1', prot=prot)]
    ...
    def plot(self, nbins=20, teams=None, logx=False, logy=False):
        """Plots all the Distances null model distribution for each model,
        for the parameters and timne_course cases

        This functions plots the histogram of each distances null model
        together with the teams distancse. 6 plots are generated one for
        each model (3) for the parameters case and time course case.
        Distribution are fitted with normal or exponential distribution.
...
def plot_dprot_vs_dparam(c=None):
    if c==None:
        ...
        c.compute_scores()
    d = [(k,v) for k,v in c.parameters_distances.items() if "copasi" not in k]
    tc = [(k,v) for k,v in c.time_course_distances.items() if "copasi" not in k]
