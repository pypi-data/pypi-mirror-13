"""
Beginnings of a general data analyzer for data generated with my MotionDetectorModel.
    ...
    def compute_direction_arrays_mean(self,t_start,t_end):
        ...
        for i in range(0,len(self.directions)):
            if ['HP','HN','VP','VN'].count(self.directions[i]):
                _mean=self.neurons[names[i]][np.where((self.t>t_start) & (self.t<t_end))].mean()
