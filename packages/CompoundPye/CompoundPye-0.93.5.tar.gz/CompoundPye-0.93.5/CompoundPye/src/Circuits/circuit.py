...
class Circuit:
    ...
    def create_weight_matrices(self):
        ...
        self.components_weight_matrix=np.zeros((self.n_comp,self.n_comp))
        
        for i in range(0,self.n_comp):
            for j in range(0,len(self.components[i].connections)):
                self.components_weight_matrix[self.components[i].connections[j].target.index_in_circuit_list][i]=self.components[i].connections[j].weight
        ...
        self.sensors_weight_matrix=np.zeros((self.n_comp,n_sens))
            
        for i in range(0,n_sens):
            for j in range(0,len(self.sensors[i].connections)):
                self.sensors_weight_matrix[self.sensors[i].connections[j].target.index_in_circuit_list][i]=self.sensors[i].connections[j].weight
                
        if self.debug:
            print '-------debugging output-----------------'
            print '-------function: Circuit.create_weight_matrices()-------'
            print 'self.components_weight_matrix:'
            print self.components_weight_matrix
            print 'self.sensors_weight_matrix:'
            print self.sensors_weight_matrix
            print '----------------------------------------'
    ...
    def update(self,dt,intensities):
        ...
        ''' 
        inputs=np.dot(self.sensors_weight_matrix,sensor_values)+np.dot(self.components_weight_matrix,outputs)
