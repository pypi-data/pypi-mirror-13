...
class StateMachine(threading.Thread, Machine, metaclass=abc.ABCMeta):
    ...
    def _notify_message(self, msg):
        if type(msg) in self._wait_for_message:
            self._wait_for_message.pop(msg)\
                .foreach(lambda f: f.set_result(True))

    def await_message(self, tpe):
        if not tpe in self._wait_for_message:
            self._wait_for_message[tpe] = List()
        fut = concurrent.futures.Future()
        self._wait_for_message[tpe] = self._wait_for_message[tpe] + [fut]
        fut.result(10)
