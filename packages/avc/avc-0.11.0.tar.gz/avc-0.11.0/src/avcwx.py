...
class TreeView(ListTreeView):
  ...
  def __init__(self):
    ...
    if self.connection.control_value.get('head', None):
      raise Error('{} widget do not support header.'.format(
        self.widget.__class__.__name__))
    key, row = \
        self.connection.control_value.get('body', None).iteritems().next()
    if type(row) == list and not len(row) == 1:
      raise Error(
        '{} widget do not allow data rows with more than one column.'.format(
          self.widget.__class__.__name__))
  ...
  def read(self):
    ...
    def read_node(node_id, node_path):
      ...
      try:
        body['.'.join(list(map(str, node_path)))] = self.row_types[0](
          self.widget.GetItemText(node_id))
      except:
        if self.row_types[0] == int:
          body['.'.join(list(map(str,node_path)))] = [0]
        elif self.row_types[0] == float:
          body['.'.join(list(map(str,node_path)))] = [0.0]
