...
class ListView(Widget):
  ...
  def read(self):
    ...
    head = list(map(lambda col_num: str(header.text(col_num)),
  ...
  def write(self,value):
    ...
    if type(body[0]) == list:
      for row in body:
        self.widget.addTopLevelItem(qt.QTreeWidgetItem(list(map(str,row))))
    else:
      for row in body:
...
class TreeView(Widget):
      ...
      for col_num in range(self.cols_num):
        try:
          ...
          if self.row_types[col_num] == int:
            ...
            row.append(0.0)
      body['.'.join(map(str,node_path))] = row
  ...
  def add_node(self,parent,last_node,current_depth,data):
    ...
    node = qt.QTreeWidgetItem(list(map(str,data)))
    if current_depth == 1:
