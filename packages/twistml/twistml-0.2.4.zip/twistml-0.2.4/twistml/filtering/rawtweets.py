...
def filter_raw_json(filepath,
    ...
    with open(filepath, 'r') as json_string:
        ...
        for line in json_string:
            ...
            except ValueError:  # indeed partial was not complete
                lines_in_part += 1
            ...
            extracted_data = _extract_metadata(entry, meta_fields,
                                               filter_words, filter_language,
                                               logger)
    ...
    if any(x in tweet['text'] for x in keywords) or keywords == []:
        ...
        for field in fields:
            if field in tweet:
                ...
                if logger is not None:
                    logger.warning(
                        "Field '{}' not present in tweet.".format(field)
                    )
        return d
    else:
        return None
...
def _is_tweet_in_language(tweet, languages):
    r"""Checks if the tweet has one of the language tags. Returns True or
        False."""
    ...
    if 'user' in tweet and 'lang' in tweet['user']:
