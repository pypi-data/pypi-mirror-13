...
from twistml.utility.utility import remap


# create named tuples to hold the results of our evaluations
EvalRegrResult = namedtuple("EvalRegrResult",
                            "mse msestd mape mapestd auc aucstd f1 f1std")
EvalBinClassResult = namedtuple("EvalBinClassResult",
                                "auc aucstd f1 f1std")


def evaluate_regression(X, y, method, cv_params, n_runs, force_dense=False,
                        relative_test_size=0.2, simulate_classification=False,
                        **kwargs):
    ...
    for i in range(n_runs):
        ...
        if(simulate_classification):
            y_test_classes = _get_classes_from_regr_preds(y_test)
            y_class_proba = _get_class_probas_from_regr_preds(y_pred)
            aucscores.append(roc_auc_score(y_test_classes, y_class_proba))
            y_class_pred = _get_classes_from_regr_preds(y_pred)
            f1scores.append(f1_score(y_test_classes, y_class_pred))
    ...
    if(simulate_classification):
        ...
        auc, aucstd, f1, f1std = None, None, None, None
...
def evaluate_binary_classification(X, y, method, cv_params, n_runs,
                                   ...
                                   return_auc=True, return_F1=True, **kwargs):
...
def _get_classes_from_regr_preds(y_pred):
    r"""Return array with 1 where y_pred >= 0.0 and 0 where y_pred < 0"""
    y_sign = np.sign(y_pred)
    y_sign[y_sign == 0] += 1
    return (y_sign + 1) / 2


def _get_class_probas_from_regr_preds(y_pred):
    r"""Approximate classe probabilities from regression predictions.

    For any regression prediction 0 <= pred <= max, the class proba
    will be 0.5 <= proba <= 1.0.
    For any regression prediction 0 >= pred >= min, the class proba will
    be 0.5 >= proba >= 0.0.
    ...
    y_min = np.min(y_pred)
    y_max = np.max(y_pred)
    return np.array([_split_remap(y, y_min, y_max, 0.0) for y in y_pred])


def _split_remap(oldValue, oldMin, oldMax, oldSplit,
                 newMin=0.0, newMax=1.0, newSplit=0.5):

    if(oldValue >= oldSplit):
        return remap(oldValue, oldMin=oldSplit, oldMax=oldMax,
                     newMin=newSplit, newMax=newMax)
    else:
        return remap(oldValue, oldMin=oldMin, oldMax=oldSplit,
                     newMin=newMin, newMax=newSplit)
