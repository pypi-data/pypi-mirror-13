...
class TaskProcessor:
    ...
    def parseargs(self, args):
        ad = dict(self.defconf) # copy default dictionary
        ...
        if self.parse_args:
            ...
            for v in va:
                ...
                if kv[0] in ad:                    
                    ...
                    self.tsclass.oc.log.error('bad argument: {}'.format(args))
...
class TaskSeq():
    ...
    def mkargs(self,s):
        if isinstance(s,six.string_types):
            ...
            # fill defargs
            code = s['command']
            for k in self.tp[code].defconf:
                if not k in s:
                    s[k]=self.tp[code].defconf[k]
    ...
    def initvars(self):
        ...
        self.v['_day'] = today.day
        self.v['_month'] = today.month
        self.v['_year'] = today.year 

        self.v['_dd']="%02d" % today.day
        self.v['_mm']="%02d" % today.month
        self.v['_yy']="%02d" % (today.year % 100 )
    ...
    def help(cls):
        ...
        def helpchapter(chapters,chapter):
            ...
            for c in sorted(chapters[chapter],key=operator.attrgetter('code')):
                msg+=c.code+'\t'+c.help+'\n'
                if c.parse_args and c.defconf:                    
                    msg+='\tkey-arguments:\n'
                    for aname in sorted(c.defconf):
                        msg+='\t'+aname+'="'+c.defconf[aname]+'"\n'
                if c.store_argline:
                    msg+='\targument ({}), default: "{}"\n'.format(c.store_argline,c.defconf[c.store_argline])
...
class TaskStr(GenTaskProcessor):
    help = 'if str argument is set, returns str, else returns str(data)'
...
class TaskName(GenTaskProcessor):
    help = 'set name for this sequence, e.g. "'
 
    store_argline='name'
    parse_args=False

    defconf = {'name': '$_name:noname'}
    def run(self,ts,data,args):
                
        vv = ts.getvars()                
        ts.iname = string.Template(args['name']).substitute(vv)
...
class TaskSort(GenTaskProcessor):
    ...
    defconf = {'num': '0', 'sep': ' ', 'empty': '0','field': ''}
...
#            for defarg in ts.tp[cmd].defconf:
#                if not defarg in e:
#                    e[defarg]= ts.tp[cmd].defconf[defarg]
