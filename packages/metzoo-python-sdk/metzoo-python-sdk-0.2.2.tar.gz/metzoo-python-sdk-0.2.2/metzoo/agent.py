...
from .errors import AgentCreationError, AgentDataError, InvalidData
...
class Agent(Comparable):
    ...
    REQ_INFO = \
            { "send": RequestInfo("POST", "/metrics/data?register=true")
    ...
    def __init__(self, customer_key, name, url="http://api.metzoo.com"):
        ...
        self.__name = name
        self.__url = url
        self.__key = self.__get_agent_key(customer_key, name)
    ...
    def configure(self, config):
        ...
        if type(config) is not dict:
            raise InvalidData(got=type(config), expected=dict)

        if len(config) == 0:
            raise InvalidData(got="an empty dict", expected="a dict with at least one key:value pair")

        units = []
        for metric in config:
            unit = config[metric]
            if type(unit) is not str:
                raise InvalidData(got=type(unit), expected="{} for the unit of {}".format(str, metric))
            units.append({"name": metric, "unit": unit})

        return self.__get_valid_response("set_unit", units, {"Agent-Key": self.key})

    def send(self, metrics):
        """For every key-value pair in the dictionary metrics it will create the metric if it doesn't exists and it will
        send the data for that metric.
        ...
        if type(metrics) is not dict:
            raise InvalidData(got=type(metrics), expected=dict)

        if len(metrics) == 0:
            raise InvalidData(got="an empty dict", expected="a dict with at least one key:value pair")

        data = {"t": calendar.timegm(time.gmtime())}
        for metric in metrics:
            value = metrics[metric]
            if not type(value) in [int, float, long]:
                raise InvalidData(
                        got=type(value),
                        expected="{}, {} or {} for the value of {}".format(int, float, long, metric))
            data[metric] = value

        return self.__get_valid_response("send", data, {"Agent-Key": self.key})
    ...
    def __repr__(self):
        return "<Agent '%s', key: %s>" % (self.name, self.key)

    def __get_agent_key(self, customer_key, name):
        key = None
        try:
            key = self.AGENT_KEYS[name]
        except KeyError:
            key = self.AGENT_KEYS[name] = self.__create_agent_key(customer_key, name)
        return key

    def __create_agent_key(self, customer_key, name):
        status, response = self.__request("create", {"host_name": name, "customer_key": customer_key})
        if not status in [200, 201]:
            raise AgentCreationError(status, response)
        return response
    ...
    def __request(self, info_name, body=None, headers={}):
        ...
        print "\n--> Request:\n  method: {}\n  url: {}\n  body: {}\n  headers: {}".format(
                info.method, self.url + info.route, json.dumps(body), headers)
        response = requests.request(info.method, self.url + info.route, data=json.dumps(body), headers=headers)
        print "<-- Response:\n  status: {}\n  text: {}".format(response.status_code, json.loads(response.text))
