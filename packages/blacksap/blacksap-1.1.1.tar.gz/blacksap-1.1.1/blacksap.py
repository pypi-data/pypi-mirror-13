...
def download_torrent_file(url, destination, filename, maxtries):
    ...
    for x in range(0, maxtries):
        # I have to set the user agent here so that torcache doesn't freak out
        curl_cmd = ['curl', '-s', '--compressed', '-G', '-A', '"Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_2) AppleWebKit/601.3.9 (KHTML, like Gecko) Version/9.0.2 Safari/601.3.9"',
                    short_url, '-o', destination + filename]
        if debug:
            click.echo('Attempting: ' + ' '.join(curl_cmd))
        result = subprocess.call(curl_cmd)
        if result == 0:
            if debug:
                click.echo('Downloaded: ' + short_url + ' in ' + str(x) + ' tries')
            return True, 'Downloaded: ' + short_url
        time.sleep(1)
    return False, 'Download attempts exceeded for torrent: ' + url
...
@click.option('--retry', '-M', default=5, help='Number of times to try and download .torrent file')
...
@click.option('--output', '-o', type=click.Path(exists=True), required=True,
...
def run(**opts):
    ...
    DEFAULTS:
        ...
        ttk = 600
    """
    # TODO - Allow individual URLS to run
    global debug
    cfg = read_cfg()
        ...
        if feed['hash'] == rss_hash and opts['force'] is False:
            ...
            for torrent in rss_entries:
                ...
                if torrent_name not in downloaded_torrents:
                    result = download_torrent_file(torrent_url.pop(), opts['output'], torrent_name, opts['retry'])
                    if result[0]:
                        downloaded_torrents.append(torrent_name)
                    else:
