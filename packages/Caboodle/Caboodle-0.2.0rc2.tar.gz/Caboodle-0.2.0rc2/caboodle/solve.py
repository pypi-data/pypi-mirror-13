...
class _rucaptcha(object):
	...
	def post(self, image, regsense = 0, is_question = 0, is_numeric = 2,
		...
		Args:
			image (str): The file path of the CAPTCHA to be solved
		...
		files = [('file', ('captcha', open(image, 'rb'), 'image/jpeg'))]
		data = {
			'method': 'post',
			'key': self.key,
		}

		r = requests.post('http://rucaptcha.com/in.php', files = files, data = data)
...
class captcha(object):
	...
	def _solve(self, form):
		...
		if self.online or self.ocr:
			...
			self.browser.save_screenshot('/tmp/screenshot.png')
			Image.open('/tmp/screenshot.png').crop((x, y + 18, x + w, y + h)).save('/tmp/captcha.jpg')

			return form['box'].send_keys(self.solver.solve('/tmp/captcha.jpg'))
