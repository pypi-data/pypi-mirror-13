...
import {0} as currentModule
	...
	for sourceFilePath in sourceFilePathList:
		...
		targetSubDirectoryTail =  sourceFilePath [len (sourceRootDirectory) : ]
		...
		if sourceFileNameExtension in sourceFileNameExtensionList:
			...
			targetChunks = targetSubDirectoryTail.split ('/')
			for index in range (len (targetChunks)):
				try:
					...
					pass
			targetSubDirectoryTail = '/'.join (targetChunks)
			targetSubDirectory = '{0}{1}'.format (targetRootDirectory, targetSubDirectoryTail) .rsplit ('/', 1) [0]
