...
def prepare_config(config):
    ...
    if runtime_config['start'] is None:
        runtime_config['start'] = (runtime_config['end'] if runtime_config['end']
                                                         else datetime.datetime.now()) - delta_30_days
    ...
    while runtime_config['seed_players_id'] and len(runtime_config['matches_to_download']) < 50:
        ...
        try:
            # TODO remove when riot makes the name consistent between league and matchlist endpoint
            queue = runtime_config['queue']
            if queue == Queue.RANKED_SOLO_5x5.name:
                queue = Queue.TEAM_BUILDER_DRAFT_RANKED_5x5.name
