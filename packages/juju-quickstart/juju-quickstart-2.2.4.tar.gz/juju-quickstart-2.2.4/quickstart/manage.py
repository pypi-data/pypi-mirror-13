...
def _validate_gui_source(options, parser):
    """Validate the optional Juju GUI branch source.

    Convert the GUI source value to be a tuple (user, branch).
    Exit with an error if the source is not well formed.
    """
    gui_source = options.gui_source
    if gui_source is None:
        return
    error = 'invalid Juju GUI source: {}'.format(gui_source).encode('utf-8')
    try:
        user, branch = [i.strip() for i in gui_source.split('/')]
    except ValueError:
        return parser.error(error)
    if not (user and branch):
        return parser.error(error)
    options.gui_source = (user, branch)


def _validate_platform(platform, options, parser):
    """Validate the platform.

    Populate the options namespace with the following names:
        - info: and object that can be used to retrieve API info on envs;
        - juju_command: the path to the Juju command on the given platform;
        - platform: the current OS platform as defined in the settings module.
    ...
    try:
        ...
        return parser.error(bytes(err))
    options.juju_command = platform_support.get_juju_command(platform)
    options.platform = platform
    options.info = apiinfo.Info(options.juju_command)


def _validate_port(port, parser):
    """Validate the given port if it is not None.

    Exit with an error if the port is not in the TCP range.
    """
    if (port is not None) and (port < 1 or port > 65535):
        return parser.error(b'invalid Juju GUI port: not in range 1-65535')
...
def _start_interactive_session(
        parser, env_type_db, env_db, active_db, env_file):
...
def _retrieve_env_data(parser, env_type_db, env_db, active_db, env_name):
    ...
    try:
        ...
        try:
            return envs.get_env_data(active_db, env_name)
        except ValueError as err:
...
def _setup_env(options, parser):
    ...
    if interactive:
        ...
        env_data = _retrieve_env_data(
            parser, env_type_db, env_db, active_db, env_name)
    # Check for local support, if requested.
    options.env_type = env_data.get('type')
...
def setup():
    ...
    Return the options as a namespace containing the following attributes:
        ...
        - env_type: the provider type of the selected Juju environment, or None
          if the environment type cannot be retrieved at setup time;
        - gui_source: the optional Juju GUI branch source on github, or None;
        - info: an instance of "quickstart.models.apiinfo.Info", used to
          retrieve information about the current environment in use;
        - interactive: whether to start the interactive session;
        - juju_command: the path to the Juju command on the current platform;
        - open_browser: whether the GUI browser must be opened;
        - platform: the host platform;
        - port: the optional Juju GUI port, or None;
        - uncommitted: whether the provided bundle must be deployed using the
          Juju GUI uncommitted state;
    ...
    parser.add_argument(
        '-u', '--uncommitted', action='store_true', dest='uncommitted',
        help='Do not start the bundle deployment. Open the GUI showing\n'
             'an uncommitted bundle state instead, so that the bundle\n'
             'can be finely tuned and tweaked before committing the\n'
             'changes. This option is ignored if no bundle deployment\n'
             'is requested.')
    parser.add_argument(
        ...
        help='The Juju GUI charm URL to deploy in the environment.\n'
             ...
             'e.g. "cs:~juju-gui/trusty/juju-gui-51". This option is\n'
             'ignored if the GUI is already present in the environment')
    parser.add_argument(
        '--gui-port', dest='port', type=int,
        help='The Juju GUI service port. By default, the GUI listens on\n'
             'ports 80 and 443, and insecure connections to port 80 are\n'
             'redirected to port 443. If a port is specified then the\n'
             'GUI service will only accept secure connections to that\n'
             'port. This option is ignored if the GUI is already\n'
    ...
    # Hidden options.
    parser.add_argument(
        # The Juju GUI source branch (in github) to use when building the GUI.
        # It has the form "<user>/<branch>", e.g. "juju/develop".
        # It defaults to the local release included in the Juju GUI charm.
        # This option is present for development purposes only, and it is
        # ignored if the GUI is already present in the environment.
        '--gui-source', dest='gui_source', help=argparse.SUPPRESS)
    ...
    # Validate the platform and add in platform info for convenience.
    _validate_platform(platform, options, parser)
    ...
    _validate_port(options.port, parser)
    _validate_gui_source(options, parser)
...
def run(options):
    ...
    env_info = options.info.get(options.env_name)
    api_address = app.get_api_address(env_info)
    ...
    bootstrap_node_series = app.status(options.env_name, options.juju_command)

    # If the environment was not already bootstrapped, we need to retrieve
    # the API address.
    if api_address is None:
        print('retrieving the Juju API address and credentials')
        env_info = options.info.get(options.env_name)
        api_address = app.get_api_address(env_info)

    # Build the Juju API endpoint based on the Juju version and environment id.
    api_url = jujutools.get_api_url(
        api_address, juju_version, env_info['uuid'])

    print('connecting to {}'.format(api_url))
    env = app.connect(api_url, env_info['user'], env_info['password'])
    ...
    gui_options = jujugui.build_options(options.port, options.gui_source)
    unit_name = app.deploy_gui(
        env, settings.JUJU_GUI_SERVICE_NAME, charm_ref.id(), machine,
        gui_options, service_data, unit_data)

    # Observe the deployment progress, and retrieve information for connecting
    # to the Juju GUI.
    address = app.watch(env, unit_name)
    print('the Juju GUI is ready: retrieving service configuration')
    gui_config = jujugui.normalize_config(
        app.get_service_config(env, settings.JUJU_GUI_SERVICE_NAME))
    env.close()

    # Print out Juju GUI unit and credential information.
    url = utils.build_web_url(
        address, gui_config['port'], gui_config['secure'])
    print('\nJuju GUI URL: {}\nusername: {}\npassword: {}\n'.format(
        url, env_info['user'], env_info['password']))

    # Connect to the GUI server WebSocket API.
    print('connecting to the Juju GUI server')
    gui_api_url = jujutools.get_api_url(
        '{}:{}'.format(address, gui_config['port']), juju_version,
        env_info['uuid'], path_prefix='ws', charm_ref=charm_ref,
        insecure=not gui_config['secure'])

    # We need to connect to an API WebSocket server supporting bundle
    # deployments and automatic auth-token login. The GUI builtin server,
    # listening on the Juju GUI address, exposes a suitable API.
    gui_env = app.connect(gui_api_url, env_info['user'], env_info['password'])
    changes_token = None
    # Handle bundle deployment.
    if options.bundle_source is not None:
        changes_token = app.deploy_bundle(
            gui_env, options.bundle, options.uncommitted, charm_ref)
    # Handle automatic auth-token login.
    auth_token = app.create_auth_token(gui_env)
    gui_env.close()

    # Calculate the complete URL and optionally open the default browser.
    url = jujugui.add_tokens_to_url(url, auth_token, changes_token)
    if options.open_browser:
        print('opening the browser at\n{}'.format(url))
        webbrowser.open(url)
    else:
        print('not opening the browser\n'
              'you can automatically log in into the Juju GUI by visiting\n'
