...
def get_api_address(env_info):
    """Return the API address for the environment described by the given info.

    Return None if the address cannot be found or the environment is not
    bootstrapped.
    """
    if not env_info:
        return None
    # Look for a reachable API address.
    candidates = env_info['state-servers']
    if not candidates:
        logging.warn('cannot retrieve the API address: no addresses found')
        return None
    for candidate in candidates:
        ...
        logging.debug(error)
    logging.warn(
        'cannot retrieve the API address: cannot connect to any of the '
...
def deploy_bundle(env, bundle, uncommitted, charm_ref):
    ...
    if version > 3:
        # The bundle uses a new-style syntax. Check whether the currently used
        # GUI server is capable of handling new-style bundles.
        revision = settings.MINIMUM_REVISIONS_FOR_V4_BUNDLES[charm_ref.series]
        if jujugui.is_promulgated(charm_ref) and charm_ref.revision < revision:
            logging.warn(
                'new style bundle syntax is not supported by currently '
                'deployed Juju GUI charm ({}): trying to deploy the bundle '
                'using old syntax'.format(charm_ref))
            # Pretend the bundle is a legacy one. Note that this might not work
            # depending on whether new style unit placement is used in the
            # bundle contents.
            yaml = bundle.serialize_legacy()
            version = 3
        else:
            yaml = bundle.serialize()
    else:
        # The bundle uses the legacy syntax.
        yaml = bundle.serialize_legacy()
