...
def env_index(app, params):
    ...
    Receives a params namedtuple-like object including the following fields:
        ...
        - active_db: the active environments database;
    ...
    active_view = functools.partial(active_detail, app, params)
    ...
    remaining_active_db = copy.deepcopy(params.active_db)
    for position, env_data in enumerate(environments):
        ...
        if env_data['is-default']:
            ...
            bullet = '\N{CHECK MARK}'
        if remaining_active_db['environments'].pop(env_data['name'], None):
    ...
    environments = list(sorted([
        envs.get_env_data(remaining_active_db, env_name)
        for env_name in remaining_active_db['environments']
    ], key=operator.itemgetter('name')))
    ...
    if environments:
        ...
        for env_data in environments:
            ...
            text = [bullet, ' {}'.format(env_short_description)]
            widgets.append(
                ui.MenuButton(text, ui.thunk(active_view, env_data)))
...
def active_detail(app, params, env_data):
    """Show details on a Juju active/imported environment.

    The environment is not included in the environments.yaml file, but just
    found in the active database.
    ...
    app.set_title(envs.get_env_short_description(env_data))
    widgets = []
    for key, value in apiinfo.get_env_details(env_data):
