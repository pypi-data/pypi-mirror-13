...
class TestGetJujuCommand(unittest.TestCase):
    ...
    def test_getenv_succeeds(self):
        # The Juju path is taken from the JUJU environment variable.
        expected_command = '/custom/juju'
        with mock.patch('os.environ', {'JUJU': expected_command}):
            command = platform_support.get_juju_command(None)
        self.assertEqual(expected_command, command)

    def test_without_env_var(self):
        # The Juju path is declared by quickstart itself.
        command = platform_support.get_juju_command('default')
        self.assertEqual(settings.JUJU_CMD_PATHS['default'], command)
