...
class FakeApiInfo(object):
    """Implement the "quickstart.models.apiinfo.Info" interface for tests."""

    def __init__(self, envs):
        """Initialize the fake object with the given envs."""
        self.envs = collections.OrderedDict((env['name'], env) for env in envs)

    def get(self, env_name):
        """See "quickstart.models.apiinfo.Info.get."""
        return self.envs.get(env_name, {})

    def all(self):
        """See "quickstart.models.apiinfo.Info.all."""
...
def make_active_db():
    """Create and return a active environments database."""
    environments = {
        'ec2-west': {
            'user': 'who',
            'password': 'geronimo',
...
def make_ordered_bundle(bundle):
    """Given a bundle, return a new one with the same data sorted.

    The provided bundle must be an an instance of
    "quickstart.models.bundles.Bundle".
    The resulting bundle can be used to test its corresponding change set in a
    deterministic way.
    """
    data = copy.deepcopy(bundle.data)
    for key, value in data.items():
        if isinstance(value, collections.Mapping):
            data[key] = collections.OrderedDict(sorted(value.items()))
    return bundle.__class__(data, reference=bundle.reference)
