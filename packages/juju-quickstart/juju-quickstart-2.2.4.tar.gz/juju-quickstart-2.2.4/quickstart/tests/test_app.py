...
class TestCheckJujuSupported(ProgramExitTestsMixin, unittest.TestCase):
    ...
    supported_versions = [(1, 22, 1), (1, 22, 2), (1, 42, 47), (2, 0, 0)]
    unsupported_versions = [(1, 22, 0), (1, 17, 42), (1, 0, 0), (0, 20, 47)]
...
class TestGetApiAddress(unittest.TestCase):
    ...
    def test_no_env_info(self):
        # A None API address is returned if no environment info is provided.
        env_info = {}
    ...
    def test_no_api_addresses(self):
        ...
        env_info = {'state-servers': []}
        logs = ['cannot retrieve the API address: no addresses found']
    ...
    def test_api_address_not_listening(self):
        ...
        env_info = {'state-servers': ['localhost:17070', '10.0.3.1:17070']}
        logs = [
            'cannot retrieve the API address: '
...
class TestDeployBundle(ProgramExitTestsMixin, unittest.TestCase):
    ...
    def test_bundle_deployment(self, mock_print):
        ...
        env.deploy_bundle.assert_called_once_with(
            yaml.safe_dump(self.bundle_data), 4, bundle_id=None)
    ...
    def test_legacy_bundle_deployment(self, mock_print):
        # A bundle with legacy v3 bundle syntax is successfully deployed.
        env, uncommitted = mock.Mock(), False
        bundle = bundles.Bundle(self.bundle_data, version=3)
        token = app.deploy_bundle(env, bundle, uncommitted, self.charm)
        self.assertIsNone(token)
    ...
    def test_bundle_deployment_v4_not_supported(self, mock_print):
        # The bundle deployment falls back to version 3 syntax if the Juju GUI
        # unit is not recent enough. In this case there is no guarantee that
        # the deployment succeeds.
        env, uncommitted = mock.Mock(), False
        charm = references.Reference.from_string('cs:trusty/juju-gui-36')
        expected_logs = [
            'new style bundle syntax is not supported by currently deployed '
            'Juju GUI charm (cs:trusty/juju-gui-36): trying to deploy the '
            'bundle using old syntax'
        ]
    ...
    def test_bundle_deployment_with_id(self, mock_print):
        ...
        env.deploy_bundle.assert_called_once_with(
            self.bundle.serialize(), 4, bundle_id='django/single')
