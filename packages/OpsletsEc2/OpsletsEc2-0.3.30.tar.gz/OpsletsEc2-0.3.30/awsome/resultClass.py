class Ec2Inst:
    ...
    def __init__(self, in_id, in_ip, in_name, in_env, in_launch, in_state, in_role, in_pip, in_sgname, in_kpname,
        ...
        :param in_cmd: instance's ssh command
        :return: N/A
        """
        self.id = in_id
        self.ip = in_ip
    ...
    def __str__(self):
        ...
        return ("Instance information -\nID: %s\nPublic IP: %s\nName: %s\nEnvironment: %s\nLaunch Time: %s\nState: %s\n"
                "Role: %s\nPrivate IP: %s\nSecGrp Name: %s\nKeyPair Name: %s\nSSH Command: %s") % (self.id, self.ip,
                self.name, self.env, self.ltime, self.state, self.role, self.pip, self.sgname, self.kpname, self.cmd)

    def set_cmd(self, in_cmd):
        """
        Method to assign the class object's instance variable cmd a new value
        :param in_cmd: instance's ssh command
        :return: none
        """
        self.cmd = in_cmd
...
class ElbNode:
    ...
    """
    def __init__(self, nd_num, nd_date, nd_name, nd_vpcid, nd_zone, nd_scheme, nd_inst, nd_sgname):
        """

        :param nd_num: node's list number
    ...
    def __str__(self):
        ...
        return ("Node information -\nNumber: %s\nDate Created: %s\nName: %s\nVPC ID: %s\nAvailability Zone: %s\n"
                "Scheme: %s\nInstances:%s\nSecGrp Name: %s") % (self.num, self.date, self.name, self.vpcid, self.zone,
                self.scheme, self.inst, self.sgname)
