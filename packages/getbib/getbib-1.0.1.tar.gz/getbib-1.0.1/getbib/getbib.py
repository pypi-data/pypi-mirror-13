...
def get_westlaw_bibliography(driver, author_name):
    ...
    if "Signon" in driver.title:
        ...
        elem.send_keys("samuel.alexander@ufl.edu")
        elem = driver.find_element_by_name("Password")
        elem.send_keys("Ian29yoatleast8")
    ...
    try:
        ...
        print("Timed out. Please check connection and try again.")
        return
    except NoSuchElementException:
    ...
    try:
        ...
        print("Timed out. Check connection, or try another search.")
    ...
    for x in range(1, max_number_of_entries + 1):
        ...
        print("%i.: %s, %s, %s %s" % (x, current_title, current_date, current_refs, "reference" if int(current_refs) == 1 else "references"))
        p.add_run(str(x)+". "+current_title+", "+current_date+", "+current_refs+" "+("reference" if int(current_refs) == 1 else "references")+"\n")

        # update the dictionary
        bibliography_list.append( (current_title, current_date, current_refs) )
...
def main():
    ...
    while True:
        ...
        if not re.match('^[A-Za-z ]+$', author_name):
            ...
            get_westlaw_bibliography(driver, author_name)
