...
class GuessRDFFormatTC(TestCase):
    ...
    def test_base(self):
        yield self.assertEqual, guess_rdf_format('file.xml'), 'xml'
        yield self.assertEqual, guess_rdf_format('file.rdf'), 'xml'
        yield self.assertEqual, guess_rdf_format('file.n3'), 'n3'

    def test_url(self):
        yield self.assertEqual, guess_rdf_format('http://url.com/rdf/file.xml'), 'xml'
        yield self.assertEqual, guess_rdf_format('http://url.com/rdf/file.rdf'), 'xml'
        yield self.assertEqual, guess_rdf_format('http://url.com/rdf/file.n3'), 'n3'

    def test_error(self):
        yield self.assertRaises, ValueError, guess_rdf_format, 'file.unknown'
        yield self.assertRaises, ValueError, guess_rdf_format, 'http://url.com/rdf/file'
        yield self.assertRaises, ValueError, guess_rdf_format, 'http://url.com/file/rdf'
...
class RDFGraphTCMixIn(object):
    ...
    def test_load_dump_roundtrip(self):
        for rdf_format in ('xml',):  # 'n3', 'nt'): XXX other formats fail, but sounds due to the
            ...
            yield self.assertRoundTrip, rdf_format
...
class AddEntitiesToRDFGraphTC(CubicWebTC):
