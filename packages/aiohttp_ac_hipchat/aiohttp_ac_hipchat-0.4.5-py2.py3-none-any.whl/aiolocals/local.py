...
class Context:
    ...
    def __repr__(self):
        return "<aiolocals.Context ident=%s locals=%r parent=%r>" % (self.ident, self.locals, self.parent)


def _spawn_context(child_ident, parent_ident=None):
    parent = None if not parent_ident else _contexts.get(parent_ident)
    ctx = Context(child_ident, locals=parent.locals if parent else [], parent=parent)
    ctx.__enter__()
    return ctx
...
def wrap_async(coro, **kwargs):
    """
    Wraps a coroutine with a Task that runs in the background.  It ensures any context information is transferred
    to the new task
    ...
    if parent_ctx:
        child_ident = id(t)
        child_ctx = _spawn_context(child_ident=child_ident, parent_ident=parent_ident)
...
class Local(object):
    ...
    def __release_local__(self, ident=None):
        ...
        if self.noisy:
            log.debug("aiolocals.Local: Cleaning local ident %s, storage=%s", ident,  pprint.pformat(self.__storage__))
        self.__storage__.pop(ident, None)
        if self.noisy:
            log.debug("aiolocals.Local: Released local ident %s, storage=%s", ident, pprint.pformat(self.__storage__))
...
__all__ = ["Local", "wrap_async", "wrap_gather", "Context", "preserve_context_in_threads"]
