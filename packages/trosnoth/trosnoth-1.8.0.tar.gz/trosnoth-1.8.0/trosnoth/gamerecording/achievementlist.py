...
class OnceEverPerPlayer(PlayerAchievement):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.player is not None and not self.player.bot:
            unlocked, progress = self.readExistingData()
            self.previouslyUnlocked = unlocked
            self.progress = progress

    def readExistingData(self):
        '''
        Returns (unlocked, progress) based on saved data.
        '''
        user = self.player.user
        if user is None:
            return False, self.getDefaultProgress()

        userAchievements = user.achievements.get(self.idstring, {})
        unlocked = userAchievements.get('unlocked', False)
        progress = userAchievements.get('progress', self.getDefaultProgress())
        return unlocked, progress

    def getDefaultProgress(self):
        '''
        Can be overridden by, e.g. checklist achievements.
        '''
        return 0
...
class PersistedChecklistAchievement(ChecklistAchievement, OnceEverPerPlayer):
    def readExistingData(self):
        previous, progress = super(
            PersistedChecklistAchievement, self).readExistingData()
        return previous, set(progress)
