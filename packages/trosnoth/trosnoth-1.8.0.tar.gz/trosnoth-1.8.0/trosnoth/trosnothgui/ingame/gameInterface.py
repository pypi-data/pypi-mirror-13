...
class GameInterface(framework.CompoundElement, ConcreteAgent):
    ...
    def __init__(
        ...
        if replay:
            ...
            self.closeReplayButton = TextButton(
                app, ScaledLocation(0, 768, 'bottomleft'),
                'End Replay', self.app.fonts.menuFont,
                app.theme.colours.inGameButtonColour,
                app.theme.colours.white)
            self.closeReplayButton.onClick.addListener(
                lambda sender: self.onDisconnectRequest.execute())
        else:
            self.joinController = JoinGameController(self.app, self, self.game)
            self.closeReplayButton = framework.CompoundElement(self.app)
        ...
        self.menuHotkey = hotkey.MappingHotkey(
            self.app, 'menu', mapping=self.keyMapping)
        ...
        self.elements = [
            self.gameViewer, self.gameInfoDisplay, self.closeReplayButton,
    ...
    def joined(self, player):
        ...
        self.elements = [self.gameViewer, pi, self.gameInfoDisplay,
                         self.menuHotkey, self.hotkeys,
    ...
    def showMenu(self):
        if self.runningPlayerInterface is not None:
            if self.runningPlayerInterface.player is not None:
                ...
                return
        self.elements = [
            self.gameViewer, self.closeReplayButton, self.timingInfo]
        self.menuShowing = True
    ...
    def hideMenu(self):
        if self.runningPlayerInterface is not None:
            self.elements = [
                ...
                self.gameInfoDisplay, self.closeReplayButton,
                self.menuHotkey, self.hotkeys, self.detailsInterface,
                self.winnerMsg, self.timingInfo]
        else:
            self.elements = [
                self.gameViewer, self.observerInterface,
                self.gameInfoDisplay, self.closeReplayButton,
                self.menuHotkey, self.hotkeys, self.detailsInterface,
                self.winnerMsg, self.timingInfo]
        if self.vcInterface is not None:
