...
class Channel(object):
    ...
    def emit(self, event, data=None, sender=None, target=None, serialize=True):
        if serialize:
            data = json.dumps(data)
        event_data = '%s;%s;%s;%s' % (event, sender or "", target or "", data)
...
class EventEmitter(object):
    ...
    def emit(self, channel, event, data=None, sender=None, target=None, serialize=True):
        self.channel(channel).emit(event, data, sender, target, serialize)
...
def emit(channel, event, data=None, sender=None, target=None, serialize=True):
    _emitter.emit(channel, event, data, sender, target, serialize)