...
class BaseMixin(PresenceMixin, ChannelMixin, AuthentifyMixin, QueueMixin):
    ...
    def on_event_message(self, msg):
        event, sender, target, data = msg.split(';', 3)
        if (target and target != self.user_id) or (sender and \
          self.application.settings['ignore_own_messages'] and sender == str(self.user_id)):
            logger.debug('Ignored event: %s' % event)
            return
        logger.debug('Event received: %s' % event)
        self.handle_event(event, data, sender or None)
    ...
    def handle_event(self, event, data, sender):
...
class WebSocketHandler(BaseMixin, WebSocketMixin, tornado.websocket.WebSocketHandler):
    ...
    def handle_event(self, event, data, sender):
        try:
            self.write_message("event:%s;%s;%s" % (event, sender, data))
