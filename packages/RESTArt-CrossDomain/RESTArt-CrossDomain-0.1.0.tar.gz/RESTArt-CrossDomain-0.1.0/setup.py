...
def get_info(name):
    ...
    with open(os.path.join(basedir, 'restart_crossdomain/__init__.py')) as f:
        locals = {}
        exec(f.read(), locals)
        return locals[name]
...
setup(
    ...
    keywords='Cross-domain access, CORS, RESTArt, Python',
    description=get_info('__doc__'),
    license=get_info('__license__'),
    long_description=get_info('__doc__'),
    packages=find_packages(exclude=['tests']),
    url='https://github.com/RussellLuo/restart-crossdomain',
    install_requires=[],
)
