...
class _MobileKeywords(KeywordGroup):
    ...
    def Mobile_Click_WebView_TextElement(self, text, target_num=1):
        ...
        if self._is_andriod():
            ...
            try:
                 self.wait_until_page_contains_element(textelement, timeout=10)
            except Exception:
                 textelement += " Link" 
            #finally:
                 #print textelement        
    ...
    def Mobile_Get_Elements_Num(self, locator):
        ...
        try:
            ...
            #print "[debug] Mobile_Get_Elements_Num: try step2 elements_list: %d!" % len(elements_list)
            return len(elements_list)
        except Exception:
            #print "[debug] Mobile_Get_Elements_Num: except path!"
            return 0        
...
# Private 
    ...
    def _click_webview_element_autoswipe(self, target_locator, target_num=1):
        ...
        if self._is_andriod():
            ...
            #print "[debug] autoswipe action data:"
            #print "[debug] swipe_times: %d, start_loc_x:%d, start_loc_y:%d, swipe_distance:%d" % (swipe_times, start_loc_x, start_loc_y, swipe_distance)
            ...
            while (swipe_times > 0 and self._is_element_present_in_webview(webview_element, target_element)==False):                 
                ...
                #print '[debug] Element not present in Webview now, and do one swipe with swipe_distance: %d' % swipe_distance
                target_element = self._get_selected_element(target_locator, target_num)
                swipe_times -=1
                sleep(2)
                #print '[debug] Now swipe_times left: %d' % swipe_times
            if self._is_element_present_in_webview(webview_element, target_element):
                ...
                print '[debug] Faile to swipe the element %s present in Webview! ' % target_locator
    ...
    def _ios_auto_swipe(self, target_locator, target_num):
        ...
        # 向下滑
            ...
            while elem_y < 0:
                ...
                elem_y = elem.location.values()[0]
        elem.click()
        print 'Click WebView Element "%s"' % target_locator
    ...
    def _webview_element_autoswipe_get_swipe_actiondata(self, webview_element, target_element):
        ...
        print '[debug] webview_location: (%d, %d)'%(webview_location[1], webview_y)
        print '[debug] webview_size: (%d, %d)' % (webview_size[0], webview_height)
        print '[debug] des_loc: (%d, %d)' % (des_loc_x, des_loc_y)
        print '[debug] des_size: (%d, %d)' % (des_size_width, des_size_height)
    ...
    def _is_element_present_in_webview(self, webview_element, target_element):
        ...
        print '[debug] y of webview_top & webview_bottom: (%d, %d)'%(webview_top, webview_bottom)
        #get element location+size
        ...
        print '[debug] y of target_element_top & target_element_bottom: (%d, %d)'%(target_element_top, target_element_bottom)
        ...
        print '[debug] The Target element is in webview controll: %d' % is_inwebview_flag
    ...
    def _get_selected_element(self, target_locator, target_num=1):
        ...
        """
        self.wait_until_page_contains_element(target_locator, 10)
        ...
        if index < len(elements_ls):
            ...
            self.capture_page_screenshot() 
            raise AssertionError("The target_num is '%d', which is out of the length of elements_ls searched by '%s'." % (int(target_num), target_locator))
