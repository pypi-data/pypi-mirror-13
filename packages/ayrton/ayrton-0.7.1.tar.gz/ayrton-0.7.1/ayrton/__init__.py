...
logging.basicConfig(filename='ayrton.%d.log' % os.getpid (), level=logging.DEBUG2, format=log_format, datefmt=date_format)
    ...
    def local_tracer (self, frame, event, arg):
        if event=='line':
            ...
            if self.params.trace_all or file_name==self.file_name:
                short_file_name= file_name[len (file_name)-32:]
                ...
                if self.params.trace_all:
                    print ("+ [%32s:%-6d] %s" % (short_file_name, lineno, line), file=sys.stderr)
