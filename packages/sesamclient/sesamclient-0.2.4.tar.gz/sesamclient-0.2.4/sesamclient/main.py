...
def get_dataset_entities_cmd(connection, args):
    ...
    entities_stream = dataset.get_entities_as_stream(since=args.since, limit=args.limit, start=args.start, history=args.history)
...
description:
    ...
    def add_subparser(*args, add_default_arguments=True, help=None, description=None, **kwargs):
...
If this is false, only the latest version of the entities will be returned""")
    ...
    start_pump_parser.add_argument(
        ...
        default=0,
        type=int,
        help="If this is not zero (the default), the command will wait for the pump to finished running. "
