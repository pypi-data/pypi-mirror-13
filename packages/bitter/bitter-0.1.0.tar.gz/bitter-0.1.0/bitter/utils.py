...
class Worker(object):
    ...
    def throttled(self):
        ...
        if delta > 0:
            print("Worker %s throttled for %s seconds" % (self.name, delta))
            return True
        return False
...
def get_users(wq, ulist, by_name=False, max_users=100):
    ...
    print('Getting users by {}: {}'.format(t, ulist))
    ilist = iter(ulist)
    while True:
        ...
        for user in resp:
            uid = user['id']
            del user['status'] 
            ts = time.strftime('%Y-%m-%d %H:%M:%S', time.strptime(user['created_at'],'%a %b %d %H:%M:%S +0000 %Y'))
