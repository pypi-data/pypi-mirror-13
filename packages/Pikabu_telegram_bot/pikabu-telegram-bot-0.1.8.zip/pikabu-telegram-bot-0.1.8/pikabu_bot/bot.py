...
class Bot(object):
    ...
    def _handleUpdate(self, update):
        ...
        if text is not None and text.startswith('/'):
            ...
            if text == '/start':
                ...
                self._handleSearchCommand(chat_id, text)
            else:
    ...
    def _handleStartCommand(self, chat_id, text):
        hello_text = u'*Список команд бота:*\n \
                       ...
                       /search <текст> - случайный найденный пост\n \
    ...
    def _handleTopCommand(self, chat_id, text):
        args = text.split(' ')
        posts_count = 3 if len(args) != 2 or args[1].isdigit() == False else int(args[1])
        if posts_count >= 10:
            posts_count = 10
        elif posts_count <= 1:
            posts_count = 1
        ...
        if posts is None:
            ...
            l = len(posts) if len(posts) < posts_count else posts_count
            for i in range(l):
    ...
    def _handleTagCommand(self, chat_id, text):
        ...
        if len(splits) <= 1:
            ...
            if posts is not None:
                ...
                self.bot.sendMessage(chat_id=chat_id,
                                     text=u'Нет постов с таким тегом %s' % telegram.Emoji.CONFUSED_FACE.decode('utf-8'))

    def _handleSearchCommand(self, chat_id, text):
        splits = text.split(' ')
        if len(splits) <= 1:
            self.bot.sendMessage(chat_id=chat_id,
                                 text=u'Необходимо указать запрос поиска.\nПример: /search киски')
        else:
            search_query = ('%20'.join(splits[1:])).decode('utf-8')
            self.bot.sendChatAction(chat_id, telegram.ChatAction.TYPING)
            posts = self._get_cache_enh(u'/search.php?q=%s' % search_query, 0, 4,
                                        24 * 60 * 60)  # Берем посты с 4 страниц с временем кэша 24 часа
            if posts is not None:
                self.bot.sendMessage(chat_id=chat_id, text=posts[random.randint(0, len(posts) - 1)])
            else:
                self.bot.sendMessage(chat_id=chat_id,
                                     text=u'Нет постов по такому запросу %s' % telegram.Emoji.CONFUSED_FACE.decode(
                                         'utf-8'))
