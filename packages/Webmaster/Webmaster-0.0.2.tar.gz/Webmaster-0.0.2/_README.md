...
A Flask based framework that adds structure your views and templates by mapping them
...
Inside of the `layout.html`, Flask-Pilot requires the tag **`{% include __template__ %}`** . This tag actually includes the view template inside of the layout. So you no longer need to add *{% extends 'layout.html' %}* inside of your templates files. It does it for you automatically. 
...
This is the page that will be included inside of `layout.html` when you add  `{% include __template__ %}	`
...
As you can see, inside of index() you notice it **`return self.render_()`** . This method glues everything together for you and render the page effortlessy. 

But you don't have to return self.render_(), you can  return and iterable object (i.e string) if that's what you want to achieve.
...
#### * MarcoPolo.render(data={}, template_=None, layout_=None)

This is the meat of MarcoPolo, 

* **data** : A dict of data to be passed to the template

* **template_** : The file path of the template to display. By default the template will map the view method. 
