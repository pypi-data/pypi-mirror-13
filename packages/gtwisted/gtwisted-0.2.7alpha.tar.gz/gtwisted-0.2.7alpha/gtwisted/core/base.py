...
class BasePortListener(Greenlet):
    """基础的端口监听器\n
    """
    def __init__(self,port,factory,server_cls,port_type,*args,**kw):
        """端口监听器\n
    ...
    def _run(self):
        """启动监听器\n
        """
        self.server = self.server_cls(self.getHost(),self.factory,backlog=100000,*self.args,**self.kw)
...
class TCPPortListener(BasePortListener):
    ...
    def __init__(self,port,factory,*args,**kw):
        """TCP服务端（端口监听器）实例化。继承于BasePortListener，这里的server_cls采用的是StreamServer\n
        (可了解gevent的StreamServer的实例)\n
        """
        BasePortListener.__init__(self, port, factory, StreamServer,port_type="TCP",*args,**kw)
...
class UDPPortListener(BasePortListener):
    ...
    def __init__(self,port,factory,*args,**kw):
        ...
        """
        BasePortListener.__init__(self, port, factory, DatagramServer,port_type="UDP",*args,**kw)
...
class WSGIPortListener(BasePortListener):
    ...
    def __init__(self,port,factory,*args,**kw):
        ...
        """
        BasePortListener.__init__(self, port, factory, WSGIServer,port_type="WSGI",*args,**kw)
...
class BaseConnector:
    ...
    def __init__(self,address,factory,timeout=30):
        """
    ...
    def connect(self):
        """开始连接\n
        """
        self.factory.startedConnecting(self,timeout=self.timeout)

    def sendall(self,data):
        """
        """
        self.factory._protocol.transport.sendall(data)
