...
class GeventReactor:
    ...
    def listenTCP(self,port,factory,*args,**kw):
        """监听一个端口.\n
        @param port: int 监听的端口.\n
        @param factory: Fctory 处理客户端消息的工厂.\n
        """
        tcp_listener = TCPPortListener(port, factory,*args,**kw)
        tcp_listener.start()
        return tcp_listener
    
    def listenWSGI(self,port,factory,*args,**kw):
        """监听一个WSGI端口\n
        """
        wsgi_listener = WSGIPortListener(port, factory,*args,**kw)
        wsgi_listener.start()
    ...
    def connectTCP(self,host,port,factory,timeout=30):
        ...
        """
        connector = BaseConnector((host,port), factory,timeout=timeout)
        connector.connect()
        return connector
