  ...
  if (static_cast<ListOfGroups*>(getParentSBMLObject())->size() < 2)
  {
    ...
    for (int n = numErrs-1; n >= 0; n--)
    {
      if (log->getError(n)->getErrorId() == UnknownPackageAttribute)
      {
        ...
        log->logPackageError("groups", GroupsGroupAllowedAttributes, pkgVersion, level,
          version, details);
      }
      else if (log->getError(n)->getErrorId() == UnknownCoreAttribute)
      {
        ...
        log->logPackageError("groups", GroupsModelLOGroupsAllowedCoreAttributes, pkgVersion, level,
          version, details);
      }
    }
  }
