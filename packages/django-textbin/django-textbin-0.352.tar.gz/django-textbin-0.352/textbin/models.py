...
ID_LENGTH = settings.TEXTBIN['ID_LENGTH']
...
class MediaType(object):
    ...
    def autodetect(cls, url):
        ...
        youtube = cls._get_youtube_embed(url)
        if youtube:
            return cls.CHOICES[cls.YT_VIDEO-1][0], youtube
...
def valid_url_media(url):
    try:
        ...
        raise ValidationError(ugettext_lazy('URL must point to one of this: %s')
                              % ', '.join([_(ch[1]) for ch in MediaType.CHOICES]))
...
class MediaManager(models.Manager):
    def create_media(self, text, url, type=None):
        ...
        # not specified)
        if not type:
            type, url = MediaType.autodetect(url)
...
# This is used to place text in locale-file
_('Image')
_('Video')
