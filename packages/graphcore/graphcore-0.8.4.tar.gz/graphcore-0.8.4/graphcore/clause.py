...
class Clause(object):
    ...
    def __init__(self, key, value):
        self.lhs, self.rhs, self.relation, self.alias = self._parse_clause(
            key, value
        )
    ...
    def _parse_clause(self, lhs, rhs):
        if ' as ' in str(lhs).lower():
            as_position = str(lhs).lower().find(' as ')
            alias = str(lhs)[as_position + 4:]
            lhs = str(lhs)[:as_position]
        else:
            alias = None

        if str(lhs)[-1] == '?':
            return Path(lhs[:-1]), OutVar(), None, alias
        ...
        if len(lhs) >= 2:
            if str(lhs)[-2:] == '==':
                return Path(lhs[:-2]), TempVar(), Relation('==', rhs), alias
            elif str(lhs)[-2:] == '!=':
                return Path(lhs[:-2]), TempVar(), Relation('!=', rhs), alias
            elif str(lhs)[-2:] == '<=':
                return Path(lhs[:-2]), TempVar(), Relation('<=', rhs), alias
        ...
        if str(lhs)[-1] == '<':
            return Path(lhs[:-1]), TempVar(), Relation('<', rhs), alias
        elif str(lhs)[-1] == '>':
            return Path(lhs[:-1]), TempVar(), Relation('>', rhs), alias
        else:
            return Path(lhs), rhs, None, alias
