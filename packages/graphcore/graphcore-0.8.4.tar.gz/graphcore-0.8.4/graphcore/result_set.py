...
def mapper(result_apply_rule, data, fn, inputs, outputs, cardinality, scope):
    from plotwatt.pwcloud import pwcloud
    import tqdm

    chunk_size = len(data) / 8

    print 'running', fn, len(data), chunk_size

    return tqdm.tqdm(pwcloud.chunk_imap(lambda result: result_apply_rule(
...
def result_set_apply_rule(data, fn, inputs, outputs, cardinality,
    ...
    if len(outputs[0]) == 1 and len(data) > 2000:
        ret = mapper(
            result_apply_rule, data, fn, inputs, outputs, cardinality, scope
        )
