...
# ############################################################################
# [2015-12-22 11:30:36.726000] THIS FILE IS AUTOGENERATED - DO NOT EDIT!
# ############################################################################
import types
import pandas as pd
import numpy as np
...
class Proxy(ProxyBase):
    ...
    _CMD_SERVO_DETACH = 0x700
    _CMD_SERVO_ATTACH = 0x701
    _CMD_MAGNET_ENGAGED = 0x702
    _CMD_LIGHT_OVERRIDE = 0x703
    _CMD_LIGHT_ENABLED = 0x704
    _CMD_LOOP = 0x705
    _CMD_ON_CONFIG_LIGHT_PIN_CHANGED = 0x706
    _CMD_ON_STATE_MAGNET_ENGAGED_CHANGED = 0x707
    _CMD_ON_STATE_LIGHT_ENABLED_CHANGED = 0x708
    _CMD__MAGNET_ENGAGE = 0x709
    _CMD__MAGNET_DISENGAGE = 0x70a
    _CMD__LIGHT_ENABLE = 0x70b
    _CMD__LIGHT_DISABLE = 0x70c
    MAX_COMMAND_CODE = 1804
    ...
    def servo_detach(self):
        command = np.dtype('uint16').type(self._CMD_SERVO_DETACH)
        payload_size = 0
        payload_data = ''
    ...
    def servo_attach(self, servo_pin):
        command = np.dtype('uint16').type(self._CMD_SERVO_ATTACH)
        ARG_STRUCT_SIZE = 1
        array_data = ''
        payload_size = ARG_STRUCT_SIZE + len(array_data)
        struct_data = np.array([(servo_pin, )],
                               dtype=[('servo_pin', 'uint8'), ])
        payload_data = struct_data.tostring() + array_data
    ...
    def light_override(self):
        command = np.dtype('uint16').type(self._CMD_LIGHT_OVERRIDE)
        payload_size = 0
        payload_data = ''
    ...
    def light_enabled(self):
        command = np.dtype('uint16').type(self._CMD_LIGHT_ENABLED)
        payload_size = 0
        payload_data = ''
    ...
    def on_config_light_pin_changed(self, new_value):
        command = np.dtype('uint16').type(self._CMD_ON_CONFIG_LIGHT_PIN_CHANGED)
        ARG_STRUCT_SIZE = 4
        array_data = ''
        payload_size = ARG_STRUCT_SIZE + len(array_data)
        struct_data = np.array([(new_value, )],
    ...
    def on_state_magnet_engaged_changed(self, new_value):
    ...
    def on_state_light_enabled_changed(self, new_value):
    ...
    def _light_enable(self):
        command = np.dtype('uint16').type(self._CMD__LIGHT_ENABLE)
        payload_size = 0
        payload_data = ''
    ...
    def _light_disable(self):
        command = np.dtype('uint16').type(self._CMD__LIGHT_DISABLE)
        payload_size = 0
        payload_data = ''
