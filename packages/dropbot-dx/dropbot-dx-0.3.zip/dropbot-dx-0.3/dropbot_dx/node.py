...
# ############################################################################
# [2016-01-11 17:17:46.663000] THIS FILE IS AUTOGENERATED - DO NOT EDIT!
# ############################################################################
import types
import pandas as pd
import numpy as np
...
class Proxy(ProxyBase):
    ...
    _CMD_MAGNET_ENGAGED = 0x700
    _CMD_LOOP = 0x701
    _CMD_ON_STATE_MAGNET_ENGAGED_CHANGED = 0x702
    _CMD_ON_STATE_LIGHT_ENABLED_CHANGED = 0x703
    _CMD_ON_CONFIG_LIGHT_INTENSITY_CHANGED = 0x704
    _CMD__MAGNET_ENGAGE = 0x705
    _CMD__MAGNET_DISENGAGE = 0x706
    _CMD__SET_LIGHT_INTENSITY = 0x707
    _CMD__DISABLE_LIGHT = 0x708
    MAX_COMMAND_CODE = 1800
    ...
    def on_config_light_intensity_changed(self, new_value):
        command = np.dtype('uint16').type(self._CMD_ON_CONFIG_LIGHT_INTENSITY_CHANGED)
        ARG_STRUCT_SIZE = 4
        array_data = ''
        payload_size = ARG_STRUCT_SIZE + len(array_data)
        struct_data = np.array([(new_value, )],
                               dtype=[('new_value', 'float32'), ])
        payload_data = struct_data.tostring() + array_data
    ...
    def _set_light_intensity(self, value):
        command = np.dtype('uint16').type(self._CMD__SET_LIGHT_INTENSITY)
        ARG_STRUCT_SIZE = 4
        array_data = ''
        payload_size = ARG_STRUCT_SIZE + len(array_data)
        struct_data = np.array([(value, )],
    ...
    def _disable_light(self):
        command = np.dtype('uint16').type(self._CMD__DISABLE_LIGHT)
        payload_size = 0
        payload_data = ''
