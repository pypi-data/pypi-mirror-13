...
class SupplementaryAlignment(namedtuple("SupplementaryAlignment",
	...
	def query_start(self):
		return self.cigar.soft_clipping_left

	@property
	def query_end(self):
		return self.cigar.soft_clipping_left + self.cigar.query_length(count_clipped=None)
...
class AlignedRead:
	...
	def _extract_supplementary_alignments(self, aligned_segment, reference_name):
		...
		alignments = [SupplementaryAlignment(reference_name, aligned_segment.pos,
			'+-'[int(aligned_segment.is_reverse)], Cigar(aligned_segment.cigar),
			aligned_segment.mapq, tags['NM'])]
		if 'SA' in tags:
