...
var BarChart = function(processed_data, div, size, labels, scales, margin) {
  ...
  GridChart.call(self, div, size, labels, scales, margin, 350);
...
BarChart.prototype.add_bars = function() {
  ...
  if (self.type === "simple") {
    ...
    var loose_bars = self.processed_data.map(function(d) {
    ...
    loose_bars = [].concat.apply([], loose_bars).filter(function(d) {
      return d !== undefined;
    });
    ...
    //Work out the labels for the legend

    self.legend_labels = [];

    loose_bars.forEach(function(d) {
    ...
    add_legend(self.svg, self.margin.left + self.width, self.margin.top, self.legend_labels.map(function(v, i) {
      return {
        "label": v,
        "colour": color(v),
        "class": "d_" + i
...
BarChart.prototype.change_layout = function(direction) {
  ...
  if (direction === "to_side") {
    ...
    self.chart_area.selectAll(".x_axis").transition().duration(1000).style("opacity", 0);
  } else {
...
var uni_format = function(d){
  var return_val;
  if (d > 1000000) {
    return_val = d3.format(".3r")(d/1000000) + "m";
  }
  else if (d > 1000) {
    return_val = d3.format(".3r")(d/1000) + "k";
  }
  else if (d > 10) {
...
var TimeSeries = function(processed_data, div, size, labels, scales, tooltip_function) {
  ...
  //Some customisations
  self.y_axis.ticks(4).tickFormat(format_millions).tickSize(0);
  ...
  self.tip = d3.tip()
    ...
    .html(function(d) {
      var formatter;
      if (d.variable === "absolute") {
        formatter = format_millions_2d;
      } else if (d.variable === "share") {
        formatter = d3.format("0%");
      } else {
        formatter = d3.format(".3n");
      }

      return quarter_year(d.time) + "<br>" + d.group + "<br>" + formatter(d.value);
    });
...
TimeSeries.prototype.flip_variable = function(to_variable, duration) {
  ...
  if (to_variable === "absolute") {
    ...
    self.y_axis.tickFormat(format_millions).ticks(6);
...
TimeSeries.prototype.redraw_timeseries = function(title) {
  ...
  //Redraw the chart
  self = self.add_svg().add_grid().add_timeseries().add_legend().add_title(self.title).flip_variable(current_y_axis, 0);
