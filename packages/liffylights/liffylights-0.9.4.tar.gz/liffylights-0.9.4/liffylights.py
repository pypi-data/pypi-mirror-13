...
class LiffyLights():
    ...
    def __init__(self, device_callback, power_callback, color_callback,
        ...
        # if no address given, use 0.0.0.0
        listener_addr = "0.0.0.0" if server_addr is None else server_addr
        ...
        if broadcast_addr is None:
            if server_addr is None:
                server_addr = socket.getfqdn()

            # ensure address is ip address
            server_addr = socket.gethostbyname(server_addr)

            # assume /24 network
            addr = ipaddress.ip_interface(server_addr + "/24")
    ...
    def _packet_ack(self, packet, sequence):
        ...
        if packet["sequence"] == sequence:
            if packet["payloadtype"] == PayloadType.GET:
                # don't remove broadcast packets on ack, want
                # them to continue to be resent until timeout
                # to improve chances of bulb detection
                return True
        ...
        while True:
            ...
            try:
                ...
                if protocol == 1024:
                    ...
                    if payloadtype == PayloadType.ACKNOWLEDGEMENT:
