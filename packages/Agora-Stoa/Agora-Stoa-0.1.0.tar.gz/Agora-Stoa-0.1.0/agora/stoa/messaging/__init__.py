...
rk_pattern = EXCHANGE_CONFIG['rk_pattern']
...
log.info("""Broker setup:
                ...
                - exchange: {}
                - queue: {}
                - rk_pattern: {}""".format(BROKER_CONFIG['host'],
                                           BROKER_CONFIG['port'],
                                           exchange, queue, rk_pattern))
...
def __setup_queues():
    try:
        ...
        channel.queue_bind(exchange=exchange, queue=queue, routing_key=rk_pattern)
        log.info('Binding to "{}"...'.format(rk_pattern))
