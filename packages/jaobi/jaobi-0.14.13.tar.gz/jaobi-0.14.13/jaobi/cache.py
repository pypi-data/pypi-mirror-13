...
class _SizedConsumptionCache:
    ...
    def add(self, obj):
        ...
        # this list to keep track of consumers so i can
        # generate_similar_consumers to them
        consumer = yield obj.consumer
        if consumer.id is None:
            yield consumer.save()

        self.similar_consumers_queue.append(consumer)
        ...
        if len(self.cache) >= self.cache_size and not self.lock:
            try:
                ...
                for consuer in self.similar_consumers_queue:
