...
class Display(object):
    def __init__(self, stat, client):
        self.columns = [
            ('hostname (#R&[dis]#G&connected#d&)', "%s", .18, "L"),
            ('progname', "%s", .20, "R"),
            ('nproc', "%d", .08, "R"),
            ('conn', "%d", .08, "R"),
            ('ckout', "%d", .08, "R"),
    ...
    def _render(self):
        ...
        if self.client.servicename:
            service_str = "[Servicename: %s:%s | %s:%s]" % (
                self.client.servicename,
                self.client.nodename,
        ...
        for hostprog in hostprogs:
            ...
            rows.append((
                ...
                hostprog.checkouts_per_second(now)
            ))
