...
class Validation(object):
    ...
    @property
    def log(self):
        """Logger that validations use to raise validation messages."""
        validator_logger = logging.getLogger('molvs.validate.Validator')
        return logging.LoggerAdapter(validator_logger, {'validation': type(self).__name__})

    def __call__(self, mol):
        try:
            log.debug('Running %s', type(self).__name__)
            self.run(mol)
...
class ValidationTest(six.with_metaclass(ABCMeta)):
    """"""

    def __call__(self, mol):
        self.log.debug('Running %s', type(self).__name__)
        return self.test(mol)

    @abstractmethod
    def test(self, mol):
        """Each ValidationTest subclass must implement a test method."""
        pass


class SmartsTest(ValidationTest):
    """"""

    def __init__(self, smarts_string, entire_fragment=False):
        """

        :param smarts_string: SMARTS pattern as a string.
        :param bool entire_fragment: (Optional) Whether the SMARTS pattern should match an entire covalent unit.
        """
        self.smarts_string = smarts_string
        self.entire_fragment = entire_fragment

    @memoized_property
    def smarts(self):
        """Lazily loaded RDKit Mol from SMARTS."""
        return Chem.MolFromSmarts(self.smarts)

    def _check_matches(self, mol):
        if mol.HasSubstructMatch(self.smarts):
            self.log.log(self.level, self.message, {'smarts': self.smarts})

    def _check_matches_fragment(self, mol):
        matches = frozenset(frozenset(match) for match in mol.GetSubstructMatches(self.smarts))
...
# MolecularWeightTest (greater than, less than)
#
...
class SmartsValidation(Validation):
    ...
    Usage::
        ...
        SmartsValidation('[Cl]-[#6]-[#6]-[Cl]', INFO, '1,2-Dichloroethane is present', True)

    """

    def __init__(self, smarts_string, level=INFO, message='Molecule matched %(smarts)s', entire_fragment=False):
        """

        :param smarts_string: SMARTS pattern as a string.
        :param level: Validation level.
        :param string message: (Optional) The message to log if the SMARTS pattern matches the molecule.
        :param bool entire_fragment: (Optional) Whether the SMARTS pattern should match an entire covalent unit.
        """
        self.smarts_string = smarts_string
        self.level = level
