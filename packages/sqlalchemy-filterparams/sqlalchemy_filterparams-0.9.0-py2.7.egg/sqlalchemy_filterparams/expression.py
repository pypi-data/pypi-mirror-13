    ...
    def _evaluate_join_path(self, query, path):
        ...
        for path_item in path:
            ...
            if current_path not in self._join_cache:
                ...
                if (not hasattr(relation, u'property') or
                        not hasattr(relation.property, u'mapper') or
                        not hasattr(
                            relation.property.mapper, u'class_')):
                    raise ValueError(
                        (
                            u'The specified join path '
                            u'from the base obj %s has a '
                            u'join configuration in %s '
                            u'which is not a relationship.'
...
class ExpressionItem(object):
    ...
    def join_path(self):
        if isinstance(self._data, dict):
            join_path = self._data.get(u'join', None)
        else:
            join_path = None

        if join_path is None:
            return tuple()
        elif isinstance(join_path, unicode):
            join_path = (join_path,)
        return join_path
    ...
    def param(self):
        if not isinstance(self._data, dict):
            return self._data
        return self._data.get(u'param', None)
