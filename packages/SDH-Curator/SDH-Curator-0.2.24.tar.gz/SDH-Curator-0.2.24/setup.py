...
with open("sdh/curator/metadata.yml", 'r') as stream:
...
setup(
    ...
    install_requires=['shortuuid', 'pika', 'flask', 'Flask-Negotiate', 'redis', 'hiredis',
                      'networkx', 'Agora-Client', 'pyYAML'],
    classifiers=[],
    scripts=['curator'],
    package_dir={'sdh.curator': 'sdh/curator', 'sdh.curator.server': 'sdh/curator/server'},
    package_data={'sdh.curator.server': ['templates/*.*', 'static/*.*'], 'sdh.curator': ['metadata.yml']},
    data_files=[('sdh/curator', ['sdh/curator/metadata.yml'])],
)
