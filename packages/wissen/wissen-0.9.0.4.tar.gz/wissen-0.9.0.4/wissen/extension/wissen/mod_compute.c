...
static int
ATbetween (Operator* ops, int min, int max, char operate, int set, int want)
{
	int _shift;
	unsigned char _uchar;
	int i, ic, k, prev = 0;
...
static int
ATbit (Operator* ops, int bit, char* smode, char operate, int set, int want)
{
	int _shift;
	unsigned char _uchar;
...
static int
ATdistance (Operator* ops, int baselat, int baselong, int pydistance, char operate, int set)
{
	int _shift;
	unsigned char _uchar;
	int i, ic, k, prev = 0;
	...
	delta = (int) (distance / (LATITUDE1DEGREE / precision)); /* meters per 1 / 10000 degree */
	maxlat = baselat + delta;
	minlat = baselat - delta;
	delta = (int) (distance / (LONGITUDE1DEGREE / precision));
...
static int ATsortset_coord (Operator* ops, int baselat, int baselong) {
	unsigned char _uchar;
	int _shift, i;
	FREQ* hfreq;
	int otherlat, otherlong, eachsize;
...
Compute_between (Compute *self, PyObject *args) {
	int res;
	int min = -1, max = -1, want = -1, set = 0;
	char operate = '*';
	if (!PyArg_ParseTuple(args, "ii|cii", &min, &max, &operate, &set, &want)) return NULL;
...
Compute_bit (Compute *self, PyObject *args) {
	int res;
	int bit = -1, want = -1, set = 0;
	char operate = '*';
	char* smode;
	
	if (!PyArg_ParseTuple(args, "is|cii", &bit, &smode, &operate, &set, &want)) return NULL;
...
Compute_distance (Compute *self, PyObject *args) {
	int res;
	int baselat, baselong;
	int pydistance, set;
	char operate = '*';	
	if (!PyArg_ParseTuple(args, "iii|ci", &baselat, &baselong, &pydistance, &operate, &set)) return NULL;
...
Compute_sortset_coord (Compute *self, PyObject *args) {
	...
	if (!PyArg_ParseTuple(args, "ii", &baselat, &baselong)) return NULL;
