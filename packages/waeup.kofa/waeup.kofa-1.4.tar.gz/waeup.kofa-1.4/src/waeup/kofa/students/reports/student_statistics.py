## $Id: student_statistics.py 13236 2015-09-03 15:12:03Z henrik $
##
    ...
    def update(
        ...
        if CREATE and session:
            ...
            grok.getSite().logger.info(
                '%s - report %s created: %s (session=%s, mode=%s, level=%s, breakdown=%s)' % (
...
class StudentStatisticsReportPDFView(grok.View):
    ...
    def _filename(self):
        return 'StudentStatisticsReport_%s_%s_%s_%s.pdf' % (
