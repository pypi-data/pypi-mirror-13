## $Id: browser.py 13610 2016-01-13 20:59:17Z henrik $
##
...
from waeup.kofa.interfaces import (
    ...
    academic_sessions_vocab, IJobManager, IDataCenter, DOCLINK)
...
from waeup.kofa.university.department import VirtualDepartmentExportJobContainer
...
class UnregisterCoursesView(UtilityView, grok.View):
    ...
    def update(self):
        if not self.context.__parent__.is_current:
            emit_lock_message(self)
            return
        elif not self.context.course_registration_allowed:
            self.flash(_(
    ...
    def _registerCourses(self, **data):
        ...
        if max_credits and self.context.total_credits > max_credits:
            self.flash(_('Maximum credits of ${a} exceeded.',
                mapping = {'a':max_credits}), type="warning")
            return
        if not self.context.course_registration_allowed:
            self.flash(_(
                "Course registration has ended. "
                "Please pay the late registration fee."), type="warning")
            #self.redirect(self.url(self.context))
            return
        IWorkflowInfo(self.context.student).fireTransition(
            'register_courses')
