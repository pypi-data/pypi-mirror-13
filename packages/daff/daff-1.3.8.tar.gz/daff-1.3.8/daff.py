...
class Coopy(object):
	...
	def coopyhx(self,io):
		...
		if (cmd1 == "diff"):
			...
			ct1 = Coopy.compareTables3(parent,a,b,self.flags)
			align = ct1.align()
			td = TableDiff(align, self.flags)
			o = SimpleTable(0, 0)
			os = Tables(o)
	...
	def diffAsAnsi(local,remote,flags = None):
		...
		render = TerminalDiffRender(flags)
	...
	def show(t):
		...
		while ((_g < h)):
			...
			while ((_g1 < w)):
				...
				txt = (HxOverrides.stringOrNull(txt) + " ")
			txt = (HxOverrides.stringOrNull(txt) + "\n")
		haxe_Log.trace(txt,_hx_AnonObject({'fileName': "Coopy.hx", 'lineNumber': 1034, 'className': "Coopy", 'methodName': "show"}))
...
Coopy = _hx_classes.registerClass("Coopy", fields=["format_preference","delim_preference","extern_preference","output_format","output_format_set","nested_output","order_set","order_preference","io","pretty","strategy","css_output","fragment","flags","mv","status","daff_cmd"], methods=["init","checkFormat","setFormat","getRenderer","applyRenderer","renderTable","renderTables","saveTable","encodeTable","saveTables","saveText","jsonToTables","jsonToTable","loadTable","command","installGitDriver","coopyhx"], statics=["VERSION","diffAsHtml","diffAsAnsi","diff","getBlankTable","align","patch","compareTables","compareTables3","keepAround","cellFor","main","show","jsonify","tablify"])(Coopy)
		...
		while ((_g1 < _g)):
			...
			if ((cact == "+++") and ((code != "---"))):
				if ((txt is not None) and ((txt != ""))):
					...
					have_column = True
			if updated:
				rc.cond.h[name] = self.cellInfo.lvalue
				rc.val.h[name] = self.cellInfo.rvalue
			elif (code == "+++"):
				if (cact != "---"):
					rc.val.h[name] = txt
			elif ((cact != "+++") and ((cact != "---"))):
...
class TableIO(object):
	...
	def sendToBrowser(self,html):
		haxe_Log.trace("do not know how to send to browser in this language",_hx_AnonObject({'fileName': "TableIO.hx", 'lineNumber': 175, 'className': "TableIO", 'methodName': "sendToBrowser"}))

	@staticmethod
	def _hx_empty_init(_hx_o):		pass
TableIO = _hx_classes.registerClass("TableIO", methods=["getContent","saveContent","args","writeStdout","writeStderr","command","async","exists","isTtyKnown","isTty","openSqliteDatabase","sendToBrowser"])(TableIO)
