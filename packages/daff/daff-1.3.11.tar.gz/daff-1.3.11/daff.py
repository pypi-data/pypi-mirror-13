...
class Coopy(object):
	...
	def runDiff(self,parent,a,b,flags,output):
		...
		if (flags.terminal_format is None):
			if ((((output is None) or ((output == "-")))) and ((self.output_format == "copy"))):
	...
	def run(self,args,io = None):
		if (io is None):
			io = TableIO()
		if (io is None):
			haxe_Log.trace("No system interface available",_hx_AnonObject({'fileName': "Coopy.hx", 'lineNumber': 684, 'className': "Coopy", 'methodName': "run"}))
			return 1
		self.init()
		self.io = io
		more = True
	...
	def show(t):
		...
		while ((_g < h)):
			...
			while ((_g1 < w)):
				...
				txt = (HxOverrides.stringOrNull(txt) + " ")
			txt = (HxOverrides.stringOrNull(txt) + "\n")
		haxe_Log.trace(txt,_hx_AnonObject({'fileName': "Coopy.hx", 'lineNumber': 1060, 'className': "Coopy", 'methodName': "show"}))
