...
@click.command(help="Set up TFTP root for PXE booting")
@click.option("-r", "--tftpboot", default="/var/lib/tftpboot", help="Path to the root of TFTP server")
def setup():
    URL_BUILDS = "https://www.koodur.com/butterknife/provisioning-image/"
    filenames = (
        (
            URL_BUILDS + "butterknife-2015.08.1-amd64.bin",
            os.path.join(tftpboot, "butterknife", "butterknife-amd64.bin")
        ), (
            URL_BUILDS + "butterknife-2015.08.1-i386.bin",
            os.path.join(tftpboot, "butterknife", "butterknife-i386.bin")
        ), (
            "/usr/lib/syslinux/pxelinux.0",
            os.path.join(tftpboot, "butterknife", "pxelinux.0")
        ), (
            "/boot/memtest86+.bin",
            os.path.join(tftpboot, "butterknife", "memtest86+.bin")
        ), (
            "/usr/lib/syslinux/*.c32",
            "/var/lib/tftpboot/butterknife/"
        )
    )
    
#    for source, dest in filenames:
