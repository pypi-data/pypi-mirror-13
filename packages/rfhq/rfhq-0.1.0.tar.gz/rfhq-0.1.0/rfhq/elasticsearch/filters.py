...
class TermsFilter(QueryParamFilterBuilderMixin, FilterOptionsMixin, BaseFilterBackend):
    ...
    def get_fields(self, view):
        return getattr(view, 'terms_filters', []) + self.get_option(view, 'fields', []) + self.get_option(view, 'field_map', {}).keys()
...
class GeoDistanceFilter(FilterOptionsMixin, BaseFilterBackend):
    ...
    def get_valid_params(self, view):
        return self.get_option(view, 'fields', []) + self.get_option(view, 'field_map', {}).keys()
    ...
    def get_filters(self, request, view):
        ...
        filters = []
        for field, values in params.iteritems():
...
class NumericRangeFilter(FilterOptionsMixin, BaseFilterBackend):
    ...
    VALID_SUFFIXES = [GREATER_THAN, GREATER_THAN_OR_EQUAL, LESS_THAN, LESS_THAN_OR_EQUAL, TIME_ZONE] + OPERATOR_SYNONYMS.keys()

    def get_valid_filters(self, view):
        filters = self.get_option(view, 'fields', []) + self.field_map.keys()
        if isinstance(filters, six.string_types):
            filters = (filters,)
...
class OrderingFilter(FilterOptionsMixin, filters.OrderingFilter):
    ...
    def get_ordering(self, request, queryset, view):
        ...
        for directive in directives:
            ...
            ordering.append({six.binary_type(field): {six.binary_type("order"): six.binary_type(direction)}})
        return ordering
