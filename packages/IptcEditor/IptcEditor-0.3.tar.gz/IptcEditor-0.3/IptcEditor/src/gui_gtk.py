...
from IptcEditor.src.tag_editor import TagEditor
...
class NbLayout(Gtk.Window):
    def __init__(self):
        ...
        # # set default image
        self.default_image = '{}/resources/default.png'.format(project_root)
    ...
    def nb_page_2(self):
        ...
        new_tag_label.set_markup('<b>New key phrase</b>')
        ...
        replace_button = Gtk.Button('Refresh Key Phrase(s)')
        ...
        tagger_grid.attach(self.tag_to_replace, 1, 1, 2, 1)
        ...
        tagger_grid.attach(message_scroller, 0, 7, 1, 1)
        tagger_grid.attach(thumbnail_window, 1, 6, 1, 3)
    ...
    def set_thumbnail(self):
        ...
        if fn and not dn:
            ...
            if m.file(fn) in self.editor.get_accepted_formats():
                pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(fn, self.thumbnail_width, -1, True)
        # # if directory
        elif not fn and dn:
            # set thumbnail title
            ...
            if file_list:
                pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(file_list[0], self.thumbnail_width, -1, True)
        if pixbuf:
    ...
    def replace_clicked(self, button):
        ...
        tags = self.editor.get_tags(new_keyphrase_list, self.editor.get_filename()) if new_keyphrase_list else None
        print('TAGS: {}'.format(tags))
        if self.editor.replace_keyphrases(tags,
                                          new_keyphrase_list,
                                          working_data,
                                          self.tag_to_replace.get_text(),
                                          self.new_tag_input.get_text()) is not False:
    ...
    def remove_clicked(self, button):
        ftr = [self.my_filter(fields) for fields in [self.remove_field_input.get_text()]]
        self.editor.remove_exif_field(ftr)
