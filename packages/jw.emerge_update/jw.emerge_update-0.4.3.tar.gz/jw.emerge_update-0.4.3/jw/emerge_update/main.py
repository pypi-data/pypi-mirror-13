...
def Main():
    ...
    class Program(object):
        ...
        def run(self):
            ...
            if packageList:
                self.do('qcheck ' + ' '.join('"%s"' % p.rstrip() for p in packageList))
            ...
            if st:
                if glob('/etc/portage/.cfg*'):
                    ...
                    pythonUpdated = any('dev-lang/python' in line for line in self.do('{EMERGE} @world'))
                else:
                    raise
            else:
                pythonUpdated = any('dev-lang/python' in line for line in emerge)
            if any('>>> Installing' in line for line in emerge):
