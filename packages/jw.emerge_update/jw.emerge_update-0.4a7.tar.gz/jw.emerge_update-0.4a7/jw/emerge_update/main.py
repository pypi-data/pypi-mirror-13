...
def Main():
    ...
    class Program(object):
        ...
        def run(self):
            ...
            st, packageList = self.do('qcheck --badonly --all')
            if st:
                self.do('qcheck ' + ' '.join('"%s"' % p for p in packageList))
            self.do('eix-sync -v')
            self.backup('/etc')
            ...
            if any(b'No outdated packages were found on your system' in line for line in emerge):
                self.log('No package maintenance done (no packages installed/updated)')
            else:
