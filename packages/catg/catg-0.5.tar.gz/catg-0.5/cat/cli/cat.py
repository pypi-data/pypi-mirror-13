...
def upload():
    ...
    git_path = os.path.join(build_path, '.git')
    if not os.path.exists(git_path):
        os.chdir(build_path)
        os.popen('git init')
    else:
        git_target_path = src_path
        shutil.move(git_path, src_path)
        os.popen('python blog.py build')
        shutil.move(os.path.join('src_path', '.git'), build_path)
        os.popen('python blog.py upload')
        logger.info('''
            \033[33m{info}\033[0m
