...
class ThorinUtils:
    ...
    def post_data(self):
        ...
        if ctype == 'application/json':
            postvars = self.req.rfile.read(int(self.req.headers['Content-Length'])).decode('utf-8')
            if postvars:
                postvars = json.loads(postvars)
        elif ctype == 'application/x-www-form-urlencoded' or ctype == 'multipart/form-data':
            ...
            if form:
                for key in form:
                    postvars[key] = form.getvalue(key)
