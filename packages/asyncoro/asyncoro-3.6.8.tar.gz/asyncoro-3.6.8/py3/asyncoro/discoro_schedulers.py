...
class RemoteCoroScheduler(object):
    ...
    def submit_at(self, where, gen, *args, **kwargs):
        """Similar to 'run_at' method of computation, except the calling
        coroutine is blocked until any server is discovered and initialized;
        unlike 'schedule', this method doesn't wait for server to be free (i.e.,
        not running any other coroutines), nor unlike 'execute_at', the caller
        is not blocked until the coroutine finishes.
        ...
        """
        if not where:
            if not self._rcoros and not self._servers:
                yield self._server_avail.wait()
        rcoro = yield self.computation.run_at(where, gen, *args, **kwargs)
        if isinstance(rcoro, Coro):
