...
class fab_kloop(kloop, ui_fab_kloop):
	...
	def produce_if_needed(self):
		...
		'''
		logging.debug("test production %s : nb_vide=%s / red_zone = %s"%(self, self.nb_kanban(kanban.empty), self.red_zone))
		if (self.nb_kanban() - self.nb_kanban(kanban.empty))<=self.red_zone:
