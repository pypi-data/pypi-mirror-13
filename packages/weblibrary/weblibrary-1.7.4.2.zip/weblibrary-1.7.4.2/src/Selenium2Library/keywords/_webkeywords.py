...
class _WebKeywords(KeywordGroup):
	...
	def web_upload_file(self, filepath):
		...
		filepath 文件路径，一般是绝对路径，注意写法，例如：${CURDIR}\\Res\\Plus_Web\\pic0.jpg
		${CURDIR}指用例运行的当前路径
	...
	def web_click_text_button(self, text):
		"""点击可以通过按钮内文字定位的Button

		text 为按钮内的文字
		如下的控件可以使用：<a href="/mass">群发消息</a>
		| Web Click Text Button | ${text} |
		| Web Click Text Button | 群发消息 |
		"""			
		self.web_click_element('link='+text)	

	def web_input_text(self, locator, text):
		"""向文本框中输入文本
	...
	def web_click_chosen_element(self, locator, chosen_num):
		"""选择符合条件:locator定义的第chosen_num个元素，并点击它

		locator 元素定位
		chosen_num 符合的第chosen_num个元素
	...
	def _get_selected_element(self, locator, selected_num):
		"""返回选中的元素
		...
		if index <= len(elements_list):
			...
			raise AssertionError("ERROR: Element selected_num %d is out of the length of elements_list" % selected_num)