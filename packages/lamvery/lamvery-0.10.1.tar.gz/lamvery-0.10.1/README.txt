...
-  | ``-s`` or ``--single-file``
   ...
   | Archive only the lambda function file, so you can inline edit in
...
-  | ``-k`` or ``--keep-empty-events``
   | This option is only needed by the ``events`` command. Keep the
     empty CloudWatch Event Rule that does not have CloudWatch Event
     Target.
...
-  | ``-v`` or ``--version``
   | This option is needed by the
     ``set-alias``,\ ``invoke``,\ ``rollback`` commands.
...
\`\`\`js var lamvery = require('./lamvery.js');

exports.lambda\_handler = function(event, context) {
lamvery.secret.get('foo', function(err, data) { console.log(data); }); }
