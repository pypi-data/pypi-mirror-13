"""
module catanlog introduces a machine-parsable, human-readable log of all actions made in a game of Catan.
...
class Reader(object):
    """
    class Reader reads game logs in the .catan format. It works like a token parser,
    reading in chunks of the game log and returning a new catan.game.Game object.

    See module [catan](https://github.com/rosshamish/catan-py) for the definition of
    catan.game.Game (and other containing and useful types).
    """
    ...
    def next(self, game):
        """
        Perform the next action in the game log, returning a new catan.game.Game
        in which the action has been performed.
        """
    ...
    def _read_game_start(self, line):
        pass

    def _read_player_roll(self):
        pass

    def _method_for_line(self, line):
        methods = {
            r'': ''
        }
        pass
...
class Writer(object):
    """
    class Writer writes game logs in the .catan format. It expects some parameters as types
    defined in module [catan](https://github.com/rosshamish/catan-py). See docstrings for
    details.
    ...
    def logpath(self):
        ...
        """
        name = '{}-{}.catan'.format(self.timestamp_str(no_spaces=True),
    ...
    def timestamp_str(self, no_spaces=False):
        if no_spaces:
            fmt = '%Y-%m-%dT%H-%M-%S'
        else:
            fmt = '%Y-%m-%d %H:%M:%S'
        return self._game_start_timestamp.strftime(fmt)
    ...
    def log_robber(self, player, location, victim):
        """
        :param player: catan.game.Player
    ...
    def log_trades_with_player(self, player, to_other, other, to_player):
        """
        :param player: catan.game.Player
    ...
    def log_plays_knight(self, player, location, victim):
        ...
        self.log_robber(player, location, victim)
