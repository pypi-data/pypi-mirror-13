...
def Decoding_BP(H,y,sigma=0.5,max_iter=15):
    ...
    sigma: (default = 0.5) Standard deviation of the channel's AWGN. 

    max_iter: (default = 15) max iterations of the main loop. (for n,k 
    less than 100, 15 is suffiscient in general) 
    ...
    p0 = np.zeros(shape=n)
    p0 = algebra.f1(y,sigma)/(algebra.f1(y,sigma) + algebra.fM1(y,sigma))
    p1 = np.zeros(shape=n)
    p1 = algebra.fM1(y,sigma)/(algebra.f1(y,sigma) + algebra.fM1(y,sigma))
    ...
    while(True):
        ...
        for i in range(m):
            Ni=algebra.Nimj(H,i,n+1)
            for j in Ni:
                Nij = algebra.Nimj(H,i,j)
        ...
        for j in range(n):
            Mj = algebra.Mjmi(H,m+1,j)
            for i in Mj:
                Mji = algebra.Mjmi(H,i,j)
        ...
        if algebra.InCode(H,x) or count > max_iter:  
            break
  
    return x


def Decoding_logBP(H,y,sigma=1,max_iter=15):
...
def DecodedMessage(G,x):
    ...
    tG=np.transpose(G)
    rtG, rx = algebra.GaussElimination(tG,x)

    message=np.zeros(k).astype(int)

    message[k-1]=rx[k-1]
    for i in reversed(range(k-1)):
        message[i]=abs(rx[i]-algebra.BinaryProduct(rtG[i,list(range(i+1,k))],message[list(range(i+1,k))]))
