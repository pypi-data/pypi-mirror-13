...
class Piwigo(piwigo.Piwigo):
    ...
    def _setcategories(self):
        self._cat= { "/%s" % i["name"].replace(" / ","/") : i["id"] for i in self.pwg.categories.getList(recursive=True, fullname=True)['categories'] }
    ...
    def idcategory(self, path):
        if not self.iscategory(path):
            raise PiwigoException("category %s not exist" % path)
    ...
    def mkdir(self, path, **kw):
        ...
        if parent and not self.iscategory(parent):
            raise PiwigoException("category %s not exist" % parent)
        if parent : kw['parent'] = self._cat[parent]
