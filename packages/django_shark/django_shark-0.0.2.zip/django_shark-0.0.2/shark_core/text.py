...
class Anchor(BaseObject):
    ...
    def get_html(self, html, indent):
        #TODO: Make js safe
        html.append('<a href="' + self.url + '"' +
                    self.base_attributes +
                    '>' + html.inline_render(self.text) + '</a>')
...
class Heading(BaseObject):
    ...
    def get_html(self, html, indent):
        ...
        html.render('    ', self.text, indent)
        if self.subtext:
            html.append('<small>')
            html.render('    ', self.subtext, indent)
            html.append('</small>')
