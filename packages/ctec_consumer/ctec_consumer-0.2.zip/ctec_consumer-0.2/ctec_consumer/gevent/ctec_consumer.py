...
class Consumer(object):
    ...
    def on_message(self, unused_channel, basic_deliver, properties, body):
        ...
        try:
            self.pool.spawn(self.message_worker, WorkerMessage(basic_deliver, properties, body))
        except AssertionError:
            self.logger.warning('线程池已关闭！不接受新任务！回退消息到队列！message = %s', body)
            self.nack_message(basic_deliver.delivery_tag)
        except Exception as e:
            self.logger.error('消息消费失败！delivery_tag = %s, error message = %s', basic_deliver.delivery_tag, e.message)
    ...
    def message_worker(self, worker_message):
        ...
        delivery_tag = worker_message.basic_deliver.delivery_tag
        ...
        if result == CONSUME_SUCCESS:
            ...
            self.logger.warning('未找到处理code。code = %s', result)
