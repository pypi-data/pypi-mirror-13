...
def sim_run(simsdir='~/.simulations', sourcedir='.', cloud=True,
    ...
    if os.path.exists(os.path.join(sourcedir, 'params.cfg')):
        ...
        sim_params = SimParams(os.path.join(sourcedir, 'params.cfg'), is_old_input_type=False)
    else:
        print '[sim-run] reading input.params'
        sim_params = SimParams(os.path.join(sourcedir, 'input.params'), is_old_input_type=True)
            ...
            with cd(simdir):
                with rollbackwrap(simdir):
                    ...
                    if particles_from:
                        ...
                        run('scp probe@argo.physics.muni.cz:{} sim.h5'.format(tmp_h5_file))
...
def _sim_cuth5(sim, last_run_number, start_run_number=0, move=False, ts_shift=0, h5file='sim.h5',
    ...
    input_params = run('cat {}'.format(os.path.join(sim['simdir'], input_params)), quiet=True).stdout
    ...
    index = names.index('ts_{:010d}'.format(int(sp.params['ntimes'] * (last_run_number - start_run_number) + ts_shift)))
