...
from tgbot import mainmessage, iniconfig
...
def sendmessage(oldvalues={},markdown=True,**kwargs):
    ...
    checkanswer(mainmessage,answer)
...
def checkanswer(message, answer):
    error_msg = iniconfig.get("error_messages","403")
    user = message.from_User
    error_code = answer.get("error_code")
    if error_code == 403:
        sendtext(message.chat_id(),"%s, %s \n"
                                   "Fehlermeldung: ```%s```"%(user.first_name,error_msg,answer.get("description")))