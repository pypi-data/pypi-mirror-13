...
class AgentServer extends Server
{
    ...
    {
        $params ['connected_status'] = 'online';
        $params ['contact_id'] = WagentProt::ParseID($from);
        $params ['id'] = '';
    ...
    {
        $params ['contact_id'] = WagentProt::ParseID($from);
        $params ['connected_status'] = 'offline';
    ...
    function _HandleMessage (Socket $client, $message)
    {
        switch ($message) {
            ...
            case 'getconnectedstatus':
                $status = $this->GetConnectedStatus();
    ...
    function CleanupPresenceResults ($results)
    {
        $connected_statuses = [];

        foreach($results as $result) {

            $connected_status['contact_id']       = $result['contact_id'];
            $connected_status['connected_status'] = $result['connected_status'];
            if (!empty($results['last_seen']))
                $connected_status['last_seen'] = $result['last_seen'];

            $connected_statuses [] = $connected_status;
        }
    ...
    function SendGetPresences($to)
    {
        ...
        $clean_result = $this->CleanupPresenceResults ($results);
