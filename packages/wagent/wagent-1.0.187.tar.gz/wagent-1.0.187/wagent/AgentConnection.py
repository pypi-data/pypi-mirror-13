...
def __create_process(server_path, agent_path, timeout):
    p = Popen(["php", "-f", server_path, agent_path, timeout],
                       stdout=subprocess.PIPE,
                       stderr=subprocess.PIPE)
    p.stdout.readline()


def __start_server_process(server_path, agent_path, timeout, event):
    __create_process(server_path, agent_path, timeout)
    event.set()


def start_and_wait_server(agent_server, agent_path, timeout):
    event = Event()
    thread = Thread(target=__start_server_process, args=(agent_server, agent_path, timeout, event))
    thread.start()
    event.wait(5)
...
class AgentConnection:
    ...
    def __readSubprocessStdout(self,process):
        thread = Thread(target = execute, args = (process, ))
        thread.start()

    def __startServer(self, agent_path, timeout):
        agent_server = self.__getServerPath()
        start_and_wait_server(agent_server, agent_path, timeout)

    def __getOrCreateConnection(self, phoneNumber, timeout):
        ...
        while conn is None and tries < 3:
            ...
            if conn is not None:
                return conn
            self.__startServer (agent_path, timeout)
            tries = tries + 1
        return conn

    def __init__(self, phoneNumber, timeout = "120"):
        self.conn = self.__getOrCreateConnection (phoneNumber, timeout)
