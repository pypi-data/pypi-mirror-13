...
class WinduAgent:
    ...
    def __init__(self, phoneNumber, nickName = '', password='', autoReply=AUTOREPLY_NONE):
    ...
    def __ensureLiteConnected(self):
        ...
        while tries < 2:
            if (self.conn == None):
                self.conn = AgentConnection.AgentConnection(self.phoneNumber)
            try:
    ...
    def __ensureConnected(self):
        ...
        if not self.conn.isConnected():
            raise AgentException.AgentException("Fail connecting to agent_server")
