...
def writeplot(OUTFNAME,fig,plt):
    fig.savefig(OUTFNAME)

    plt.close()

def plot(counts,minlength=18,maxlength=33,title=None,ylims=None,stranded=False,noN=True,fwonly=False,rvonly=False):
    ...
    for n in nucs:
        if n[0] in df.index:
            ax.bar(idx, df.loc[n[0]], color=n[1], bottom=bottom, width=width,label=n[0])
            bottom += df.loc[n[0]]
            if stranded:
    ...
    if ext == ".bam":
        ...
        if chrnames:
            ...
            for chrname in chrnames:
                reads += bamfile.fetch(chrname) #get it into a list, otherwise it doesn't rewind after loop #TODO: use multi_iterators param
        else:
            reads = bamfile.fetch() #get it into a list, otherwise it doesn't rewind after loop #TODO: use multi_iterators param
...
if __name__ == "__main__":
    ...
    if args["ylims"]:
        ylims = [float(lim) for lim in ylims.split(",")]
    ...
    f,p = plot(counts,args["minlength"],args["maxlength"],title=title,ylims=ylims,stranded=args["stranded"],fwonly=args["fwonly"],rvonly=args["rvonly"])
    writeplot(args["OUTFNAME"],f,p)
