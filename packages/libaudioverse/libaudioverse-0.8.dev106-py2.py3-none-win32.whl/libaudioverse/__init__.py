...
class ObjectTypes(enum.IntEnum):
    ...
    one_pole_filter_node = 36
    

    first_order_filter_node = 37
    

    allpass_node = 38
    

    nested_allpass_network_node = 39
    ...
    leaky_integrator_node = 43
    

    file_streamer_node = 44
...
Any functionality to change a property on all sources needs to be implemented by the app, and is not offered by Libaudioverse."""
    ...
    def init_with_handle(self, handle):
        with _object_states_lock:
            ...
            if should_add_properties:
                ...
                self._state['properties']["default_distance_model"] = _libaudioverse.Lav_ENVIRONMENT_DEFAULT_DISTANCE_MODEL

                self._state['properties']["default_max_distance"] = _libaudioverse.Lav_ENVIRONMENT_DEFAULT_MAX_DISTANCE

                self._state['properties']["default_panning_strategy"] = _libaudioverse.Lav_ENVIRONMENT_DEFAULT_PANNER_STRATEGY
            ...
            self._property_instances[_libaudioverse.Lav_ENVIRONMENT_DEFAULT_DISTANCE_MODEL] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_ENVIRONMENT_DEFAULT_DISTANCE_MODEL, enum = DistanceModels)
            ...
            self._property_instances[_libaudioverse.Lav_ENVIRONMENT_DEFAULT_PANNER_STRATEGY] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_ENVIRONMENT_DEFAULT_PANNER_STRATEGY, enum = PanningStrategies)
    ...
    @property
    def default_distance_model(self):   
        """Type: int
...
Distance models control how quickly sources get quieter as they move away from the listener."""
        return self._property_instances[_libaudioverse.Lav_ENVIRONMENT_DEFAULT_DISTANCE_MODEL]


    @default_distance_model.setter
    def default_distance_model(self, value):
        self.default_distance_model.value=value
...
The max distance of a source is the maximum distance at which that source will be audible."""
    ...
    @property
    def default_panning_strategy(self):   
        """Type: int
...
the default panner strategy for the internal panner of new sources."""
        return self._property_instances[_libaudioverse.Lav_ENVIRONMENT_DEFAULT_PANNER_STRATEGY]


    @default_panning_strategy.setter
    def default_panning_strategy(self, value):
        self.default_panning_strategy.value=value
    ...
    def init_with_handle(self, handle):
        with _object_states_lock:
            ...
            if should_add_properties:
                ...
                self._state['properties']["panner_strategy"] = _libaudioverse.Lav_SOURCE_PANNER_STRATEGY
            ...
            self._property_instances[_libaudioverse.Lav_SOURCE_PANNER_STRATEGY] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_SOURCE_PANNER_STRATEGY, enum = PanningStrategies)
...
The strategy for the internal multipanner."""
        return self._property_instances[_libaudioverse.Lav_SOURCE_PANNER_STRATEGY]


    @panner_strategy.setter
    def panner_strategy(self, value):
        self.panner_strategy.value=value
    ...
    def init_with_handle(self, handle):
        with _object_states_lock:
            ...
            if should_add_properties:
                ...
                self._state['properties']["has_center"] = _libaudioverse.Lav_PANNER_HAS_CENTER

                self._state['properties']["has_lfe"] = _libaudioverse.Lav_PANNER_HAS_LFE

                self._state['properties']["passthrough"] = _libaudioverse.Lav_PANNER_PASSTHROUGH

                self._state['properties']["should_crossfade"] = _libaudioverse.Lav_PANNER_SHOULD_CROSSFADE

                self._state['properties']["skip_center"] = _libaudioverse.Lav_PANNER_SKIP_CENTER

                self._state['properties']["skip_lfe"] = _libaudioverse.Lav_PANNER_SKIP_LFE
            ...
            self._property_instances[_libaudioverse.Lav_PANNER_HAS_CENTER] = BooleanProperty(handle = self.handle, slot = _libaudioverse.Lav_PANNER_HAS_CENTER)
            

            
            
            self._property_instances[_libaudioverse.Lav_PANNER_HAS_LFE] = BooleanProperty(handle = self.handle, slot = _libaudioverse.Lav_PANNER_HAS_LFE)
            

            
            
            self._property_instances[_libaudioverse.Lav_PANNER_PASSTHROUGH] = FloatProperty(handle = self.handle, slot = _libaudioverse.Lav_PANNER_PASSTHROUGH)
            ...
            self._property_instances[_libaudioverse.Lav_PANNER_SKIP_CENTER] = BooleanProperty(handle = self.handle, slot = _libaudioverse.Lav_PANNER_SKIP_CENTER)
            

            
            
            self._property_instances[_libaudioverse.Lav_PANNER_SKIP_LFE] = BooleanProperty(handle = self.handle, slot = _libaudioverse.Lav_PANNER_SKIP_LFE)
...
If it does, this is assumed to be channel 3."""
        return self._property_instances[_libaudioverse.Lav_PANNER_HAS_CENTER]


    @has_center.setter
    def has_center(self, value):
        self.has_center.value=value
...
If it does, this is assumed to be channel 4."""
        return self._property_instances[_libaudioverse.Lav_PANNER_HAS_LFE]


    @has_lfe.setter
    def has_lfe(self, value):
        self.has_lfe.value=value
...
Specifically, the input signal is added to all outputs with ``passthrough`` as the gain, and ``1-passthrough`` as the gain of the panned signal."""
        return self._property_instances[_libaudioverse.Lav_PANNER_PASSTHROUGH]
...
You  almost always want this on."""
        return self._property_instances[_libaudioverse.Lav_PANNER_SKIP_CENTER]


    @skip_center.setter
    def skip_center(self, value):
        self.skip_center.value=value
...
You  almost always want this on."""
        return self._property_instances[_libaudioverse.Lav_PANNER_SKIP_LFE]


    @skip_lfe.setter
    def skip_lfe(self, value):
        self.skip_lfe.value=value
...
class PannerBankNode(GenericNode):
    r"""A bank of panners, separated to cover a configurable cone.

The primary use for this node is to simulate virtual speakers.

The number of panners is set by the *panner_count* parameter to the constructor.
This must be at least 2.
Each input corresponds to a panner, starting with the panner set to the lowest angle."""
    
    def __init__(self, simulation, panner_count, hrtf_path):
        super(PannerBankNode, self).__init__(_lav.create_panner_bank_node(simulation, panner_count, hrtf_path))

    def init_with_handle(self, handle):
        with _object_states_lock:
            #our super implementation adds us, so remember if we weren't there.
            should_add_properties = handle.handle not in _object_states
            super(PannerBankNode, self).init_with_handle(handle)

            if should_add_properties:

                self._state['properties']["azimuth"] = _libaudioverse.Lav_PANNER_AZIMUTH

                self._state['properties']["count"] = _libaudioverse.Lav_PANNER_BANK_COUNT

                self._state['properties']["is_centered"] = _libaudioverse.Lav_PANNER_BANK_IS_CENTERED

                self._state['properties']["should_crossfade"] = _libaudioverse.Lav_PANNER_SHOULD_CROSSFADE

                self._state['properties']["spread"] = _libaudioverse.Lav_PANNER_BANK_SPREAD
            ...
            self._property_instances[_libaudioverse.Lav_PANNER_BANK_COUNT] = IntProperty(handle = self.handle, slot = _libaudioverse.Lav_PANNER_BANK_COUNT)
            
            

            
            
            self._property_instances[_libaudioverse.Lav_PANNER_BANK_IS_CENTERED] = BooleanProperty(handle = self.handle, slot = _libaudioverse.Lav_PANNER_BANK_IS_CENTERED)
            ...
            self._property_instances[_libaudioverse.Lav_PANNER_BANK_SPREAD] = FloatProperty(handle = self.handle, slot = _libaudioverse.Lav_PANNER_BANK_SPREAD)
...
This is the same as the number of inputs to the node."""
        return self._property_instances[_libaudioverse.Lav_PANNER_BANK_COUNT]
...
Otherwise, the azimuth controls the left edge of the cone."""
        return self._property_instances[_libaudioverse.Lav_PANNER_BANK_IS_CENTERED]


    @is_centered.setter
    def is_centered(self, value):
        self.is_centered.value=value
...
This cone always inclues the endpoints, and the panners are separated as ``spread/(panner_count-1)``.
This implies that for ``spread=360.0``, the first and last inputs are effectively the same."""
        return self._property_instances[_libaudioverse.Lav_PANNER_BANK_SPREAD]
...
_types_to_classes[ObjectTypes.panner_bank_node] = PannerBankNode
