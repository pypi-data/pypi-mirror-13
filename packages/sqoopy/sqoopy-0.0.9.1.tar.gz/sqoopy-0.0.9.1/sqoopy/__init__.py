...
class Db(object):
  ...
  def __init__(self, conn_str, **opts):
    ...
    # ignore jdbc for now.
    self.conn_str = conn_str.replace('jdbc:', '')
...
def gen(**opts):
  """
  Generate a list of sqoop commands given options.
  ...
  opts.setdefault('import_args', ['--hive-drop-import-delims'])
  ...
  sqoop import \
    ...
    {import_args}
  """
  # for each table compute optimal number of mappers + format target dir
