...
class TestLoader(unittest.TestCase):
    ...
    def setUp(self):
        ...
        sql = 'CREATE TABLE ' + Query._table_name(Age) \
              + ' (name varchar, age int, uuid varchar);'

        sql += 'CREATE TABLE ' + Query._table_name(Person) + \
               ' (name varchar, lastname varchar, \
               age int, weight int, uuid varchar)'
        execute(sql)

    def tearDown(self):
        sql = 'DROP TABLE ' + Query._table_name(Age) + ';'
        sql += 'DROP TABLE ' + Query._table_name(Person) + ';'
        execute(sql)
