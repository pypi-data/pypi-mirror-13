...
class TestQuerys(unittest.TestCase):
    ...
    def test_insert_query(self):
        ...
        self.assertEqual("INSERT INTO " + Query._table_name(Age) + " (age,name) VALUES (%s,%s)", sql[0])
        self.assertEqual([22.5, 'mouse'], sql[1])

    def test_select_query(self):
        result = ("SELECT * FROM " + Query._table_name(Age) + " WHERE 1=1 AND name=%s LIMIT 30", ['mickey'])
        query = Query(Age).select(['*']).where({"name": 'mickey'}).limit(30).query()
        self.assertEqual(result, query)

        result = ("SELECT * FROM " + Query._table_name(Age) + "\
...
WHERE 1=1 AND age=%s OR name=%s LIMIT 30", [20, 'mickey'])
        ...
        result = ("SELECT * FROM " + Query._table_name(Age) + " WHERE 1=1 AND name=%s LIMIT 1", ['mickey'])
        query = Query(Age).select(['*']).where({"name": 'mickey'}).limit(1).query()
        self.assertEqual(result, query)

        result = ("SELECT * FROM " + Query._table_name(Age) + " LIMIT 1", [])
        query = Query(Age).select(['*']).limit(1).query()
        self.assertEqual(result, query)

        result = ("SELECT a.name FROM " + Query._table_name(Age) + " a WHERE 1=1 AND a.name=%s LIMIT 1", ['mickey'])
...
AND a.age=%s AND p.lastname!=%s LIMIT 1000', ['mickey', 22, 'mouse'])
    ...
    def test_delete(self):
        q = Query(Age).delete()
        r = ("DELETE FROM " + Query._table_name(Age), [])
        self.assertEqual(q, r)

        q = Query(Age).where({'name': 'mouse'}).delete()
        r = ("DELETE FROM " + Query._table_name(Age) + " WHERE 1=1 AND name=%s", ['mouse'])
        self.assertEqual(q, r)

        q = Query(Age).where({'name': 'mouse', 'age': 20}).delete()
        r = ("DELETE FROM " + Query._table_name(Age) + " WHERE 1=1 AND age=%s AND name=%s", [20, 'mouse'])
        self.assertEqual(q, r)

    def test_update(self):
        q = Query(Age).values({"age": 22, "name": "minnie"}).where({"age": 20, "name": "mickey"}).update()
        r = ("UPDATE " + Query._table_name(Age) + " SET age=%s,name=%s \
WHERE 1=1 AND age=%s AND name=%s", [22, "minnie", 20, "mickey"])
        self.assertEqual(q, r)

        q = Query(Age).values({"age": 22, "name": "minnie"}).update()
        r = ("UPDATE " + Query._table_name(Age) + " SET age=%s,name=%s", [22, "minnie"])
        self.assertEqual(q, r)
