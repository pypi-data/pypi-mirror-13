...
def create_pipeline((dp_conn, dynamodb_conn, s3_conn_dest, s3_conn_src,
    ...
    if action == 'restore':
        ...
        tmp_bucket_name = __copy_backup_from_source__(
            s3_conn_dest, region, table, backup, backup_bucket, debug
        )
...
def __copy_backup_from_source__(s3_conn, region, table,
                                backup, src_bucket_name, debug):
    logger = set_logging(level=debug)

    tmp_bucket_name = str(uuid.uuid4())

    dst_bucket = s3_conn.create_bucket(tmp_bucket_name, location='EU')
    src_bucket = s3_conn.get_bucket(src_bucket_name)
    ...
    path_to_backup = 'data/%s/%s/%s' % (region, table, backup)
    for k in src_bucket.list(prefix='%s' % (path_to_backup)):
        logger.debug('Copying %s/%s to %s/%s' % (src_bucket_name,
                                                 k.key,
                                                 tmp_bucket_name,
                                                 k.key))
...
def __increase_pt__(conn,
    ...
    conn.update_throughput(conn.get_table(table),
                           updated_pt['read'],
                           updated_pt['write'])
...
def set_permissions_for_user(bucket_name,
    ...
    print 'XXX: bucket name: %s' % bucket_name
